...
| Deployed receiver refuses missing payload and unsupported providers without changing real stores | Invoked the installed remote `auth import --provider <provider> --stdin --json` over pinned-host SSH with empty EOF, using the real VM home. | **Actual public CLI, no credential fixture:** OpenAI and Claude each returned exit 1 and `The credential transfer payload is invalid.` Copilot returned exit 1 and the OpenAI/Claude-only requirement. Real credential-file metadata was unchanged after every call. A subsequent real `auth status --json` still reported the same five providers as not configured. |
...
At 01:51 UTC the deployed receiver's empty-input and unsupported-provider
boundaries were also exercised directly against the actual VM stores, as recorded
above. No payload containing credentials was supplied. These observations extend
the deployed integration checks without crossing the explicit consent boundary.
