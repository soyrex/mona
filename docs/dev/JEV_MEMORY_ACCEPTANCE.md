...
```sh
MONA_MEMORY_JEV_PROVIDER=openrouter MONA_MEMORY_JEV_LIVE_TEST=1 \
  scripts/dev_cargo.sh test --profile selfdev -p mona-base --lib \
...
Artifacts are in `$MONA_SCRATCH_DIR/jev-gateway-e2e-20260919/`: the opt-in
