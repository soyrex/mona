...
def build_specs() -> dict[str, ToolSpec]:
    jcode = shutil.which("mona") or str(Path.home() / ".local/bin/jcode")
    ...
    specs = {
        "mona_memory_off": ToolSpec(
            name="mona_memory_off",
            argv=[jcode, "--no-update", "--no-selfdev"],
            version_argv=[jcode, "version"],
            env={"MONA_NO_TELEMETRY": "1", "MONA_MEMORY_ENABLED": "0"},
            jcode=True,
        ),
        "mona_memory_on": ToolSpec(
            name="mona_memory_on",
            argv=[jcode, "--no-update", "--no-selfdev"],
            version_argv=[jcode, "version"],
            env={"MONA_NO_TELEMETRY": "1", "MONA_MEMORY_ENABLED": "1"},
            jcode=True,
        ),
        "pi": ToolSpec(
            name="pi",
...
def run_tool(spec: ToolSpec, sessions: int, cwd: Path, timeout_s: float, settle_s: float) -> ToolRunResult:
    ...
    try:
        if spec.jcode:
            temp_root = tempfile.mkdtemp(prefix="mona-memory-bench-")
            env = os.environ.copy()
            if spec.env:
                env.update(spec.env)
            env["MONA_HOME"] = os.path.join(temp_root, "home")
            env["MONA_RUNTIME_DIR"] = os.path.join(temp_root, "run")
            ...
            per_session_settle = max(settle_s, 2.0) if spec.name == "mona_memory_on" else settle_s
