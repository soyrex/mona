...
Use it before/after changing ``MONA_OPENAI_WS_IDLE_RECONNECT_SECS`` (or the
...
With no arguments it scans ~/.mona/logs/mona-*.log.
"""
...
def _log_files(argv: list[str]) -> list[str]:
    ...
    return sorted(glob.glob(os.path.join(home, ".jcode", "logs", "mona-*.log")))
