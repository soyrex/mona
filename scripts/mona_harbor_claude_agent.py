...
DEFAULT_BINARY_PATH = "/tmp/mona-compat-dist/mona-linux-x86_64.bin"
DEFAULT_CLAUDE_AUTH_PATH = "~/.mona/auth.json"
...
def _load_anthropic_key() -> str | None:
    return _load_key_from_env_file(
        os.environ.get("MONA_HARBOR_ANTHROPIC_ENV", DEFAULT_ANTHROPIC_ENV_PATH),
        "ANTHROPIC_API_KEY",
        "ANTHROPIC_API_KEY",
        "MONA_HARBOR_ANTHROPIC_KEY",
    )


def _load_openrouter_key() -> str | None:
    # Priority: explicit env, then the jcode openrouter.env file (raw key per line).
    direct = os.environ.get("OPENROUTER_API_KEY") or os.environ.get("MONA_HARBOR_OPENROUTER_KEY")
    if direct and direct.strip():
        return direct.strip()
    path = Path(os.environ.get("MONA_HARBOR_OPENROUTER_ENV", DEFAULT_OPENROUTER_ENV_PATH)).expanduser()
    if path.exists() and path.is_file():
        for line in path.read_text().splitlines():
            line = line.strip()
            if not line or line.startswith("#"):
...
class JcodeClaudeHarborAgent(BaseAgent):
    ...
    OAuth (provider=claude via ~/.mona/auth.json) are also supported.
    """
    ...
    def __init__(self, logs_dir: Path, model_name: str | None = None, *args, **kwargs):
        ...
        if "/" in self._model_arg:
            ...
            self._provider_arg, self._mona_model = "anthropic-api", self._model_arg
        ...
        if self._provider_arg == "claude":
            self._claude_auth = _resolve_existing_file(
                env_name="MONA_HARBOR_CLAUDE_AUTH",
                default_path=DEFAULT_CLAUDE_AUTH_PATH,
            )

    @staticmethod
    def name() -> str:
        return "mona-harbor-claude"
    ...
    async def run(self, instruction: str, environment: BaseEnvironment, context: AgentContext) -> None:
        ...
        env = {
            ...
            "MONA_ANTHROPIC_REASONING_EFFORT": os.environ.get("MONA_ANTHROPIC_REASONING_EFFORT", "high"),
