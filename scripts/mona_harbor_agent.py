...
IN_CONTAINER_HOME = "/tmp/mona-home"
IN_CONTAINER_RUNTIME = "/tmp/mona-runtime"
IN_CONTAINER_INPUT = "/tmp/mona-input"
IN_CONTAINER_OUTPUT = "/tmp/mona-output"
...
DEFAULT_BINARY_PATH = "/tmp/mona-compat-dist/mona-linux-x86_64"
DEFAULT_OPENAI_AUTH_PATH = "~/.mona/openai-auth.json"
CA_BUNDLE_CANDIDATES = (
    os.environ.get("MONA_HARBOR_CA_BUNDLE"),
...
OPENSSL_RUNTIME_LIBS = tuple(
    ...
    for lib in (
        _resolve_optional_existing_file(
            candidates=(
                os.environ.get("MONA_HARBOR_LIBSSL"),
                *_sibling_runtime_lib_candidates(MONA_BINARY, "libssl"),
                "/usr/lib/libssl.so.3",
        ...
        _resolve_optional_existing_file(
            candidates=(
                os.environ.get("MONA_HARBOR_LIBCRYPTO"),
                *_sibling_runtime_lib_candidates(MONA_BINARY, "libcrypto"),
...
def _benchmark_instruction_preamble() -> str:
    # Keep Harbor runs aligned with normal TUI/mona-run prompting by default.
    ...
    if os.environ.get("MONA_HARBOR_LEGACY_PREAMBLE"):
        return LEGACY_BENCHMARK_INSTRUCTION_PREAMBLE
    return os.environ.get("MONA_HARBOR_EXTRA_PREAMBLE", "")


def _load_task_hint() -> str:
    if not os.environ.get("MONA_HARBOR_ENABLE_HINTS"):
        return ""
    task_name = os.environ.get("MONA_HARBOR_CURRENT_TASK", "").strip()
    hints_path = os.environ.get("MONA_HARBOR_TASK_HINTS_FILE", "").strip()
    extra = os.environ.get("MONA_HARBOR_EXTRA_PREAMBLE", "").strip()
    parts: list[str] = []
    if extra:
        parts.append(extra)
...
class JcodeHarborAgent(BaseAgent):
    def __init__(self, logs_dir: Path, model_name: str | None = None, *args, **kwargs):
        ...
        if "/" in self._model_arg:
            self._provider_arg, self._mona_model = self._model_arg.split("/", 1)
        else:
            self._provider_arg, self._mona_model = "openai", self._model_arg
    ...
    async def setup(self, environment: BaseEnvironment) -> None:
        ...
        await environment.upload_file(MONA_BINARY, IN_CONTAINER_BINARY)
        ...
        version_result = await environment.exec(
            ...
            env={
                ...
                "MONA_HOME": IN_CONTAINER_HOME,
                "MONA_RUNTIME_DIR": IN_CONTAINER_RUNTIME,
                "MONA_NO_TELEMETRY": "1",
    ...
    async def run(self, instruction: str, environment: BaseEnvironment, context: AgentContext) -> None:
        ...
        env = {
            ...
            "MONA_RUNTIME_DIR": IN_CONTAINER_RUNTIME,
            "MONA_NO_TELEMETRY": "1",
            "MONA_PROVIDER": self._provider_arg,
            "MONA_MODEL": self._mona_model,
            "MONA_OPENAI_REASONING_EFFORT": os.environ.get("MONA_OPENAI_REASONING_EFFORT", "high"),
            "MONA_OPENAI_SERVICE_TIER": os.environ.get("MONA_OPENAI_SERVICE_TIER", "priority"),
        ...
        result = await environment.exec(
            command=(
                ...
                '--provider "$MONA_PROVIDER" --model "$MONA_MODEL" '
        ...
        try:
            await environment.download_dir(IN_CONTAINER_OUTPUT, self.logs_dir / "mona-output")
        ...
        metadata: dict[str, Any] = {
            ...
            "model": self._mona_model,
            "mona_binary": str(MONA_BINARY),
        }

        output_dir = self.logs_dir / "mona-output"
