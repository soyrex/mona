...
use mona::tui::login_picker::{LoginPicker, LoginPickerItem, LoginPickerSummary};
...
impl TestEnv {
    fn new() -> Result<Self> {
        ...
        mona::env::set_var("HOME", temp.path());
        mona::env::set_var("XDG_CONFIG_HOME", temp.path().join("config"));
        mona::env::set_var("APPDATA", temp.path().join("AppData").join("Roaming"));
        mona::env::set_var("MONA_HOME", temp.path().join("mona-home"));
        mona::env::set_var("NO_PROXY", "127.0.0.1,localhost");
        mona::env::set_var("no_proxy", "127.0.0.1,localhost");
    ...
    fn configure_openai_compatible_runtime(
        ...
        mona::env::set_var("MONA_OPENROUTER_API_BASE", api_base);
        mona::env::set_var("MONA_OPENROUTER_API_KEY_NAME", "AUTH_FLOW_TEST_KEY");
        mona::env::set_var("MONA_OPENROUTER_ENV_FILE", "auth-flow-test.env");
        mona::env::set_var("MONA_OPENROUTER_CACHE_NAMESPACE", cache_namespace);
        mona::env::set_var("MONA_OPENROUTER_PROVIDER_FEATURES", "0");
        mona::env::set_var("MONA_OPENROUTER_MODEL_CATALOG", "1");
        if let Some(key) = key {
            mona::env::set_var("AUTH_FLOW_TEST_KEY", key);
        } else {
            mona::env::remove_var("AUTH_FLOW_TEST_KEY");
        }
        if allow_no_auth {
            mona::env::set_var("MONA_OPENROUTER_ALLOW_NO_AUTH", "1");
        } else {
            mona::env::remove_var("MONA_OPENROUTER_ALLOW_NO_AUTH");
        }
        AuthStatus::invalidate_cache();
    }
}
...
fn live_models_contract_supports_api_key_header_mode() -> Result<()> {
    ...
    mona::env::set_var("MONA_OPENROUTER_AUTH_HEADER", "api-key");
    mona::env::set_var("MONA_OPENROUTER_AUTH_HEADER_NAME", "x-api-key");
...
fn model_picker_cache_miss_schedules_single_background_refresh_and_updates_routes() -> Result<()> {
    ...
    mona::env::set_var("MONA_OPENROUTER_MODEL", "background-race-selected-model");
...
fn live_model_catalog_failure_keeps_static_and_selected_model_picker_fallbacks() -> Result<()> {
    ...
    mona::env::set_var("MONA_OPENROUTER_MODEL", "selected-fallback-model");
    mona::env::set_var(
        "MONA_OPENROUTER_STATIC_MODELS",
