...
async fn binary_version_command() -> Result<()> {
    ...
    assert!(
        stdout.contains("mona") || stdout.contains("20"),
...
async fn binary_integration_reload_handoff() -> Result<()> {
    ...
    let release_binary =
        mona::build::release_binary_path(std::path::Path::new(env!("CARGO_MANIFEST_DIR")));
    ...
    let test_result = async {
        ...
        while mona::server::reload_marker_active(Duration::from_secs(30)) {
...
async fn binary_integration_selfdev_reload_reconnects_quickly() -> Result<()> {
    ...
    let temp_root = tempfile::Builder::new()
        .prefix("mona-selfdev-reload-e2e-")
        .tempdir()?;
    let runtime_dir = temp_root.path().join("runtime");
    let home_dir = temp_root.path().join("home");
    let install_dir = temp_root.path().join("install");
    ...
    let _home_guard = EnvVarGuard::set("MONA_HOME", &home_dir);
    let _runtime_guard = EnvVarGuard::set("MONA_RUNTIME_DIR", &runtime_dir);
    let _install_guard = EnvVarGuard::set("MONA_INSTALL_DIR", &install_dir);
...
async fn binary_integration_selfdev_client_reload_resumes_session() -> Result<()> {
    ...
    let temp_root = tempfile::Builder::new()
        .prefix("mona-selfdev-client-reload-e2e-")
        .tempdir()?;
    let runtime_dir = temp_root.path().join("runtime");
    let home_dir = temp_root.path().join("home");
    let install_dir = temp_root.path().join("install");
    ...
    let socket_path = runtime_dir.join("jcode.sock");
    let debug_socket_path = runtime_dir.join("mona-debug.sock");
    let starter_binary = temp_root.path().join("mona-selfdev-client-starter");
...
async fn binary_integration_selfdev_full_reload_resumes_session_quickly() -> Result<()> {
    ...
    let temp_root = tempfile::Builder::new()
        .prefix("mona-selfdev-full-reload-e2e-")
        .tempdir()?;
    let runtime_dir = temp_root.path().join("runtime");
    let home_dir = temp_root.path().join("home");
    let install_dir = temp_root.path().join("install");
    ...
    let socket_path = runtime_dir.join("jcode.sock");
    let debug_socket_path = runtime_dir.join("mona-debug.sock");
    let starter_binary = temp_root.path().join("mona-selfdev-full-reload-starter");
    std::fs::copy(env!("CARGO_BIN_EXE_jcode"), &starter_binary)?;
    let starter_mtime = std::fs::metadata(&release_binary)?
        .modified()?
        .checked_sub(Duration::from_secs(60))
