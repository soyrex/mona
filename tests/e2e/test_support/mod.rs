...
pub(crate) use mona::agent::Agent;
pub(crate) use mona::message::{ContentBlock, Message, Role, StreamEvent, ToolDefinition};
pub(crate) use mona::protocol::{Request, ServerEvent};
pub(crate) use mona::provider::{EventStream, Provider};
pub(crate) use mona::server;
pub(crate) use mona::session::{Session, StoredCompactionState};
pub(crate) use mona::tool::Registry;
...
static MONA_HOME_LOCK: std::sync::OnceLock<Mutex<()>> = std::sync::OnceLock::new();
...
fn lock_mona_home() -> std::sync::MutexGuard<'static, ()> {
    let mutex = MONA_HOME_LOCK.get_or_init(|| Mutex::new(()));
...
impl TestEnvGuard {
    pub(crate) fn new() -> Result<Self> {
        let lock = lock_mona_home();
        let temp_home = tempfile::Builder::new()
            .prefix("mona-e2e-home-")
            .tempdir()?;
        let prev_home = std::env::var_os("MONA_HOME");
        let prev_runtime_dir = std::env::var_os("MONA_RUNTIME_DIR");
        let prev_test_session = std::env::var_os("MONA_TEST_SESSION");
        let prev_debug_control = std::env::var_os("MONA_DEBUG_CONTROL");
        ...
        let prev_openrouter_cache_namespace = std::env::var_os("MONA_OPENROUTER_CACHE_NAMESPACE");
        let runtime_dir = temp_home.path().join("runtime");
        std::fs::create_dir_all(&runtime_dir)?;

        mona::env::set_var("MONA_HOME", temp_home.path());
        mona::env::set_var("MONA_RUNTIME_DIR", &runtime_dir);
        mona::env::set_var("MONA_TEST_SESSION", "1");
        mona::env::set_var("MONA_DEBUG_CONTROL", "1");
        mona::env::remove_var("MONA_RUNTIME_PROVIDER");
        mona::env::remove_var("MONA_ACTIVE_PROVIDER");
        mona::env::remove_var("MONA_OPENROUTER_CACHE_NAMESPACE");
        ...
        mona::env::set_var("MONA_MEMORY_ENABLED", "0");
        mona::env::set_var("MONA_MEMORY_SIDECAR_ENABLED", "0");
...
impl Drop for TestEnvGuard {
    fn drop(&mut self) {
        if let Some(prev_home) = &self.prev_home {
            mona::env::set_var("MONA_HOME", prev_home);
        } else {
            mona::env::remove_var("MONA_HOME");
        }

        if let Some(prev_runtime_dir) = &self.prev_runtime_dir {
            mona::env::set_var("MONA_RUNTIME_DIR", prev_runtime_dir);
        } else {
            mona::env::remove_var("MONA_RUNTIME_DIR");
        }

        if let Some(prev_test_session) = &self.prev_test_session {
            mona::env::set_var("MONA_TEST_SESSION", prev_test_session);
        } else {
            mona::env::remove_var("MONA_TEST_SESSION");
        }

        if let Some(prev_debug_control) = &self.prev_debug_control {
            mona::env::set_var("MONA_DEBUG_CONTROL", prev_debug_control);
        } else {
            mona::env::remove_var("MONA_DEBUG_CONTROL");
        }

        if let Some(prev_runtime_provider) = &self.prev_runtime_provider {
            mona::env::set_var("MONA_RUNTIME_PROVIDER", prev_runtime_provider);
        } else {
            mona::env::remove_var("MONA_RUNTIME_PROVIDER");
        }

        if let Some(prev_active_provider) = &self.prev_active_provider {
            mona::env::set_var("MONA_ACTIVE_PROVIDER", prev_active_provider);
        } else {
            mona::env::remove_var("MONA_ACTIVE_PROVIDER");
        }
...
impl Drop for EnvVarGuard {
    fn drop(&mut self) {
        if let Some(prev) = &self.prev {
            mona::env::set_var(self.name, prev);
        } else {
            mona::env::remove_var(self.name);
        }
    }
}
...
fn pair_test_device(token: &str) -> Result<()> {
    let mut registry = mona::gateway::DeviceRegistry::load();
    ...
    registry.devices.push(mona::gateway::PairedDevice {
...
pub(crate) async fn run_unix_transport_scenario() -> Result<TransportScenarioResult> {
    ...
    let provider: Arc<dyn mona::provider::Provider> = Arc::new(provider);
    let server_instance =
    ...
    let result = async {
        ...
        if !saw_message_done {
            ...
            if !response_persisted {
                ...
                let logs = std::env::var_os("MONA_HOME")
...
pub(crate) async fn run_websocket_transport_scenario() -> Result<TransportScenarioResult> {
    ...
    let server_instance =
        server::Server::new_with_paths(provider, socket_path.clone(), debug_socket_path.clone())
            .with_gateway_config(mona::gateway::GatewayConfig {
                port: gateway_port,
...
pub(crate) async fn wait_for_selfdev_reload_cycle(
    ...
    while Instant::now() < deadline {
        let marker_active = mona::server::reload_marker_active(Duration::from_secs(30));
