...
async fn test_resume_restores_model_and_tool_history() -> Result<()> {
    ...
    session.add_message(
        mona::message::Role::User,
        vec![mona::message::ContentBlock::Text {
            text: "Run a tool".to_string(),
            cache_control: None,
        }],
    );
    session.add_message(
        mona::message::Role::Assistant,
        vec![
            mona::message::ContentBlock::Text {
                text: "Running...".to_string(),
                cache_control: None,
            },
            mona::message::ContentBlock::ToolUse {
                id: "tool-1".to_string(),
                name: "bash".to_string(),
    ...
    session.add_message(
        mona::message::Role::User,
        vec![mona::message::ContentBlock::ToolResult {
...
async fn test_resume_session_with_local_history_uses_metadata_only_history() -> Result<()> {
    ...
    let provider_dyn: Arc<dyn mona::provider::Provider> = provider.clone();
...
/// The agent should identify as "mona" or just a generic "coding assistant powered by Claude"
#[tokio::test]
