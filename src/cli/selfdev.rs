...
pub use mona_selfdev_types::CLIENT_SELFDEV_ENV;
pub use mona_selfdev_types::client_selfdev_requested;

const MONA_REPO_URL: &str = "https://github.com/1jehuang/jcode.git";

fn selfdev_clone_dir() -> Result<PathBuf> {
    Ok(crate::storage::mona_dir()?.join("source").join("mona"))
}
...
pub async fn run_self_dev(should_build: bool, resume_session: Option<String>) -> Result<()> {
    ...
    crate::env::set_var("MONA_REPO_DIR", &repo_dir);
