...
fn set_socket_test_env(socket_path: &Path, runtime_dir: &Path) -> EnvVarGuard {
    let guard = EnvVarGuard::capture(&["MONA_SOCKET", "MONA_RUNTIME_DIR"]);
...
impl TestEnvGuard {
    fn new() -> anyhow::Result<Self> {
        ...
        let temp_home = tempfile::Builder::new()
            .prefix("mona-selfdev-test-home-")
            .tempdir()?;
        let env = EnvVarGuard::capture(&["MONA_HOME", "MONA_TEST_SESSION"]);

        crate::env::set_var("MONA_HOME", temp_home.path());
        crate::env::set_var("MONA_TEST_SESSION", "1");
...
fn isolated_launcher_env() -> (
    ...
    let lock = lock_env();
    let temp = tempfile::tempdir().expect("tempdir");
    let env = EnvVarGuard::capture(&["MONA_INSTALL_DIR", "MONA_HOME", "HOME", "USERPROFILE"]);
    crate::env::set_var("HOME", temp.path());
    crate::env::set_var("USERPROFILE", temp.path());
    crate::env::remove_var("MONA_INSTALL_DIR");
...
#[test]
fn test_launcher_dir_uses_trimmed_install_dir_before_mona_home() {
...
fn default_launcher_dir(home: &Path) -> PathBuf {
    if cfg!(windows) {
        home.join("AppData").join("Local").join("mona").join("bin")
    } else {
...
fn test_selfdev_build_command_prefers_repo_wrapper_when_present() {
    ...
    assert!(!command.contains("mona-desktop"));
    assert!(build.display.contains("-p jcode --bin jcode"));
    assert!(!build.display.contains("mona-desktop"));
}
