...
async fn audit_openrouter_context_windows(
    ...
    let provider = match mona_provider_openrouter_runtime::OpenRouterProvider::new() {
        ...
        match step.name.as_str() {
            ...
            "tool_smoke" => {
                ...
                if !tool_smoke_skipped {
                    capabilities.push("real_mona_tool_smoke");
                }
                ...
                expected.push(crate::live_tests::checkpoints::REAL_MONA_TOOL_SMOKE);
                let stage = auth_test_step_stage(
                    crate::live_tests::checkpoints::REAL_MONA_TOOL_SMOKE,
                    step,
                )
...
fn auth_test_coverage_provider_identity(report: &AuthTestProviderReport) -> (String, String) {
    if report.provider == "openai-compatible"
        && let Ok(profile_name) = std::env::var("MONA_NAMED_PROVIDER_PROFILE")
    {
...
fn auth_test_tool_derived_stage(
    ...
    auth_test_step_stage(checkpoint, step).with_evidence(
        "derived_from",
        serde_json::json!(crate::live_tests::checkpoints::REAL_MONA_TOOL_SMOKE),
    )
}
