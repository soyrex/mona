...
mod tests {
    ...
    #[test]
    fn mona_desktop_uses_kitty_graphics_protocol() {
        assert!(is_kitty_terminal_name("mona-desktop"));
        assert!(is_kitty_terminal_name("Jcode-Desktop"));
        assert!(!is_kitty_terminal_name("mona"));
        assert!(!is_kitty_terminal_name("xterm-256color"));
    }

    #[test]
    fn mona_desktop_environment_uses_kitty_graphics() {
        // Isolate environment-dependent detection from the parallel test suite.
        const CHILD: &str = "MONA_TEST_DESKTOP_IMAGE_CHILD";
        ...
        let output = std::process::Command::new(std::env::current_exe().unwrap())
            .args([
                "--exact",
                "display::tests::mona_desktop_environment_uses_kitty_graphics",
                "--nocapture",
            ])
            .env(CHILD, "1")
            .env("TERM", "xterm-256color")
            .env("TERM_PROGRAM", "mona-desktop")
