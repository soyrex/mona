...
use mona_message_types::{ContentBlock, Message, Role, ToolCall};
...
/// `mona-session-types` so the foundation/import layer can match on it without
/// depending on any `mona-tui-*` crate. The session-picker UI re-exports it.
...
pub struct EnvSnapshot {
    ...
    pub mona_git_hash: Option<String>,
    pub mona_git_dirty: Option<bool>,
    pub os: String,
    pub arch: String,
    pub pid: u32,
    pub is_selfdev: bool,
...
pub struct SessionSearchReport {
    ...
    pub scanned_mona_sessions: usize,
    pub candidate_mona_sessions: usize,
