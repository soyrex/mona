...
//!
//! ```sh
//! cargo run -p mona-build-support --example write_dev_sidecar
//! ```
fn main() -> anyhow::Result<()> {
    let repo = std::env::current_dir()?;
    let state = mona_build_support::current_source_state(&repo)?;
    let path = mona_build_support::write_current_dev_binary_source_metadata(&repo, &state)?;
