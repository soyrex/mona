...
pub fn builds_dir() -> Result<PathBuf> {
    let dir = resolve_builds_dir(
        std::env::var_os("MONA_HOME").map(PathBuf::from),
...
fn resolve_builds_dir(
    mona_home: Option<PathBuf>,
    local_app_data: Option<PathBuf>,
    default_mona_dir: PathBuf,
    is_windows: bool,
) -> PathBuf {
    if let Some(mona_home) = mona_home {
        return mona_home.join("builds");
    }
    ...
    if is_windows && let Some(local_app_data) = local_app_data {
        ...
        return local_app_data.join("mona").join("builds");
    }

    default_mona_dir.join("builds")
}
...
mod tests {
    ...
    #[test]
    fn mona_home_override_wins_on_windows() {
        let resolved = resolve_builds_dir(
    ...
    #[test]
    fn non_windows_builds_stay_under_mona_home() {
        let resolved = resolve_builds_dir(
            None,
        ...
        assert_eq!(resolved, PathBuf::from("/home/test/.mona/builds"));
    }
}
