...
fn with_temp_mona_home<T>(f: impl FnOnce() -> T) -> T {
    ...
    mona_core::env::set_var("MONA_HOME", temp_home.path());
    let result = f();
...
fn test_binary_version_hash_mismatch_rejects_publish_candidate() {
    ...
    let error = validate_binary_version_matches_source_report(&report, Path::new("mona"), &source)
...
fn test_find_repo_in_ancestors_walks_upward() {
    let temp = tempfile::tempdir().expect("tempdir");
    let repo = temp.path().join("mona-repo");
...
#[test]
fn launcher_dir_uses_sandbox_bin_when_mona_home_is_set() {
    with_temp_mona_home(|| {
        let launcher_dir = launcher_dir().expect("launcher dir");
        let expected = storage::mona_dir().expect("jcode dir").join("bin");
...
fn update_launcher_symlink_stays_inside_sandbox_home() {
    with_temp_mona_home(|| {
        ...
        let expected_launcher = storage::mona_dir()
            .expect("jcode dir")
            .join("bin")
            .join(binary_name());
