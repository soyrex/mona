...
pub fn get_repo_dir() -> Option<PathBuf> {
    if let Ok(path) = std::env::var("MONA_REPO_DIR") {
        let path = PathBuf::from(path);
        if is_mona_repo(&path) {
            return Some(path);
        }
    }
...
/// `mona-<platform>.bin` payload (the wrapper sets `LD_LIBRARY_PATH` and execs
...
fn selfdev_build_command_for_target_on_platform(
    ...
    let specs = match target {
        SelfDevBuildTarget::Tui => vec![("mona", "mona")],
        SelfDevBuildTarget::All | SelfDevBuildTarget::Auto => vec![("mona", "mona")],
    };
...
/// Overridable with `MONA_INSTALL_DIR`.
pub fn launcher_dir() -> Result<PathBuf> {
    if let Some(custom) = non_empty_env_path("MONA_INSTALL_DIR") {
        return Ok(custom);
    }

    if let Some(sandbox_home) = non_empty_env_path("MONA_HOME") {
    ...
    #[cfg(windows)]
    {
        if let Ok(local) = std::env::var("LOCALAPPDATA") {
            return Ok(PathBuf::from(local).join("mona").join("bin"));
        }
        Ok(home_dir()?
            .join("AppData")
            .join("Local")
...
pub fn is_mona_repo(dir: &Path) -> bool {
    // Check for Cargo.toml with name = "mona"
...
mod tests {
    ...
    fn repo_fixture(git_file: bool) -> tempfile::TempDir {
        ...
        if git_file {
            std::fs::write(temp.path().join(".git"), "gitdir: /tmp/mona-test-git\n")
                .expect("git file");
        } else {
    ...
    fn find_repo_in_ancestors_finds_workspace_from_crate_dir() {
        ...
        let crate_dir = repo.path().join("crates").join("mona-build-support");
    ...
    fn auto_routes_changed_paths_to_the_right_binary() {
        let cases: Vec<(Vec<&str>, SelfDevBuildTarget)> = vec![
            ...
            (vec!["crates/mona-tui/src/lib.rs"], SelfDevBuildTarget::Tui),
    ...
    fn porcelain_lines_are_parsed_including_renames() {
        ...
        assert_eq!(
            porcelain_path("?? crates/mona-tui/src/new.rs"),
            "crates/mona-tui/src/new.rs"
        );
        assert_eq!(
            porcelain_path("R  old/path.rs -> crates/mona-tui/src/moved.rs"),
            "crates/mona-tui/src/moved.rs"
        );
    }
    ...
    fn build_targets_parse_from_their_names() {
        ...
        for removed in ["desktop", "desktop2", "mona-desktop2"] {
            assert!(
    ...
    #[test]
    fn is_mona_repo_accepts_git_file_for_worktree() {
        let repo = repo_fixture(true);
        assert!(is_mona_repo(repo.path()));
    }
    ...
    fn release_install_fixture() -> (tempfile::TempDir, PathBuf, PathBuf) {
        ...
        let wrapper = temp.path().join("mona");
        let payload = temp.path().join("mona-linux-x86_64.bin");
        std::fs::write(
            &wrapper,
            "#!/usr/bin/env sh\nexec ./mona-linux-x86_64.bin \"$@\"\n",
        )
        .expect("wrapper");
    ...
    fn resolve_binary_payload_follows_channel_symlink_to_wrapper() {
        ...
        let link = channel_dir.join("mona");
        #[cfg(unix)]
    ...
    fn resolve_binary_payload_keeps_real_binary() {
        ...
        let temp = tempfile::TempDir::new().expect("temp install");
        let binary = temp.path().join("mona");
        std::fs::write(&binary, vec![0x7fu8; 8192]).expect("binary");
        std::fs::write(temp.path().join("mona-linux-x86_64.bin"), [0u8; 8]).expect("bin");
        assert_eq!(
    ...
    fn resolve_binary_payload_keeps_small_script_without_payload() {
        let temp = tempfile::TempDir::new().expect("temp install");
        let script = temp.path().join("mona");
    ...
    fn resolve_binary_payload_refuses_ambiguous_payloads() {
        ...
        std::fs::write(temp.path().join("mona-macos-aarch64.bin"), [0u8; 8]).expect("second bin");
        assert_eq!(
            resolve_binary_payload(&wrapper),
