use anyhow::Result;
use mona_message_types::{ContentBlock, Message, Role, ToolCall, ToolDefinition};
...
/// `mona-schema-dialect` so that every provider shares one implementation and
...
pub fn gemini_compatible_schema(schema: &Value) -> Value {
    mona_schema_dialect::normalize(schema, &mona_schema_dialect::registry::GEMINI)
}
