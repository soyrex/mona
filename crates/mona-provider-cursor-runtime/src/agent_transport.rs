...
use mona_message_types::StreamEvent;
...
fn agent_host_from_cursor_cli_config() -> Option<String> {
    let path = match mona_base::storage::user_home_path(".cursor/cli-config.json") {
        Ok(path) => path,
        Err(err) => {
...
/// 1. `MONA_CURSOR_AGENT_HOST` / `CURSOR_AGENT_HOST` (explicit override)
...
fn agent_host() -> String {
    for var in ["MONA_CURSOR_AGENT_HOST", "CURSOR_AGENT_HOST"] {
...
mod tests {
    ...
    fn agent_host_normalizes_env_overrides() {
        ...
        // shared lock in mona-base.
        let _guard = mona_base::storage::lock_test_env();
        let prev = std::env::var_os("MONA_CURSOR_AGENT_HOST");

        mona_base::env::set_var(
            "MONA_CURSOR_AGENT_HOST",
            "https://agentn.us.api5.cursor.sh/agent.v1.AgentService/Run",
        );
        assert_eq!(agent_host(), "agentn.us.api5.cursor.sh");

        mona_base::env::set_var("MONA_CURSOR_AGENT_HOST", "agentn.eu.api5.cursor.sh:443");
        assert_eq!(agent_host(), "agentn.eu.api5.cursor.sh");

        // A blank override must not win; it falls through to the next source.
        mona_base::env::set_var("MONA_CURSOR_AGENT_HOST", "   ");
        assert_ne!(agent_host(), "   ");

        match prev {
            Some(value) => mona_base::env::set_var("MONA_CURSOR_AGENT_HOST", value),
            None => mona_base::env::remove_var("MONA_CURSOR_AGENT_HOST"),
        }
    }
