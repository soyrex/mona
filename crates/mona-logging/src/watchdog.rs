...
///
/// Disable entirely with `MONA_WATCHDOG=0`. Tune with
/// `MONA_WATCHDOG_STALL_SECS` and `MONA_WATCHDOG_HEARTBEAT_SECS`.
pub fn start() {
    if std::env::var("MONA_WATCHDOG").is_ok_and(|value| value == "0" || value == "off") {
        return;
    }
    ...
    let stall = Duration::from_secs(env_secs("MONA_WATCHDOG_STALL_SECS", DEFAULT_STALL_SECS));
    let heartbeat = Duration::from_secs(env_secs(
        "MONA_WATCHDOG_HEARTBEAT_SECS",
        DEFAULT_HEARTBEAT_SECS,
    ));

    std::thread::Builder::new()
        .name("mona-watchdog".to_string())
