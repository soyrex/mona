...
impl LogLevel {
    ...
    fn is_enabled(self) -> bool {
        !matches!(self, Self::Debug) || std::env::var("MONA_TRACE").is_ok()
    }
}
...
fn log_dir() -> Option<PathBuf> {
    mona_storage::logs_dir().ok()
}

impl Logger {
    fn new() -> Option<Self> {
        let log_dir = log_dir()?;
        mona_storage::ensure_dir(&log_dir).ok()?;

        // Use date-based log file
        let date = Local::now().format("%Y-%m-%d");
        let path = log_dir.join(format!("mona-{}.log", date));
...
pub fn init() {
    ...
    // `mona-*.log` files, so it is safe to run unconditionally.
    ...
    if !CLEANUP_STARTED.swap(true, Ordering::SeqCst) {
        std::thread::Builder::new()
            .name("mona-log-cleanup".to_string())
...
/// Log a debug message (only if MONA_TRACE is set)
#[expect(
    clippy::collapsible_if,
    reason = "Debug logging keeps env gating and logger access explicit"
)]
pub fn debug(message: &str) {
    if std::env::var("MONA_TRACE").is_ok() {
        if let Ok(mut guard) = LOGGER.lock() {
            if let Some(logger) = guard.as_mut() {
...
fn structured_json_enabled() -> bool {
    matches!(
        std::env::var("MONA_LOG_JSON").as_deref(),
...
pub fn log_path() -> Option<PathBuf> {
    let log_dir = log_dir()?;
    let date = Local::now().format("%Y-%m-%d");
    Some(log_dir.join(format!("mona-{}.log", date)))
}

/// Remove daily `mona-*.log` / `mona-desktop-*.log` files older than 7 days.
///
...
fn cleanup_old_logs_in(log_dir: &std::path::Path, now: chrono::DateTime<Local>) {
    ...
    for entry in entries.flatten() {
        ...
        let is_mona_log = (name.starts_with("mona-") || name.starts_with("mona-desktop-"))
            && name.ends_with(".log");
        if !is_mona_log {
            continue;
        }
...
fn truncate(s: &str, max_len: usize) -> String {
    if s.len() > max_len {
        format!("{}...", mona_core::util::truncate_str(s, max_len))
    } else {
        s.to_string()
    }
}
...
mod tests {
    ...
    #[test]
    fn cleanup_removes_only_old_mona_logs() {
        use std::time::{Duration, SystemTime};

        let dir = std::env::temp_dir().join(format!(
            "mona-log-cleanup-test-{}-{}",
            std::process::id(),
            SystemTime::now()
        ...
        let old_log = write("mona-2000-01-01.log", true);
        let old_desktop = write("mona-desktop-2000-01-01.log", true);
        // Recent log file that SHOULD survive.
        let new_log = write("mona-2099-01-01.log", false);
