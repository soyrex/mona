...
//! aliases when `mona-desktop` is focused. This runtime route uses `niri`, `jq`,
...
fn launch_shell_command(
    ...
    let desktop_route = desktop_keys
        .map(|keys| {
            format!(
                "if [ \"$(niri msg -j focused-window 2>/dev/null | jq -r '.app_id // empty' 2>/dev/null)\" = 'mona-desktop' ]; then exec wtype {keys}; fi; "
            )
        })
        .unwrap_or_default();
...
mod tests {
    ...
    fn renders_bind_line_with_cd_and_terminal() {
        let line = render_niri_bind_line(
            &hk("alt+;", "/home/jeremy/jcode", "mona", true),
    ...
    fn spliced_config_is_accepted_by_the_real_niri_parser() {
        ...
        let dir = std::env::temp_dir().join(format!("mona-niri-719-{}", std::process::id()));
    ...
    fn fuzz_corpus_stays_valid_after_splicing() {
        ...
        let dir = std::env::temp_dir().join(format!("mona-niri-fuzz-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
