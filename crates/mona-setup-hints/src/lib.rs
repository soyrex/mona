...
//! State is persisted in `~/.mona/setup_hints.json`.
...
use macos_terminal::{
    ...
    launch_command_for_macos_terminal, paused_mona_shell_command, save_preferred_macos_terminal,
};
#[cfg(windows)]
use windows_setup::{
...
///   `mona-terminal-launch`, avoiding the generated shell-script hop on hotkey
...
impl SetupHintsState {
    fn path() -> Result<PathBuf> {
        Ok(storage::mona_dir()?.join("setup_hints.json"))
    }
...
fn mac_hotkey_support_dir() -> Result<PathBuf> {
    Ok(storage::mona_dir()?.join("hotkey"))
}
...
/// Load the `[launch_hotkeys]` table from `~/.mona/config.toml`.
///
...
fn load_launch_hotkeys_config() -> mona_config_types::LaunchHotkeysConfig {
    #[derive(serde::Deserialize, Default)]
    struct Wrapper {
        #[serde(default)]
        launch_hotkeys: mona_config_types::LaunchHotkeysConfig,
    }
    let Ok(dir) = storage::mona_dir() else {
...
pub fn record_launch_dirs(dir: &std::path::Path, repo_dir: Option<&std::path::Path>) {
    #[cfg(any(target_os = "macos", target_os = "linux", windows))]
    {
        if let Err(err) = record_launch_dirs_inner(dir, repo_dir) {
            mona_logging::warn(&format!("failed to record launch dirs for hotkeys: {err}"));
        }
    }
...
<dict>
    ...
    <dict>
        <key>MONA_PREFERRED_TERMINAL</key>
...
#[cfg(target_os = "macos")]
pub fn launch_mona_in_macos_terminal(extra_args: &[String]) -> Result<()> {
    let terminal = effective_macos_terminal();
    let exe = std::env::current_exe()?;
    let exe_path = exe.to_string_lossy().into_owned();
    let shell_command = macos_terminal::paused_mona_shell_command_with_args(&exe_path, extra_args);
    ...
    let command = match macos_terminal::no_automation_launch(terminal, &shell_command) {
        ...
        macos_terminal::NoAutomationLaunch::CommandFile { app } => {
            let dir = storage::mona_dir()?.join("launcher");
...
pub(crate) fn install_cli_launch_hints_notice() {
    match cli_launch_hints::install_available() {
        ...
        Ok(_) => {}
        Err(err) => mona_logging::warn(&format!(
...
pub(crate) fn active_primary_launch_hotkey() -> Option<(String, String)> {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let exe_path = std::env::current_exe()
            ...
            .unwrap_or_else(|_| "mona".to_string());
...
fn run_macos_hotkey_listener() -> Result<()> {
    ...
    use mona_terminal_launch::{TerminalCommand, spawn_command_in_new_terminal_with};
    ...
    GlobalHotKeyEvent::set_event_handler(Some(move |event: GlobalHotKeyEvent| {
        ...
        if let Some(launch) = launch_for_id.get(&event.id) {
            ...
            let command = TerminalCommand::new(&exe_path, args)
                .fresh_spawn()
                .kind("hotkey")
                .spawn_env("MONA_SPAWN_LABEL", launch.label.clone());
            ...
            && state.launch_hotkey_tracking_version < LAUNCH_HOTKEY_TRACKING_VERSION
        {
            match windows_setup::refresh_windows_launch_hotkeys() {
                Ok(()) => {
                    state.launch_hotkey_tracking_version = LAUNCH_HOTKEY_TRACKING_VERSION;
                    let _ = state.save();
                    mona_logging::info("Migrated Windows launch hotkeys to usage tracking");
                }
                Err(err) => mona_logging::warn(&format!(
...
fn linux_hotkeys_installed(comp: linux_env::LinuxCompositor) -> bool {
    use linux_env::LinuxCompositor;
    match comp {
        LinuxCompositor::Gnome => gnome_keybinding_list().contains("/mona-launch-"),
        LinuxCompositor::Cinnamon => {
            dconf_read("/org/cinnamon/desktop/keybindings/custom-list").contains("mona-launch-")
        }
        LinuxCompositor::Mate => {
            dconf_list("/org/mate/desktop/keybindings/").contains("mona-launch-")
        }
        LinuxCompositor::Xfce => xfce_shortcut_commands_text().contains("/launch_mona_"),
        LinuxCompositor::Kde => kde_globalshortcutsrc_path()
            .and_then(|p| std::fs::read_to_string(p).ok())
            .map(|text| text.contains("[services][mona-launch-"))
            .unwrap_or(false),
...
/// scripts written under `~/.mona/hotkey/`, so the config never embeds shell
...
fn install_xfce_launch_hotkeys() -> Result<bool> {
    ...
    for line in existing.lines() {
        ...
        if value.trim().contains("/launch_mona_") && !wanted.iter().any(|(p, _)| p == prop) {
...
/// `~/.mona/hotkey/` and return the chord -> script binds. The scripts `cd`
...
/// Timestamped backup matching the `.bak-mona-*` convention, taken before
...
fn backup_compositor_config(config_path: &std::path::Path) {
    ...
    let backup = config_path.with_file_name(format!("{file_name}.bak-mona-hotkeys-{ts}"));
...
fn reload_compositor_config(comp: linux_env::LinuxCompositor) {
        ...
        .status()
    {
        ...
        Ok(status) => mona_logging::warn(&format!(
            "{} exited with {status} while reloading hotkey binds",
            cmd[0]
        )),
        Err(err) => mona_logging::warn(&format!("failed to run {} reload: {err}", cmd[0])),
    }
}
...
pub fn maybe_show_keymap_conflict_hint(
    keybindings: &mona_config_types::KeybindingsConfig,
) -> Option<StartupHints> {
    if !io::stdin().is_terminal() || !io::stderr().is_terminal() {
        return None;
    }
...
/// in `mona-tui-style`'s color module and `mona-app-core::perf` so the
/// disclosure fires exactly when the behavior is active. Overridable with
/// `MONA_GLYPH_SAFE_MODE=on|off`.
fn glyph_safe_mode_active() -> bool {
    if let Ok(raw) = std::env::var("MONA_GLYPH_SAFE_MODE") {
...
For full color, use Ghostty, iTerm2, kitty, or WezTerm, or set MONA_GLYPH_SAFE_MODE=off."
        .to_string();
    (
        Some(StartupHints::with_status_and_display(
...
fn create_desktop_shortcut(state: &mut SetupHintsState) -> Result<()> {
    #[cfg(any(test, target_os = "macos"))]
    {
        ...
        mona_logging::info(&format!("Created macOS app bundle: {}", app_dir.display()));
    }
...
fn uninstall_macos_hotkey_listener() -> Result<()> {
    ...
    mona_logging::info("Removed macOS launch-hotkey LaunchAgent (launch_hotkeys.enabled = false)");
    Ok(())
}
...
pub fn reinstall_launch_hotkeys_after_config_change() {
    #[cfg(target_os = "macos")]
    {
        ...
        match install_macos_hotkey_listener(preferred) {
            Ok(terminal) => mona_logging::info(&format!(
                "Reinstalled launch hotkeys after config change for {}",
                terminal.label()
            )),
            Err(err) => mona_logging::warn(&format!("failed to reinstall launch hotkeys: {err}")),
        }
    }
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        match install_linux_launch_hotkeys(comp) {
            Ok(true) => mona_logging::info(&format!(
