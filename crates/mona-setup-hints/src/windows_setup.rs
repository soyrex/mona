...
use mona_config_types::{LaunchHotkeyEntry, LaunchHotkeysConfig};
use mona_core::{terminal_eprint as eprint, terminal_eprintln as eprintln};
...
fn startup_shortcut_path() -> PathBuf {
    startup_dir().join("mona-hotkey.lnk")
}

fn hotkey_vbs_path() -> Result<PathBuf> {
    Ok(storage::mona_dir()?
        .join("hotkey")
        .join("mona-hotkey-launcher.vbs"))
}

fn legacy_hotkey_ps1_path() -> Result<PathBuf> {
    Ok(storage::mona_dir()?
        .join("hotkey")
        .join("mona-hotkey.ps1"))
}
...
Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
  Where-Object {{
    ($_.ProcessId -ne $current) -and ($_.ProcessId -ne $PID) -and (
      (($_.Name -eq 'powershell.exe' -or $_.Name -eq 'pwsh.exe') -and $_.CommandLine -like '*mona-hotkey*') -or
...
fn create_hotkey_shortcut(_use_alacritty: bool) -> Result<()> {
    ...
    let hotkey_dir = storage::mona_dir()?.join("hotkey");
...
fn launch_windows_hotkey(entry: &WindowsHotkey) -> Result<()> {
    ...
    let command = mona_terminal_launch::TerminalCommand::new(exe, args)
        .title("mona")
        .fresh_spawn()
        .kind("launch-hotkey");

    let launched =
        mona_terminal_launch::spawn_command_in_new_terminal_with(&command, &cwd, |cmd| {
            cmd.spawn().map(|_| ())
        })?;
    if !launched {
...
fn windows_native_hotkey_loop(entries: Vec<WindowsHotkey>) -> Result<()> {
    ...
    let mutex = loop {
        ...
        if retry_count >= 40 {
            mona_logging::warn("previous Windows launch-hotkey listener is still exiting");
            return Ok(());
        }
        retry_count += 1;
...
pub(super) fn reinstall_windows_launch_hotkeys() {
    ...
    match refresh_windows_launch_hotkeys() {
        Ok(()) => mona_logging::info("Reinstalled Windows launch hotkeys after config change"),
...
mod tests {
    ...
    #[test]
    fn listener_stop_sweep_excludes_mona_and_powershell_processes_running_it() {
    ...
    fn startup_shortcut_uses_native_listener_without_vbscript_or_bypass() {
        let script = render_startup_shortcut_script(
            Path::new(r"C:\Users\O'Hara\Startup\mona-hotkey.lnk"),
...
"#,
    ...
    if output.status.success() {
        ...
        if stdout.contains("OK") {
            ...
            let _ = state.save();
            mona_logging::info(&format!("Created desktop shortcut: {}", shortcut_path));
        }
    }
