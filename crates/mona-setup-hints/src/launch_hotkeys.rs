...
#[cfg(any(test, target_os = "macos"))]
fn paused_mona_shell_command_with_args(exe_path: &str, args: &[String]) -> String {
...
pub(crate) struct ResolvedLaunchHotkey {
    /// mona-style chord string, e.g. `cmd+;`.
    pub chord: String,
...
/// `launch_mona_cmd_bracketleft.sh`. Collisions are avoided by appending the
...
fn script_name_for(chord: &str, index: usize) -> String {
    ...
    format!("launch_mona_{index}_{slug}.sh")
}
...
pub(crate) fn shell_command_for(entry: &ResolvedLaunchHotkey, exe_path: &str) -> String {
    format!(
        "{}{}",
        entry.cd_prefix,
        paused_mona_shell_command_with_args(exe_path, &entry.args)
    )
}
