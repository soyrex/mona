...
//! bind = SUPER, semicolon, exec, '/home/u/.mona/hotkey/launch_mona_0_cmd_semicolon.sh'
//! # <<< jcode launch hotkeys (managed) <<<
//! ```
...
pub(crate) fn gnome_keybinding_path(index: usize) -> String {
    format!(
        "/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/mona-launch-{index}/"
    )
}
...
/// replacing any previous jcode slots (matched on the `mona-launch-` marker).
...
pub(crate) fn kde_desktop_file_name(index: usize) -> String {
    format!("mona-launch-{index}.desktop")
}
...
/// INI text: existing `[services][mona-launch-*.desktop]` sections are
...
pub(crate) fn upsert_kde_shortcut_sections(current: &str, shortcuts: &[KdeShortcut]) -> String {
    ...
    for line in current.lines() {
        if line.starts_with('[') {
            skipping = line.starts_with("[services][mona-launch-");
        }
        if !skipping {
...
mod tests {
    ...
    fn gnome_list_merge_preserves_foreign_entries_and_replaces_ours() {
        ...
        assert!(!merged.contains("mona-launch-7"));
        assert_eq!(merged.matches("mona-launch-0").count(), 1);
        assert_eq!(merged.matches("mona-launch-1").count(), 1);
        // Idempotent.
    ...
    fn dconf_keybindings_share_gnome_accelerators_with_slot_names() {
        ...
        assert_eq!(kbs.len(), 2);
        assert_eq!(kbs[0].slot, "mona-launch-0");
        assert_eq!(kbs[1].slot, "mona-launch-1");
    ...
    fn gnome_list_merge_also_handles_cinnamon_slot_names() {
        ...
        let current = "['custom0', 'mona-launch-0', 'mona-launch-5']";
        let merged = merge_gnome_keybinding_list(current, &ours);
        assert!(merged.contains("'custom0'"));
        assert!(!merged.contains("mona-launch-5"));
        assert_eq!(merged.matches("mona-launch-0").count(), 1);
        assert!(merged.contains("mona-launch-1"));
        assert_eq!(merge_gnome_keybinding_list(&merged, &ours), merged);
    }
    ...
    fn kde_shortcuts_use_meta_and_literal_keys() {
        ...
        assert_eq!(scs.len(), 2);
        assert_eq!(scs[0].desktop_file_name, "mona-launch-0.desktop");
    ...
    fn kde_ini_upsert_replaces_our_sections_and_keeps_others() {
        ...
        let current = "[services][firefox.desktop]\n_launch=Meta+F\n\n[services][mona-launch-0.desktop]\n_launch=Meta+X\n\n[services][mona-launch-9.desktop]\n_launch=Meta+Y\n";
        ...
        assert_eq!(
            updated
                .matches("[services][mona-launch-0.desktop]")
                .count(),
            1
        );
        assert!(updated.contains("_launch=Meta+;"));
        assert!(!updated.contains("Meta+X"));
        assert!(!updated.contains("mona-launch-9"));
        // Idempotent.
    ...
    fn hyprland_bind_line_maps_cmd_to_super() {
        let line = render_hyprland_bind_line(&bind(
            "cmd+;",
            "/home/u/.mona/hotkey/launch_mona_0_cmd_semicolon.sh",
            "mona",
            false,
        ))
        .unwrap();
    ...
    fn sway_bind_line_maps_cmd_to_mod4_and_alt_to_mod1() {
        let line = render_sway_bind_line(&bind("cmd+;", "/s.sh", "mona", false)).unwrap();
        assert_eq!(
            line,
    ...
    fn blocks_wrap_sentinels_and_carry_labels() {
        ...
        let sway = render_sway_block(&[bind("cmd+;", "/a.sh", "mona", false)]).unwrap();
