...
pub use conflicts::{Conflict, JcodeBinding, conflict_signature, detect_conflicts, mona_bindings};
...
pub fn snapshot_path() -> anyhow::Result<std::path::PathBuf> {
    Ok(mona_storage::mona_dir()?.join("keymap-snapshot.json"))
}

/// Collect a fresh snapshot and persist it to `~/.mona/keymap-snapshot.json`.
...
pub fn refresh_and_save() -> KeymapSnapshot {
    ...
    if let Ok(path) = snapshot_path()
        ...
        && let Err(err) = mona_storage::write_json_fast(&path, &snapshot)
    {
        mona_logging::warn(&format!("keymap snapshot write failed: {err}"));
    }
    snapshot
}
