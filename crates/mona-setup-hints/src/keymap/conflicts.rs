...
//!      from [`mona_config_types::KeybindingsConfig`] ([`mona_bindings`]),
...
/// chord.
pub fn mona_bindings(cfg: &KeybindingsConfig) -> Vec<JcodeBinding> {
...
pub fn detect_conflicts(cfg: &KeybindingsConfig, snapshot: &KeymapSnapshot) -> Vec<Conflict> {
    ...
    for jcode in mona_bindings(cfg) {
