...
fn default_resolved_hotkeys_match_legacy_three() {
    ...
    let resolved = launch_hotkeys::resolve_launch_hotkeys(
        &mona_config_types::LaunchHotkeysConfig::default(),
        "/usr/local/bin/jcode",
        "/home/u/.mona/hotkey/last_dir",
        "/home/u/.mona/hotkey/last_repo",
    );
    ...
    assert!(last_dir.contains("cat '/home/u/.mona/hotkey/last_dir'"));
    assert!(last_dir.contains("cd \"$HOME\""));

    let selfdev = launch_hotkeys::shell_command_for(&resolved[2], "/usr/local/bin/jcode");
    assert!(selfdev.contains("cat '/home/u/.mona/hotkey/last_repo'"));
...
fn baked_repo_hotkey_cds_into_fixed_dir() {
    ...
    let config = mona_config_types::LaunchHotkeysConfig {
        enabled: Some(true),
        imported: true,
        entries: vec![mona_config_types::LaunchHotkeyEntry {
            chord: "cmd+[".to_string(),
            dir: "/Users/jeremy/mona-github".to_string(),
            label: "mona-github".to_string(),
            self_dev: false,
        }],
    };
    let resolved = launch_hotkeys::resolve_launch_hotkeys(
    ...
    assert!(cmd.contains("/Users/jeremy/mona-github"));
...
fn mac_hotkey_launch_agent_plist_uses_valid_xml_quotes() {
    let plist = mac_hotkey_launch_agent_plist(
        ...
        "/tmp/mona-hotkey.out.log",
        "/tmp/mona-hotkey.err.log",
        "ghostty",
    );
...
#[test]
fn paused_mona_shell_command_keeps_failures_visible() {
    let command = paused_mona_shell_command("/tmp/jcode");
...
fn keymap_conflict_hint_full_path_debounces_and_persists_signature() {
    ...
    use mona_config_types::KeybindingsConfig;
...
fn glyph_safe_notice_shows_once_then_debounces() {
    ...
    assert!(body.contains("MONA_GLYPH_SAFE_MODE=off"));
