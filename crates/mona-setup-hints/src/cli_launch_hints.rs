...
fn install_hook(path: &Path, source: CliSource) -> Result<()> {
    ...
    bytes.push(b'\n');
    mona_storage::write_bytes(path, &bytes)
...
fn hook_command(source: CliSource) -> Result<String> {
    let executable = trusted_mona_executable()?;
    Ok(format!(
        "{} {HOOK_COMMAND_MARKER}{}",
...
fn trusted_mona_executable() -> Result<PathBuf> {
    #[cfg(windows)]
    {
    ...
    #[cfg(not(windows))]
    {
        if let Some(home) = dirs::home_dir() {
            let launcher = home.join(".local").join("bin").join("mona");
            if launcher.is_file() {
                return Ok(launcher);
            }
        }
