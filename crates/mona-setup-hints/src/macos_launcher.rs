use super::{
    ...
    escape_shell_single_quotes, launch_command_for_macos_terminal, paused_mona_shell_command,
...
const MACOS_NOTIFICATION_EXECUTABLE: &str = "mona-notification-broker";
const MACOS_NOTIFICATION_VERSION_MARKER: &str = "mona-broker-version";
...
pub(super) fn install_macos_app_launcher() -> Result<(PathBuf, MacTerminalKind)> {
    ...
    let launcher_path = macos_dir.join("mona-launcher");
...
"#,
        version = mona_build_meta::version(),
...
fn macos_notification_broker_is_valid(app_dir: &Path) -> bool {
    app_dir.is_dir()
        ...
        && std::fs::read_to_string(macos_notification_broker_marker_path(app_dir))
            .is_ok_and(|version| version.trim() == mona_build_meta::version())
}
...
fn install_macos_notification_broker(mona_executable: &Path) -> Result<PathBuf> {
    ...
    if std::fs::hard_link(mona_executable, &broker_executable).is_err() {
        std::fs::copy(mona_executable, &broker_executable).with_context(|| {
            format!(
                "failed to install notification broker executable from {}",
                mona_executable.display()
            )
        })?;
    }
    #[cfg(unix)]
...
fn macos_launcher_script(terminal: MacTerminalKind, exe_path: &str, app_dir: &Path) -> String {
    ...
    let shell_command = paused_mona_shell_command(exe_path);
    ...
    let terminal_failure_message = escape_applescript_text(&format!(
        "Jcode could not open {}.\n\nTry rerunning:\njcode setup-launcher\n\nLauncher log:\n~/.mona/launcher/macos-launcher.log",
        terminal.label()
    ));
