...
fn public_sdk_enrichment_helper_accepts_older_session_records() {
    ...
    .unwrap();
    let mut sessions: Vec<mona_sdk::SessionInfo> =
        serde_json::from_str(r#"[{"session_id":"child","status":"idle"}]"#).unwrap();
    mona_sdk::enrich_sessions_from_swarm_state(&mut sessions, dir.path());
...
fn missing_launch_binary_is_typed_and_cleans_up_its_temporary_home() {
    let before = std::fs::read_dir(std::env::temp_dir())
        ...
        .filter(|entry| {
            entry
                .file_name()
                .to_string_lossy()
                .starts_with("mona-sdk-instance-")
        })
        .count();
    let error = match JcodeClient::launch(LaunchOptions {
        binary: Some(PathBuf::from("mona-definitely-not-installed-sdk-test")),
    ...
    assert_eq!(error.code(), "mona_not_found");
