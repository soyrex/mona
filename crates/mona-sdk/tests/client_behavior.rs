...
use mona_sdk::{ConnectOptions, JcodeClient, SearchTextOptions, Transport};
...
impl Transport for PairTransport {
    fn split(
        self: Box<Self>,
    ) -> mona_sdk::Result<(Box<dyn BufRead + Send>, Box<dyn Write + Send>)> {
...
fn ga_runtime_and_file_methods_map_requests_and_typed_replies() {
    ...
    let routes = vec![
        ModelRouteInfo {
            usage: Some(mona_sdk::ModelUsage {
                count: 7,
    ...
    client.clear_api_key("mona").expect("clear key");
...
fn an_error_frame_becomes_a_typed_failure() {
    let client = fake_harness(|frame, writer| {
        reply(
            frame,
            ApiEvent::Error {
                code: mona_harness_api::ErrorCode::UnknownSession,
...
fn an_error_mid_turn_fails_the_run_instead_of_hanging() {
    let client = fake_harness(|frame, writer| {
        if let ApiRequest::SendMessage { session_id, .. } = &frame.request {
            ...
            push(
                ApiEvent::Error {
                    code: mona_harness_api::ErrorCode::Internal,
...
fn model_switch_refusal_is_a_typed_error_not_a_success() {
    let client = fake_harness(|frame, writer| {
        ...
        reply(
            frame,
            ApiEvent::Error {
                code: mona_harness_api::ErrorCode::InvalidRequest,
    ...
    assert_eq!(
        error.kind,
        mona_sdk::ErrorKind::Harness(mona_harness_api::ErrorCode::InvalidRequest)
    );
            ...
            for sid in ["other", session_id.as_str()] {
                push(
                    ApiEvent::SidePanelState {
                        session_id: sid.into(),
                        snapshot: mona_sdk::SidePanelSnapshot {
                            focus_revision: 0,
                            focused_page_id: Some("notes".into()),
                            pages: vec![mona_sdk::SidePanelPage {
                                id: "notes".into(),
                                content: "# Notes".into(),
                                ..Default::default()
                            }],
