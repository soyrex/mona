...
#[test]
#[ignore = "set MONA_AUTH_TEST_BINARY to an installed CLI for compatibility validation"]
fn installed_cli_begin_cancel_isolated() {
    let binary =
        PathBuf::from(std::env::var_os("MONA_AUTH_TEST_BINARY").expect("explicit CLI path"));
    for provider in ["claude", "openai"] {
        let home = tempfile::tempdir().unwrap();
        let client = AuthClient::new(AuthOptions {
            binary: binary.clone(),
            mona_home: Some(home.path().to_owned()),
...
fn catalog_is_capability_filtered_and_uses_shared_aliases() {
    ...
    assert_eq!(
        client.resolve_provider("mona").unwrap().method,
        LoginMethod::ApiKey
    );
    ...
    assert!(client.begin("mona", None).is_err());
}
...
home = Path(os.environ['MONA_HOME'])
args = sys.argv[1:]
...
"#,
        ...
        let client = AuthClient::new(AuthOptions {
            binary,
            mona_home: Some(dir.path().to_owned()),
