...
use mona_provider_metadata::{LoginProviderDescriptor, LoginProviderTarget};
...
pub struct AuthOptions {
    ...
    pub binary: PathBuf,
    /// Credential home. None inherits the current MONA_HOME/default home.
...
impl Default for AuthOptions {
    fn default() -> Self {
        Self {
            binary: std::env::var_os("MONA_BIN")
                .map(PathBuf::from)
                .unwrap_or_else(|| "mona".into()),
            mona_home: None,
            socket: None,
...
impl AuthClient {
    ...
    pub fn providers(&self) -> Vec<LoginProvider> {
        mona_provider_metadata::cli_login_providers()
            .into_iter()
    ...
    pub fn resolve_provider(&self, input: &str) -> Option<LoginProvider> {
        mona_provider_metadata::resolve_login_provider_loose(input).and_then(supported)
    }
...
impl FlowInner {
    fn notify_daemon_best_effort(&self) {
        let socket = self
            ...
            .socket
            .clone()
            .unwrap_or_else(mona_harness_api::legacy_socket_path);
        let provider = self.provider;
        let (tx, rx) = mpsc::sync_channel(1);
        std::thread::spawn(move || {
            if let Ok(mut stream) = mona_transport::SyncStream::connect(&socket) {
                #[cfg(unix)]
    ...
    fn command(&self, operation: Operation) -> Command {
        ...
        if let Some(home) = &self.options.mona_home {
            command.env("MONA_HOME", home);
        }
        command.args([
            "login",
            "--provider",
