...
type PlatformUnixStream = mona_transport::SyncStream;
                ...
                match cause.kind() {
                    std::io::ErrorKind::NotFound => format!(
                        "no harness API socket at {}: the jcode harness is not running. \
                         Start it with `jcode serve` and `mona-harness-api-bridge`, or \
...
pub struct JcodeClient {
    ...
    /// Server identity from the handshake, e.g. "mona-harness-api-bridge/0.1.0".
    pub server: String,
...
impl JcodeClient {
    ...
    pub fn launch(options: LaunchOptions) -> Result<Self> {
        ...
        client.instance_home = Some(instance.mona_home.clone());
    ...
    pub fn get_history_with_images(
        &self,
        session_id: &str,
    ) -> Result<(Vec<HistoryMessage>, Vec<mona_harness_api::RenderedImage>)> {
        match self
...
fn start_global_child(parent: &JcodeClient, control: &Arc<GlobalEventControl>, session_id: String) {
    ...
    if let Err(error) = child.attach_session(&session_id) {
        if !matches!(
            error.kind,
            ErrorKind::Harness(mona_harness_api::ErrorCode::UnknownSession)
        ) {
            stop_global_stream(control, Some(error));
        }
        return;
