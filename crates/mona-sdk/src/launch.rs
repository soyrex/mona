...
pub struct LaunchOptions {
    ...
    /// `MONA_SWARM_MODEL` override and takes precedence over `env`.
    pub swarm_model: Option<String>,
    /// Who executes autonomous wake requests. Applied as the operator-level
    /// `MONA_WAKE_MODE` override and takes precedence over `env`.
...
impl Default for LaunchOptions {
    fn default() -> Self {
        Self {
            ...
            client_name: concat!("mona-sdk-rs/", env!("CARGO_PKG_VERSION")).to_string(),
...
impl LaunchedInstance {
    fn shutdown(&self) {
        ...
        stop_instance_daemon(&self.mona_home, &self.runtime_dir);
        ...
        if self.ephemeral {
            remove_ephemeral_home(&self.mona_home, self.cleanup_timeout);
        }
    }
}
...
pub fn launch_instance(options: &LaunchOptions) -> Result<LaunchedInstance> {
    let ephemeral = options.mona_home.is_none();
    let mona_home = match &options.mona_home {
        Some(path) => {
            fs::create_dir_all(path).map_err(launch_io)?;
            path.clone()
        }
        None => tempfile::Builder::new()
            .prefix("mona-sdk-instance-")
            .tempdir()
            .map_err(launch_io)?
            .keep(),
    };
    set_owner_only_dir(&mona_home)?;

    let runtime_dir = mona_home.join("run");
    fs::create_dir_all(&runtime_dir).map_err(launch_io)?;
    set_owner_only_dir(&runtime_dir)?;
    let socket_path = runtime_dir.join("mona-api.sock");
    for stale in [
        "mona-api.sock",
        "jcode.sock",
        "mona-debug.sock",
    ...
    let cleanup_on_error = || {
        if ephemeral {
            remove_ephemeral_home(&mona_home, Duration::ZERO);
        }
    };
    if options.inherit_logins {
        if let Err(error) = inherit_credentials(&user_mona_home(), &mona_home) {
            cleanup_on_error();
            return Err(error);
        }
    }
    ...
    let binary = options
        .binary
        .clone()
        .unwrap_or_else(|| PathBuf::from("mona"));
    ...
    command
        ...
        .env("MONA_HOME", &mona_home)
        .env("MONA_RUNTIME_DIR", &runtime_dir)
        .env("MONA_API_SOCKET", &socket_path)
        .env("MONA_SOCKET", runtime_dir.join("jcode.sock"))
    ...
    if let Some(swarm_model) = options.swarm_model.as_deref() {
        command.env("MONA_SWARM_MODEL", swarm_model);
    }
    if let Some(wake_mode) = options.wake_mode {
        command.env("MONA_WAKE_MODE", wake_mode.as_str());
    }
...
pub fn user_mona_home() -> PathBuf {
    std::env::var_os("MONA_HOME")
        .map(PathBuf::from)
...
pub fn inherit_credentials(from_home: &Path, to_home: &Path) -> Result<Vec<PathBuf>> {
    ...
    if let Ok(entries) = fs::read_dir(user_app_config_dir()) {
        for entry in entries.flatten() {
            ...
            let relative = PathBuf::from("config").join("mona").join(&name);
...
pub fn socket_accepts(path: &Path) -> bool {
    mona_transport::is_socket_path(path)
}
...
pub fn ensure_runtime(options: &LaunchOptions, progress: &Progress<'_>) -> Result<()> {
    let api = options
        .api_socket
        .clone()
        .unwrap_or_else(mona_harness_api::api_socket_path);
    if socket_accepts(&api) {
        return Ok(());
    }
    let legacy = mona_harness_api::legacy_socket_path();
    let jcode = options
        .binary
        .clone()
        .unwrap_or_else(|| sibling_exe("mona"));
    ...
    let standalone_bridge = sibling_exe("mona-harness-api-bridge");
...
fn remove_ephemeral_home(home: &Path, timeout: Duration) {
    ...
    if home.parent() != Some(std::env::temp_dir().as_path())
        || !name.starts_with("mona-sdk-instance-")
    {
        return;
    }
    let deadline = Instant::now() + timeout;
...
mod tests {
    ...
    fn swarm_model_reaches_runtime_and_overrides_generic_environment() {
        ...
        fs::write(
            &binary,
            "#!/bin/sh\nprintf '%s' \"$MONA_SWARM_MODEL\" > \"$CAPTURE_PATH\"\nexit 1\n",
        )
        ...
        let mut options = LaunchOptions {
            mona_home: Some(sandbox.path().join("instance")),
        ...
        options.env.insert(
            OsString::from("MONA_SWARM_MODEL"),
    ...
    fn wake_mode_reaches_runtime_and_overrides_generic_environment() {
        ...
        fs::write(
            &binary,
            "#!/bin/sh\nprintf '%s' \"$MONA_WAKE_MODE\" > \"$CAPTURE_PATH\"\nexit 1\n",
        )
        .unwrap();
        ...
        options
            .env
            .insert("MONA_WAKE_MODE".into(), "internal".into());
    ...
    fn dropping_an_ephemeral_instance_stops_its_bridge_and_removes_its_home() {
        ...
        let instance = LaunchedInstance {
            socket_path: runtime_dir.join("mona-api.sock"),
            mona_home: home.clone(),
            runtime_dir,
            child: Mutex::new(Some(child)),
            ephemeral: true,
