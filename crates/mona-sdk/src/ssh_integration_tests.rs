...
//! Run: MONA_SSH_TEST_BINARY=/absolute/path/to/jcode cargo test -p mona-sdk \
...
#[test]
#[ignore = "requires /usr/bin/sshd, ssh-keygen and MONA_SSH_TEST_BINARY freshly built CLI"]
fn localhost_native_harness() {
    let binary = std::fs::canonicalize(
        std::env::var_os("MONA_SSH_TEST_BINARY")
            .expect("set MONA_SSH_TEST_BINARY to the freshly built jcode CLI"),
    )
    .unwrap();
    ...
    assert!(
        ...
        "MONA_SSH_TEST_BINARY must be a fresh build supporting api --stdio: {}",
    ...
    daemon_command
        ...
        .env("XDG_RUNTIME_DIR", &runtime)
        .env("MONA_RUNTIME_DIR", &runtime)
        .env("MONA_SOCKET", &daemon_socket)
        .env("MONA_DEFERRED_AUTH_BOOTSTRAP", "1")
        .env("MONA_WAKE_MODE", "external")
        .env("MONA_NO_TELEMETRY", "1")
    ...
    std::fs::write(&remote_script, format!(
        "export HOME={} MONA_HOME={} XDG_RUNTIME_DIR={} MONA_SOCKET={} MONA_WAKE_MODE=external MONA_NO_TELEMETRY=1\nexport MONA_RUNTIME_DIR=\"$XDG_RUNTIME_DIR\"\ncd \"$HOME\" || exit 1\nexec /bin/sh -c \"$SSH_ORIGINAL_COMMAND\"\n",
    ...
    let options = SshConnectOptions {
        ...
        client_name: "mona-sdk-ssh-acceptance".into(),
        ..SshConnectOptions::new("mona-test")
    };
