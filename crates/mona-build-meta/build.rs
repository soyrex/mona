...
//
// This is the single source of truth for the MONA_* compile-time values that
// previously lived in the root `jcode` crate's build.rs. It is hosted in the
// leaf `mona-build-meta` crate so every workspace crate can read identical
// values (via `mona_build_meta::*`) without duplicating this script.
//
...
fn main() {
    ...
    let dirty = match std::env::var("MONA_BUILD_GIT_DIRTY") {
        Ok(value) => matches!(
    ...
    let raw_log = std::env::var("MONA_BUILD_CHANGELOG_RAW")
        .ok()
    ...
    let is_release = std::env::var("MONA_RELEASE_BUILD").is_ok();
    ...
    println!("cargo:rustc-env=MONA_GIT_HASH={}", git_hash);
    println!("cargo:rustc-env=MONA_GIT_DATE={}", git_date);
    println!("cargo:rustc-env=MONA_VERSION={}", version);
    println!("cargo:rustc-env=MONA_SEMVER={}", build_semver);
    println!("cargo:rustc-env=MONA_BASE_SEMVER={}", base_semver);
    println!("cargo:rustc-env=MONA_UPDATE_SEMVER={}", update_semver);
    println!("cargo:rustc-env=MONA_GIT_TAG={}", git_tag);
    println!("cargo:rustc-env=MONA_CHANGELOG={}", changelog);
    println!("cargo:rustc-env=MONA_PKG_VERSION={}", pkg_version);

    // Forward MONA_RELEASE_BUILD env var if set (CI sets this for release binaries)
    if std::env::var("MONA_RELEASE_BUILD").is_ok() {
        println!("cargo:rustc-env=MONA_RELEASE_BUILD=1");
    }
    ...
    // Since `mona-build-meta` sits at the bottom of the crate graph
    ...
    //   * Release/dist builds set MONA_RELEASE_BUILD=1 and MONA_BUILD_SEMVER,
    ...
    println!("cargo:rerun-if-env-changed=MONA_RELEASE_BUILD");
    println!("cargo:rerun-if-env-changed=MONA_BUILD_SEMVER");
    // Allow callers to force a metadata refresh (e.g. install scripts) without a
    // full clean, by bumping this env var.
    println!("cargo:rerun-if-env-changed=MONA_BUILD_GIT_HASH");
    println!("cargo:rerun-if-env-changed=MONA_BUILD_GIT_DATE");
    println!("cargo:rerun-if-env-changed=MONA_BUILD_GIT_DIRTY");
    println!("cargo:rerun-if-env-changed=MONA_BUILD_GIT_TAG");
}

/// Workspace root, derived from this crate's manifest dir (`crates/mona-build-meta`).
fn repo_root() -> PathBuf {
    let manifest_dir = std::env::var("CARGO_MANIFEST_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("."));
    // crates/mona-build-meta -> crates -> <repo root>
    manifest_dir
        .parent()
        .and_then(|p| p.parent())
...
fn resolve_build_semver(base_version: (u32, u32, u32)) -> Result<String, String> {
    ...
    // function of HEAD, so the emitted MONA_SEMVER/MONA_VERSION only change when
    ...
    // turn invalidated `mona-build-meta` and force-recompiled the entire crate
...
fn metadata_value(key: &str) -> Option<String> {
    let path = std::env::var("MONA_BUILD_METADATA_FILE").ok()?;
