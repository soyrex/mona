...
fn copy_selection_text_with_math_targets(
    ...
    let math_targets: Vec<_> = prepared
        ...
        .filter(|target| {
            matches!(
                &target.kind,
                mona_tui_markdown::CopyTargetKind::Math { .. }
            )
...
mod tests {
    use super::*;
    use mona_tui_markdown::CopyTargetKind;
    use mona_tui_messages::{CopyTarget, PreparedChatFrame, PreparedMessages, WrappedLineMap};
