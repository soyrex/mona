...
impl App {
    pub(super) fn open_auth_browser(url: &str) -> bool {
        // Honors --no-browser/NO_BROWSER/MONA_NO_BROWSER and never opens real
    ...
    pub(super) fn show_mona_subscription_status(&mut self) {
        ...
        let configured_base = crate::subscription_catalog::configured_api_base()
            .unwrap_or_else(|| crate::subscription_catalog::DEFAULT_MONA_API_BASE.to_string());
    ...
    pub(super) fn start_logout_provider(
        ...
        if matches!(provider.target, LoginProviderTarget::Jcode) {
            self.start_mona_account_logout();
            return;
        }
    ...
    pub(super) fn start_logout_all(&mut self) {
        ...
        Self::clear_api_key_logout_summary(
            ...
            crate::subscription_catalog::MONA_ENV_FILE,
        );
        for env_key in [
            crate::subscription_catalog::MONA_API_BASE_ENV,
            crate::subscription_catalog::MONA_ACCOUNT_ID_ENV,
            crate::subscription_catalog::MONA_ACCOUNT_EMAIL_ENV,
            crate::subscription_catalog::MONA_TIER_ENV,
        ] {
    ...
    pub(super) fn start_login_provider(
        ...
        match provider.target {
            ...
            crate::provider_catalog::LoginProviderTarget::Jcode => self.start_mona_login(),
    ...
    pub(super) fn open_mona_account_management(&mut self) {
    ...
    pub(super) fn start_mona_account_logout(&mut self) {
    ...
    fn start_api_key_login(
        ...
        let provider_id = openai_compatible_profile
            ...
            .unwrap_or_else(|| match key_name {
                crate::subscription_catalog::MONA_API_KEY_ENV => "mona".to_string(),
        ...
        match pending {
            ...
            PendingLogin::ApiKeyProfile {
                ...
                let save_result: anyhow::Result<()> =
                    ...
                    } else if key_name == crate::subscription_catalog::MONA_API_KEY_ENV {
                        (|| {
                    ...
                    Ok(()) => {
                        ...
                        if key_name == crate::provider::bedrock::API_KEY_ENV {
                            ...
                            if let Some(default_model) = default_model.as_deref() {
                                crate::env::set_var("MONA_BEDROCK_MODEL", default_model);
                            }
                        }
                        ...
                        let model_hint = effective_default_model
                            ...
                            .unwrap_or_default();
                        let guidance = if key_name == crate::subscription_catalog::MONA_API_KEY_ENV
                        {
                            format!(
    ...
    pub(super) fn onboarding_should_prefer_strongest_model(&self) -> bool {
        ...
        let runtime_provider_explicit = std::env::var("MONA_INITIAL_PROVIDER_EXPLICIT")
            .ok()
            .is_some_and(|value| {
