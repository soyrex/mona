...
/// NO_BROWSER/MONA_NO_BROWSER and refuses to open anything from test binaries
...
pub(crate) fn open_path_or_url_detached(
    ...
    if crate::auth::browser_suppressed(false) {
        return Err(std::io::Error::other(
            "opening files/URLs is suppressed (NO_BROWSER/MONA_NO_BROWSER or test harness)",
        ));
    }
...
pub(super) fn launch_client_executable() -> PathBuf {
    crate::build::client_update_candidate(mona_selfdev_types::client_selfdev_requested())
...
pub(super) fn debug_cmd_path() -> PathBuf {
    if let Ok(path) = std::env::var("MONA_DEBUG_CMD_PATH") {
        return PathBuf::from(path);
    }
    std::env::temp_dir().join("mona_debug_cmd")
}

/// Debug response file path
pub(super) fn debug_response_path() -> PathBuf {
    if let Ok(path) = std::env::var("MONA_DEBUG_RESPONSE_PATH") {
        return PathBuf::from(path);
    }
    std::env::temp_dir().join("mona_debug_response")
}
...
pub(super) fn inferred_reasoning_efforts(
    ...
    mona_provider_core::inferred_reasoning_efforts(provider_name, model_name)
}
...
pub(super) fn build_resume_command(
    ...
    match target {
        ...
        ResumeTarget::CodexSession { session_id, .. } => {
            ...
            let args = resume_invocation_args(&imported_id, socket);
            let title = format!("🧠 Codex {}", mona_core::util::truncate_str(session_id, 8));
            (exe, args, title)
        }
        ...
        ResumeTarget::CursorSession { session_id, .. } => {
            ...
            let args = resume_invocation_args(&imported_id, socket);
            let title = format!("▮ Cursor {}", mona_core::util::truncate_str(session_id, 8));
            (exe, args, title)
        }
    }
}
...
pub(super) fn clipboard_image() -> Option<(String, String)> {
            ...
            && !html_output.stdout.is_empty()
        {
            ...
            if let Some(url) = extract_image_url(&html) {
                crate::logging::info(&format!(
                    ...
                    mona_core::util::truncate_str(&url, 80)
                ));
    ...
    #[cfg(target_os = "macos")]
    {
        let temp_path = std::env::temp_dir().join("mona_clipboard.png");
        let script = format!(
