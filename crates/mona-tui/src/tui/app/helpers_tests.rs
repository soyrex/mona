...
fn resume_invocation_args_includes_socket_when_present() {
    let args = resume_invocation_args("ses_123", Some("/tmp/mona-test.sock"));
    assert_eq!(
        args,
        vec![
...
/// Pin MONA_HOME to a tempdir containing a `builds/current/jcode` binary so
...
/// other tests mutating MONA_HOME in parallel. Returns the guards that keep
...
fn pinned_resume_test_home() -> (
    ...
    std::fs::write(current.join("mona"), b"#!/bin/sh\n").expect("write fake jcode binary");
    let home = EnvVarGuard::set_path("MONA_HOME", temp.path());
    (env_lock, temp, home)
}

#[test]
fn build_resume_command_uses_imported_mona_session_for_claude_code() {
...
#[test]
fn build_resume_command_uses_imported_mona_session_for_codex() {
    let _pinned = pinned_resume_test_home();
    let (program, args, title) = build_resume_command(
...
fn gather_ambient_info_filters_to_session_reminders_when_ambient_disabled() {
    ...
    // Read synchronously while the temporary MONA_HOME and its files are pinned:
    // an unrelated in-flight cache refresh can otherwise overwrite the cleared
    // process-global cache with data loaded under another test's MONA_HOME.
