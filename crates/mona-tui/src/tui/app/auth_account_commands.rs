...
fn parse_account_command(trimmed: &str) -> Option<Result<AccountCommand, String>> {
    ...
    if let Some(provider) = resolve_account_provider_descriptor(first) {
        ...
        let parsed = match subcommand {
            ...
            "status" if provider.id == "mona" => AccountCommand::JcodeStatus,
            "manage" if provider.id == "mona" => AccountCommand::JcodeManage,
            "logout" if provider.id == "mona" => AccountCommand::JcodeLogout,
...
pub(crate) fn execute_account_command_local(app: &mut App, command: AccountCommand) {
    match command {
        ...
        AccountCommand::JcodeStatus => app.show_mona_subscription_status(),
        AccountCommand::JcodeManage => app.open_mona_account_management(),
        AccountCommand::JcodeLogout => app.start_mona_account_logout(),
...
fn save_default_provider_setting(app: &mut App, provider: Option<&str>) {
    ...
    let provider = match normalized.as_deref() {
        ...
        Some(other) if mona_provider_core::AuthRoute::parse(other).is_some() => normalized,
        Some(other) => {
            app.push_display_message(DisplayMessage::error(format!(
...
fn save_openai_compat_setting(app: &mut App, setting: OpenAiCompatSetting, value: Option<&str>) {
    ...
    let (env_key, normalized_value) = match setting {
        OpenAiCompatSetting::ApiBase => {
            ...
            ("MONA_OPENAI_COMPAT_API_BASE", normalized)
        }
...
mod tests {
    ...
    #[test]
    fn parse_native_mona_account_actions() {
        assert!(matches!(
            parse_account_command("/account jcode login"),
            Some(Ok(AccountCommand::Login { provider_id })) if provider_id == "mona"
        ));
        assert!(matches!(
