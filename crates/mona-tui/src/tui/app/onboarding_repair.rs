...
/// `MONA_HOME`/external isolation, matching the onboarding import detectors.
...
pub(crate) fn build_repair_brief(
    ...
    let log_line = mona_logging::log_path()
        .map(|p| format!("Logs:    {}", p.display()))
        .unwrap_or_else(|| "Logs:    ~/.mona/logs/mona-<date>.log".to_string());
...
/// to paste anything. Lives under the jcode home so it honors `MONA_HOME`.
