...
fn kv_cache_baseline_from_other_session_is_ignored() {
    ...
    // ~/.mona/logs KV_CACHE_USAGE telemetry (common_prefix=0, current
...
fn stale_server_history_is_deferred_before_remote_state_is_applied() {
    crate::env::remove_var("MONA_ALLOW_SERVER_VERSION_MISMATCH");
    let mut app = create_test_app();
    let rt = tokio::runtime::Runtime::new().unwrap();
    let _guard = rt.enter();
    let mut remote = crate::tui::backend::RemoteConnection::dummy();
...
fn deferred_stale_server_history_captures_session_id_for_reload_handoff() {
    ...
    crate::env::remove_var("MONA_ALLOW_SERVER_VERSION_MISMATCH");
    crate::env::set_var("MONA_TEST_CLIENT_VERSION_OVERRIDE", "v0.21.0 (deadbeef)");
    ...
    crate::env::remove_var("MONA_TEST_CLIENT_VERSION_OVERRIDE");
}
...
fn ancient_server_history_is_deferred_via_client_side_release_check() {
    ...
    crate::env::set_var("MONA_TEST_CLIENT_VERSION_OVERRIDE", "v0.17.0 (d741696f)");
...
fn older_server_reporting_no_update_is_still_deferred_via_client_check() {
    ...
    let _env_guard = crate::storage::lock_test_env();
    crate::env::remove_var("MONA_ALLOW_SERVER_VERSION_MISMATCH");
    crate::env::set_var("MONA_TEST_CLIENT_VERSION_OVERRIDE", "v0.22.0 (abcd1234)");
...
fn older_server_history_repairs_stale_shared_server_channel_end_to_end() {
    // Full-path sandbox: a real temp MONA_HOME set up in the exact field state
...
fn cache_timer_openai_routes_distinguish_api_oauth_and_unknown() {
    ...
    for (credential, route, identity, has_timer) in [
        ...
        (
            Some(mona_provider_core::ResolvedCredential::Oauth),
            None,
            "openai-oauth",
            false,
        ),
...
fn cache_timer_local_explicit_openai_route_does_not_probe_auto_credentials() {
    #[derive(Clone)]
    struct PinnedOpenAI(Option<mona_provider_core::ResolvedCredential>);
