...
impl App {
    ...
    pub(super) fn restore_input_for_reload(session_id: &str) -> Option<RestoredReloadInput> {
        let mona_dir = crate::storage::mona_dir().ok()?;
        let path = mona_dir.join(format!("client-input-{}", session_id));
        if !path.exists() {
            return None;
        }
    ...
    pub fn debug_socket_path() -> std::path::PathBuf {
        crate::storage::runtime_dir().join("mona-debug.sock")
    }
}
...
/// plus the curated list of mona-endorsed skills (marking which are installed).
fn build_skills_report(app: &App) -> String {
    ...
    if app.is_remote && !app.remote_skills.is_empty() {
        ...
        if skills.is_empty() {
            out.push_str(
                "- none loaded\n  Add skills under ~/.mona/skills/<name>/SKILL.md or ./.mona/skills/<name>/SKILL.md\n",
            );
        } else {
            for skill in skills {
...
pub(super) fn handle_info_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if trimmed == "/version" {
        let version = mona_build_meta::version();
    ...
    if trimmed == "/info" {
        ...
        if let Some(ref provider_id) = app.provider_session_id {
            info.push_str(&format!(
                "Provider Session: {}...\n",
                mona_core::util::truncate_str(provider_id, 16)
            ));
        }
        ...
        let compaction_summary = if app.provider.supports_compaction() {
            ...
            if let Ok(manager) = manager.try_read() {
                ...
                format!(
                    "- supported: yes\n- mode: {}\n- mona-managed: {}\n- active summary: {} ({})\n- compacted messages: {}\n- active messages: {}\n- summary chars: {}\n- estimated tokens: {}\n- effective tokens: {}\n- observed tokens: {}\n- usage: {:.1}%\n- compacting now: {}\n- budget: {}",
                    mode,
                    if app.provider.uses_mona_compaction() {
                        "yes"
                    } else {
                        "no"
                    },
