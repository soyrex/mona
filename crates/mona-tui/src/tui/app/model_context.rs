...
impl App {
    ...
    fn failover_config_hint() -> &'static str {
        "To turn this off, set [provider].cross_provider_failover = \"manual\" in ~/.mona/config.toml or export MONA_CROSS_PROVIDER_FAILOVER=manual."
    }
    ...
    pub(super) fn current_route_api_method(&self) -> Option<String> {
        ...
        let provider =
            mona_provider_core::parse_provider_hint(&self.current_provider_label_for_fallback());
        let credential = if self.is_remote {
            self.remote_resolved_credential
        } else {
            self.provider.active_resolved_credential()
        };
        use mona_provider_core::{ActiveProvider, ResolvedCredential};
    ...
    fn is_guardrail_reroute_model(model: &str) -> bool {
        let canonical = mona_provider_core::model_id::canonical(model);
        mona_provider_core::model_id::strip_date_suffix(&canonical) == GUARDRAIL_REROUTE_MODEL
    }
    ...
    pub(super) fn effective_context_tokens_from_usage(
        ...
        // Shared heuristic (mona-compaction-core): keeps the sidebar figure
