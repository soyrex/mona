...
fn build_channel() -> String {
    ...
    if mona_build_meta::is_release_build() {
...
fn read_telemetry_id() -> Option<String> {
    let path = crate::storage::mona_dir().ok()?.join("telemetry_id");
...
fn gather_diagnostics(app: &App) -> SupportDiagnostics {
    ...
    let (account_id, account_email, tier) = if has_subscription {
        (
            load_env_value_from_env_or_config(cat::MONA_ACCOUNT_ID_ENV, cat::MONA_ENV_FILE),
            load_env_value_from_env_or_config(cat::MONA_ACCOUNT_EMAIL_ENV, cat::MONA_ENV_FILE),
