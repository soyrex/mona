...
impl App {
    pub(super) fn command_help(&self, topic: &str) -> Option<String> {
        ...
        let help = match topic.as_str() {
            ...
            "log" => {
                "/log mark [note]\nWrite a distinctive MONA_LOG_MARK line to ~/.mona/logs/mona-YYYY-MM-DD.log with the current session, provider, model, working directory, and optional note. Use this to mark a spot for agents to inspect later."
            }
            "goals" => {
            ...
            "poke" => {
                ...
                Set auto_poke = false under [features] in ~/.mona/config.toml to start with it disabled.\n\
        ...
        let help = help.replace(
            "Alt+",
            &format!("{}+", mona_tui_core::keybind::alt_label()),
        );
        Some(help)
    }
}
