...
impl App {
    ...
    /// the header uses (session stub, `MONA_MODEL`, config default) so effort
    ...
    pub(super) fn cache_provider_identity(&self, provider: &str) -> String {
        ...
        let credential = if self.uses_server_or_replay_metadata() {
            self.remote_resolved_credential.or_else(|| {
                self.session
                    .route_api_method
                    .as_deref()
                    .and_then(mona_provider_core::AuthRoute::parse)
                    .filter(|route| {
                        route.active_provider() == mona_provider_core::ActiveProvider::OpenAI
                    })
        ...
        match credential {
            Some(mona_provider_core::ResolvedCredential::ApiKey) => "openai-api".to_string(),
            Some(mona_provider_core::ResolvedCredential::Oauth) => "openai-oauth".to_string(),
    ...
    ///   [`AuthStatus`] snapshot (cached up to 60s) or a `MONA_RUNTIME_PROVIDER`
    ...
    fn widget_auth_method(&self, route: WidgetRouteInfo) -> crate::tui::info_widget::AuthMethod {
        ...
        match route.provider {
            WidgetProviderKind::Anthropic => {
                match self
                    .dual_credential_active(route, mona_provider_core::ActiveProvider::Claude)
                {
            ...
            WidgetProviderKind::OpenAI => {
                match self
                    .dual_credential_active(route, mona_provider_core::ActiveProvider::OpenAI)
                {
...
impl crate::tui::TuiState for App {
    ...
    fn session_compaction_count(&self) -> usize {
        if self.is_remote || !self.provider.uses_mona_compaction() {
            return 0;
        }
        self.registry
            .compaction()
    ...
    fn status_notice(&self) -> Option<String> {
        if !self.is_remote
            && self.provider.uses_mona_compaction()
    ...
    fn context_snapshot(&self) -> crate::tui::ContextSnapshot {
        ...
        let (compaction_count, compaction_summary_chars, is_compacting, compaction_fresh) =
            if self.is_remote {
                (0, 0, false, true)
            } else if self.provider.uses_mona_compaction() {
        ...
        if self.is_remote {
            ...
            let skip = if self.provider.uses_mona_compaction() {
    ...
    fn info_widget_data(&self) -> crate::tui::info_widget::InfoWidgetData {
                ...
                || !managed_members.is_empty()
            {
                ...
                Some(crate::tui::info_widget::SwarmInfo {
                    ...
                    spinner_frame: (self.animation_elapsed()
                        * mona_tui_render::swarm_gallery::STRIP_SPINNER_FPS)
                        as usize,
                    managed_members,
                })
            } else {
        ...
        let compaction_info = if !self.is_remote && self.provider.uses_mona_compaction() {
            let compaction = self.registry.compaction();
        ...
        crate::tui::info_widget::InfoWidgetData {
            ...
            cache_hit_info,
            compaction_info,
            is_compacting: if !self.is_remote && self.provider.uses_mona_compaction() {
                let compaction = self.registry.compaction();
                compaction
                    .try_read()
...
impl App {
    ...
    pub(crate) fn pop_out_selected_swarm_agent(&mut self) {
        ...
        let exe = std::env::current_exe().unwrap_or_else(|_| std::path::PathBuf::from("mona"));
        let cwd = std::env::current_dir().unwrap_or_else(|_| std::path::PathBuf::from("."));
        match mona_app_core::session_launch::spawn_resume_in_new_terminal(&exe, &session_id, &cwd)
        {
