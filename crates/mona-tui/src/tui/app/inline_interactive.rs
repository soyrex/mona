...
struct RemoteModelCatalogCache {
    ...
    origin: String,
    #[serde(flatten)]
    snapshot: mona_provider_core::ModelCatalogSnapshot,
...
fn model_picker_route_is_current(
    ...
    mona_provider_core::model_route_provider_labels_match(&route.provider, current_provider)
        && current_api_method
            .map(|method| route.api_method.eq_ignore_ascii_case(method))
            .unwrap_or(true)
}
...
fn model_picker_route_is_recommended(model_name: &str, route: &PickerOption) -> bool {
    RECOMMENDED_MODELS.contains(&model_name)
        && mona_provider_core::model_route_metadata_is_recommended(
            model_name,
            &route.provider,
            &route.api_method,
            route.available,
...
fn model_picker_route_provider_matches_key(
    ...
    mona_provider_core::model_route_provider_matches_key(
        route_provider_key,
...
fn model_picker_route_is_default(
    ...
    if let Some((bare_model, provider_label)) = default_model.rsplit_once('@') {
        return bare_model == model_name
            && mona_provider_core::model_route_provider_labels_match(
                &route.provider,
                provider_label,
            );
    }
...
impl App {
    ...
    pub(super) fn remote_model_catalog_snapshot(
        &self,
    ) -> mona_provider_core::ModelCatalogSnapshot {
        mona_provider_core::ModelCatalogSnapshot::new(
    ...
    fn append_mona_subscription_routes_static(
        ...
        let existing = routes
            ...
            .filter(|route| {
                route
                    .api_method
                    .eq_ignore_ascii_case(crate::subscription_catalog::MONA_ROUTE_API_METHOD)
            })
            ...
            })
        {
            routes.push(crate::provider::ModelRoute {
                ...
                provider: crate::subscription_catalog::MONA_PROVIDER_DISPLAY_NAME.to_string(),
                api_method: crate::subscription_catalog::MONA_ROUTE_API_METHOD.to_string(),
                available: true,
    ...
    fn extend_remote_routes_for_uncovered_models_static(
        ...
        let provider_is_mona_subscription = remote_provider_name.is_some_and(|name| {
            name.eq_ignore_ascii_case(crate::subscription_catalog::MONA_PROVIDER_DISPLAY_NAME)
        });
        if provider_is_mona_subscription {
            routes.clear();
            Self::append_mona_subscription_routes_static(
        ...
        let poisoned_by_mona_subscription = !routes.is_empty()
            && routes.iter().all(|route| {
                route
                    .api_method
                    .eq_ignore_ascii_case(crate::subscription_catalog::MONA_ROUTE_API_METHOD)
            });
        if poisoned_by_mona_subscription {
    ...
    fn open_cached_model_picker_if_fresh(
        ...
        if std::env::var("MONA_LOG_MODEL_PICKER_TIMING").is_ok() {
    ...
    fn open_model_picker_with_routes(
        ...
        fn route_matches_recent_auth(route_provider: &str, login_provider: &str) -> bool {
            mona_provider_core::model_route_provider_labels_related(route_provider, login_provider)
        }
        ...
        entries.sort_by(|a, b| {
            ...
            a_current
                ...
                .then(mona_provider_core::compare_model_usage(
                    entry_usage(a),
                    entry_usage(b),
                ))
        ...
        if total_ms >= 250 || std::env::var("MONA_LOG_MODEL_PICKER_TIMING").is_ok() {
            crate::logging::info(&format!(
    ...
    pub(super) fn handle_session_picker_selection(&mut self, targets: &[ResumeTarget]) {
        ...
        let socket = std::env::var("MONA_SOCKET").ok();
        ...
        for target in targets {
            ...
            let name = match target {
                ...
                ResumeTarget::ClaudeCodeSession { session_id, .. } => {
                    format!(
                        "Claude Code {}",
                        mona_core::util::truncate_str(session_id, 8)
                    )
                }
                ResumeTarget::CodexSession { session_id, .. } => {
                    format!("Codex {}", mona_core::util::truncate_str(session_id, 8))
                }
                ...
                ResumeTarget::OpenCodeSession { session_id, .. } => {
                    format!("OpenCode {}", mona_core::util::truncate_str(session_id, 8))
                }
                ResumeTarget::CursorSession { session_id, .. } => {
                    format!("Cursor {}", mona_core::util::truncate_str(session_id, 8))
                }
            };
    ...
    fn handle_live_claude_takeover(&mut self, target: &ResumeTarget) -> bool {
        ...
        let display_id = mona_core::util::truncate_str(session_id, 8);
    ...
    pub(super) fn picker_fuzzy_score(pattern: &str, text: &str) -> Option<i32> {
        mona_fuzzy::fuzzy_score_tokens(pattern, text)
    }
    ...
    pub(super) fn apply_inline_interactive_filter(picker: &mut InlineInteractiveState) {
        if picker.filter.is_empty() {
            ...
            let prepared = mona_fuzzy::PreparedTokenQuery::new(&picker.filter);
...
mod tests {
    ...
    fn model_picker_current_route_allows_provider_aliases() {
        assert!(mona_provider_core::model_route_provider_labels_match(
            "Anthropic",
            "Claude"
        ));
        assert!(mona_provider_core::model_route_provider_labels_match(
    ...
    fn model_picker_recommended_route_is_provider_aware() {
        ...
        // Current policy (see mona-provider-core): claude-opus-4-8 is the
    ...
    fn remote_model_catalog_cache_rejects_stale_and_future_timestamps() {
        let snapshot = mona_provider_core::ModelCatalogSnapshot::new(
    ...
    fn remote_model_catalog_cache_rejects_forged_or_oversized_routes() {
        let safe_snapshot = mona_provider_core::ModelCatalogSnapshot::new(
