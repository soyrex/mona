...
impl App {
    ...
    pub(super) fn is_new_user_install() -> bool {
        let Ok(dir) = crate::storage::mona_dir() else {
            return true;
        };
    ...
    fn has_established_native_session_history(mona_dir: &std::path::Path) -> bool {
        const ESTABLISHED_SESSION_THRESHOLD: usize = 10;
        let Ok(entries) = std::fs::read_dir(mona_dir.join("sessions")) else {
            return false;
        };
    ...
    fn handle_onboarding_import_review_key(&mut self, code: KeyCode) -> bool {
        ...
        if open_pricing {
            self.onboarding_import_error = None;
            let url = crate::subscription_catalog::MONA_PRICING_URL;
    ...
    /// with a large `~/.mona/sessions` directory. Doing it inline on Enter made
    ...
    fn onboarding_other_provider_rows(skip: Option<&str>) -> (Vec<String>, Vec<String>) {
        ...
        let providers: [(&str, &str, AuthState); 8] = [
            ...
            ("Jcode subscription", "mona", status.jcode),
