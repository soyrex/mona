...
#[test]
fn tui_openai_compatible_api_base_keeps_mona_docs_and_remote_endpoint() -> anyhow::Result<()> {
    with_temp_mona_home(|| {
...
#[test]
fn tui_mona_subscription_logout_clears_credentials_and_preserves_api_base() -> anyhow::Result<()> {
    with_temp_mona_home(|| {
        ...
        assert!(std::env::var_os(crate::subscription_catalog::MONA_API_KEY_ENV).is_none());
        assert_eq!(
            std::env::var(crate::subscription_catalog::MONA_API_BASE_ENV).as_deref(),
            Ok("https://subscription.example/v1")
        );
        assert!(std::env::var_os(crate::subscription_catalog::MONA_ACCOUNT_ID_ENV).is_none());
        assert!(std::env::var_os(crate::subscription_catalog::MONA_ACCOUNT_EMAIL_ENV).is_none());
