...
pub(super) fn is_auto_poke_connectivity_error(error: &str) -> bool {
    // Delegate to the shared connectivity classifier (mona-app-core's
...
pub(super) fn handle_log_command(app: &mut App, trimmed: &str) -> bool {
    ...
    crate::logging::info(&format!(
        "MONA_LOG_MARK id={} session={} provider={} model={} cwd={} note={}",
        marker_id,
        app.session.id,
        app.provider_name(),
        app.provider_model(),
    ...
    let mut message = format!(
        "Log mark written: {}\n\nAgents can search ~/.mona/logs/ for MONA_LOG_MARK or this marker id.",
        marker_id
    );
    if !note.is_empty() {
...
Type your password into that SSH prompt if asked. Jcode will not see or store it.",
            ...
            crate::ssh_remote::control_socket_path(&profile.name)
                ...
                .unwrap_or_else(|_| "~/.mona/ssh-control/remote.sock".to_string()),
            profile.ssh_target
        ))),
        Err(error) => app.push_display_message(DisplayMessage::error(format!(
...
pub(super) fn build_remote_release_prompt() -> String {
    let mona_release = build_release_prompt(
        "",
...
pub(super) fn handle_dictation_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if trimmed.starts_with("/dictate ") || trimmed.starts_with("/dictation ") {
        app.push_display_message(DisplayMessage::error(
            "Usage: /dictate\nConfigure [dictation] in ~/.mona/config.toml to customize command, mode, hotkey, and timeout."
                .to_string(),
        ));
        return true;
    }
...
fn ensure_swarm_prompt_edit_path(
    ...
    let global_path = mona_dir.join("swarm-prompt.md");
    if file_has_nonblank_content(&global_path) {
        return Ok(global_path);
    }

    std::fs::create_dir_all(mona_dir)?;
...
pub(super) fn handle_swarm_prompt_command(app: &mut App, trimmed: &str) -> bool {
    ...
    let mona_dir = match crate::storage::mona_dir() {
        Ok(path) => path,
        Err(error) => {
            app.push_display_message(DisplayMessage::error(format!(
    ...
    let path = match ensure_swarm_prompt_edit_path(app.session.working_dir.as_deref(), &mona_dir) {
        Ok(path) => path,
        Err(error) => {
            app.push_display_message(DisplayMessage::error(format!(
...
fn handle_alignment_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if rest.is_empty() || matches!(rest, "show" | "status") {
        ...
        app.push_display_message(DisplayMessage::system(format!(
            ...
            alignment_label(saved), mona_tui_core::keybind::alt_chord("C")
        )));
        return true;
    }
...
pub(super) fn handle_config_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if trimmed == "/subscription" || trimmed == "/subscription status" {
        app.show_mona_subscription_status();
        return true;
    }
