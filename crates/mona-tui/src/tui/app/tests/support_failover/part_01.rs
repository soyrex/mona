...
fn cleanup_background_task_files(task_id: &str) {
    let task_dir = std::env::temp_dir().join("mona-bg-tasks");
...
/// Point `MONA_HOME` at a per-process scratch directory if nothing set one.
///
...
/// that scope their own `MONA_HOME` restore it by *removing* the variable, so
/// without serialization this function would observe the gap and repoint
/// `MONA_HOME` at the shared scratch home while that test was still running.
/// That is the race behind mona-tui's intermittent failures in unrelated
/// ambient, header, and model-picker tests, which all read files under
/// `MONA_HOME` mid-assertion.
///
...
/// `with_temp_mona_home` hold this same non-reentrant mutex for their whole
...
/// test has scoped its own `MONA_HOME`.
fn shared_test_mona_home() -> &'static std::path::Path {
...
fn ensure_test_mona_home_if_unset() {
    ...
    // own MONA_HOME under `lock_test_env`. The mutex is not reentrant and
    // several tests hold it while calling `create_test_app` (e.g. the
    // pinned-todo-band test), so a blocking `lock_test_env()` here would
    // self-deadlock whenever a preceding test removed MONA_HOME on drop.
    ...
    crate::env::set_var("MONA_HOME", shared_test_mona_home());
}

fn clear_persisted_test_ui_state() {
    // Only wipe ambient files in the *shared* per-process test home, and only
    // while `MONA_HOME` still points at it. `create_test_app` runs from ~800
    // sites without the env lock, so following whatever `MONA_HOME` happens
    ...
    let home_is_shared = std::env::var_os("MONA_HOME")
        .is_some_and(|home| std::path::Path::new(&home) == shared_test_mona_home());
    if home_is_shared {
        let ambient_dir = shared_test_mona_home().join("ambient");
...
fn with_temp_mona_home<T>(f: impl FnOnce() -> T) -> T {
...
/// Run `f` in a hermetic `MONA_HOME` with reasoning display pinned to
/// `current`.
///
...
fn create_mona_repo_fixture() -> tempfile::TempDir {
