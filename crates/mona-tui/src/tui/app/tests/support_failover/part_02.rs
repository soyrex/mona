...
impl Provider for DualMethodMockProvider {
    ...
    fn active_resolved_credential(&self) -> Option<mona_provider_core::ResolvedCredential> {
        mona_provider_core::AuthRoute::parse(&self.api_method.lock().unwrap())
...
fn test_fallback_uses_remote_openai_credential_over_stale_route() {
    with_temp_mona_home(|| {
        ...
        for (credential, active_method, alternative) in [
            (
                mona_provider_core::ResolvedCredential::ApiKey,
                "openai-api",
                "openai-oauth",
            ),
            (
                mona_provider_core::ResolvedCredential::Oauth,
                "openai-oauth",
                "openai-api",
            ),
        ] {
