...
fn test_remote_hydrated_catalog_restores_missing_direct_bedrock_route() {
    with_temp_mona_home(|| {
        let previous_enable = std::env::var_os("MONA_BEDROCK_ENABLE");
        crate::env::set_var("MONA_BEDROCK_ENABLE", "1");
        crate::auth::AuthStatus::invalidate_cache();
        ...
        match previous_enable {
            Some(value) => crate::env::set_var("MONA_BEDROCK_ENABLE", value),
            None => crate::env::remove_var("MONA_BEDROCK_ENABLE"),
        }
        crate::auth::AuthStatus::invalidate_cache();
...
#[test]
fn test_remote_mona_subscription_catalog_is_not_augmented_with_local_auth_routes() {
    with_temp_mona_home(|| {
        ...
        app.remote_provider_name =
            Some(crate::subscription_catalog::MONA_PROVIDER_DISPLAY_NAME.to_string());
        ...
        assert!(app.remote_model_options.iter().all(|route| {
            route.provider == crate::subscription_catalog::MONA_PROVIDER_DISPLAY_NAME
                && route.api_method == crate::subscription_catalog::MONA_ROUTE_API_METHOD
                && route.available
        }));
        assert_eq!(
            app.remote_model_options
...
#[test]
fn test_remote_mixed_catalog_keeps_mona_subscription_separate_from_other_providers() {
    ...
    // `with_temp_mona_home` takes the shared env lock and points auth at an
...
#[test]
fn test_remote_hydrated_catalog_adds_entitled_mona_subscription_routes() {
    with_temp_mona_home(|| {
        let previous_key = std::env::var_os(crate::subscription_catalog::MONA_API_KEY_ENV);
        let previous_tier = std::env::var_os(crate::subscription_catalog::MONA_TIER_ENV);
        crate::env::set_var(
            crate::subscription_catalog::MONA_API_KEY_ENV,
            "mona_test_subscription_key",
        );
        crate::env::set_var(crate::subscription_catalog::MONA_TIER_ENV, "plus");
        ...
        match previous_key {
            Some(value) => {
                crate::env::set_var(crate::subscription_catalog::MONA_API_KEY_ENV, value)
            }
            None => crate::env::remove_var(crate::subscription_catalog::MONA_API_KEY_ENV),
        }
        match previous_tier {
            Some(value) => crate::env::set_var(crate::subscription_catalog::MONA_TIER_ENV, value),
            None => crate::env::remove_var(crate::subscription_catalog::MONA_TIER_ENV),
        }
        ...
        let expected = crate::subscription_catalog::curated_models()
            ...
            .map(|model| model.id)
            .collect::<std::collections::BTreeSet<_>>();
        assert_eq!(mona_routes.len(), expected.len());
        assert_eq!(
            mona_routes
                .iter()
                .map(|route| route.model.as_str())
...
#[test]
fn test_remote_non_mona_catalog_repairs_poisoned_all_mona_routes() {
    with_temp_mona_home(|| {
        let previous_tier = std::env::var_os(crate::subscription_catalog::MONA_TIER_ENV);
        crate::env::set_var(crate::subscription_catalog::MONA_TIER_ENV, "plus");
