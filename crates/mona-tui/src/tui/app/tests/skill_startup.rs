fn with_skill_startup_home(f: impl FnOnce(&std::path::Path)) {
    ...
    crate::env::set_var("MONA_HOME", home.path().join(".jcode"));
    crate::env::remove_var("MONA_SSH_REMOTE");
    crate::env::remove_var("MONA_RELOAD_FAST_START");
    crate::config::invalidate_config_cache();
    // MONA_HOME redirects global home-relative files into its external sandbox.
...
fn skill_startup_ssh_history_replaces_warmed_empty_candidates_without_local_skills() {
    with_skill_startup_home(|home| {
        ...
        crate::env::set_var("MONA_SSH_REMOTE", "fixture-host");
        let mut app = App::new_for_remote_with_options(None, false);
        ...
        let event = serde_json::from_value(serde_json::json!({
            ...
            "server_version": mona_build_meta::version()
        }))
        .unwrap();
