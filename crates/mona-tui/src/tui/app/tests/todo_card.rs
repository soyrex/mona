...
/// RAII guard for the MONA_PIN_TODOS env override used by the band tests.
struct PinTodosEnvGuard;

impl PinTodosEnvGuard {
    fn enable() -> Self {
        crate::env::set_var("MONA_PIN_TODOS", "1");
        // mona-base's config cache throttles env re-checks (the zero
        // interval under cfg!(test) applies only when mona-base itself is
        // the crate under test), so force a reload or a sibling test's
        // MONA_PIN_TODOS state leaks into this one for up to 500ms.
        crate::config::invalidate_config_cache();
        Self
    }

    fn disable() -> Self {
        crate::env::set_var("MONA_PIN_TODOS", "0");
        crate::config::invalidate_config_cache();
        Self
    }
}
...
impl Drop for PinTodosEnvGuard {
    fn drop(&mut self) {
        crate::env::remove_var("MONA_PIN_TODOS");
