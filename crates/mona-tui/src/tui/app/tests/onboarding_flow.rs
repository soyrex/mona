    ...
    fn model_routes(&self) -> Vec<crate::provider::ModelRoute> {
        vec![
            crate::provider::ModelRoute {
                model: mona_provider_core::DEFAULT_CLAUDE_MODEL.to_string(),
            ...
            crate::provider::ModelRoute {
                model: mona_provider_core::DEFAULT_OPENAI_MODEL.to_string(),
                provider: "OpenAI".to_string(),
                api_method: "openai-api-key".to_string(),
                available: true,
                detail: String::new(),
...
fn onboarding_strongest_model_only_runs_without_explicit_defaults() {
    with_temp_mona_home(|| {
        let previous_explicit = std::env::var_os("MONA_INITIAL_PROVIDER_EXPLICIT");
        ...
        if let Some(value) = previous_explicit {
            crate::env::set_var("MONA_INITIAL_PROVIDER_EXPLICIT", value);
        } else {
            crate::env::remove_var("MONA_INITIAL_PROVIDER_EXPLICIT");
        }
    });
...
fn subscription_choice_exposes_the_canonical_pricing_page() {
    with_temp_mona_home(|| {
        ...
        assert_eq!(
            ...
            Some(format!(
                "Open Jcode pricing: {}",
                crate::subscription_catalog::MONA_PRICING_URL
            ))
        );
...
fn openrouter_key_typed_through_full_key_path_does_not_reopen_picker() {
    ...
    with_temp_mona_home(|| {
        ...
        // It is written to $MONA_HOME/config/jcode/openrouter.env and exported
...
fn startup_check_skips_user_with_established_session_history() {
    with_temp_mona_home(|| {
        ...
        let sessions_dir = crate::storage::mona_dir()
            .expect("jcode dir")
            .join("sessions");
...
fn import_continue_reaches_ready_quality_first_openai_model() {
    ...
    with_temp_mona_home(|| {
        ...
        runtime.block_on(async {
            ...
            assert_eq!(model, mona_provider_core::DEFAULT_OPENAI_MODEL);
