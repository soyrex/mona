...
//   cargo test -p mona-tui onboarding_golden -- --nocapture
...
fn onboarding_import_happy_path_images() {
    ...
    let output_dir = std::env::var_os("MONA_ONBOARDING_SCREENSHOT_DIR")
        .map(std::path::PathBuf::from)
        .expect("MONA_ONBOARDING_SCREENSHOT_DIR must be set");
