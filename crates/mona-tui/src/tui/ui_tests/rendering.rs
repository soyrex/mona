...
fn test_light_theme_adapted_frame_has_readable_contrast() {
    ...
    terminal
        .draw(|frame| {
            ...
            mona_tui_style::theme_mode::adapt_buffer(
                frame.buffer_mut(),
                mona_tui_style::ThemeMode::Light,
            );
        })
        .expect("draw");
...
///
/// The unit tests in `mona-tui-style` cover the substitution in isolation, but
...
fn test_configured_palette_recolors_a_real_rendered_frame() {
    ...
    let mut palette = mona_tui_style::Palette::default();
    palette.set(mona_tui_style::Role::User, (250, 40, 200));
    mona_tui_style::set_palette(palette);
