...
fn cached_grouped_sessions_round_trip_from_disk() {
    ...
    let _scan_limit = EnvVarGuard::set_str("MONA_SESSION_PICKER_MAX_SESSIONS", "100");
    let _include_saved = EnvVarGuard::set_str("MONA_SESSION_PICKER_INCLUDE_OLD_SAVED", "0");
...
fn load_sessions_hides_external_sessions_when_opted_out() {
    ...
    let temp = tempfile::tempdir().expect("temp dir");
    let _home = EnvVarGuard::set_path("MONA_HOME", temp.path());
    let _opt_out = EnvVarGuard::set_str("MONA_EXTERNAL_SESSIONS", "0");
...
fn load_sessions_includes_saved_sessions_beyond_scan_limit() {
    ...
    let temp = tempfile::tempdir().expect("temp dir");
    let _home = EnvVarGuard::set_path("MONA_HOME", temp.path());
    let _scan_limit = EnvVarGuard::set_str("MONA_SESSION_PICKER_MAX_SESSIONS", "50");
...
#[test]
fn session_matches_query_searches_mona_transcript_contents() {
    let _env_lock = crate::storage::lock_test_env();
    let temp = tempfile::tempdir().expect("temp dir");
    let _home = EnvVarGuard::set_path("MONA_HOME", temp.path());
...
#[test]
fn mona_search_index_keeps_late_turns_after_reaching_its_budget() {
    let _env_lock = crate::storage::lock_test_env();
    let temp = tempfile::tempdir().expect("temp dir");
    let _home = EnvVarGuard::set_path("MONA_HOME", temp.path());
    invalidate_session_list_cache();
...
fn benchmark_real_resume_loading_phases() {
    ...
    let sessions_dir = storage::mona_dir().expect("jcode dir").join("sessions");
    ...
    let mona_summary_elapsed = phase_start.elapsed();
    ...
    eprintln!(
        concat!(
            ...
            "mona_summary={}ms mona_loaded={} skipped_empty={} skipped_imported={} summary_errors={} ",
        ...
        mona_summary_elapsed.as_millis(),
        sessions.len(),
        skipped_empty,
        skipped_imported,
        summary_errors,
...
#[test]
#[ignore = "developer benchmark: scans the real MONA_HOME session directory"]
...
fn onboarding_scoped_loader_returns_only_codex_sessions() {
    ...
    let mut mona_session = Session::create_with_id(
        "session_onboarding_mona_1780000000000".to_string(),
        Some("/tmp/mona-onboard".to_string()),
        Some("Jcode Onboarding".to_string()),
    );
    mona_session.append_stored_message(crate::session::StoredMessage {
        id: "msg-1".to_string(),
        role: crate::message::Role::User,
        content: vec![crate::message::ContentBlock::Text {
    ...
    mona_session.save().expect("save jcode session");
...
mod external_session_opt_out {
    ...
    fn disk_cache_written_with_externals_is_rejected_after_opting_out() {
        let dir = PathBuf::from("/tmp/mona-test-sessions");
    ...
    fn legacy_disk_cache_without_the_field_defaults_to_externals_included() {
        let json = serde_json::json!({
            "version": super::super::SESSION_LIST_DISK_CACHE_VERSION,
            "generated_at": chrono::Utc::now(),
            "sessions_dir": "/tmp/mona-test-sessions",
            "scan_limit": 50,
    ...
    fn external_sessions_default_is_on() {
        assert!(mona_config_types::DisplayConfig::default().external_sessions);
    }
}
