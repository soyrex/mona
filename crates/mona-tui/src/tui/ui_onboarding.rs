...
/// everywhere. Override with `MONA_ROUNDED_PILLS=on|off`.
fn rounded_pill_caps_supported() -> bool {
    ...
    *SUPPORTED.get_or_init(|| {
        ...
        if let Ok(raw) = std::env::var("MONA_ROUNDED_PILLS") {
...
fn telemetry_settings_lines(
    ...
    if env_forced_off {
        lines.push(
            Line::from(Span::styled(
                "Your environment already disables telemetry (MONA_NO_TELEMETRY).",
                dim,
            ))
            .alignment(align),
        );
...
fn telemetry_header_lines(width: u16) -> Vec<Line<'static>> {
    ...
    let lines = vec![
        ...
        "Change anytime: /telemetry (or export MONA_NO_TELEMETRY=1)",
    ];
    lines
        .into_iter()
        .map(|text| {
