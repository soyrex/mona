...
fn onboarding_external_filter_picks_latest_visible_transcript() {
    ...
    let jcode = make_session("mona_one", "mona", false, SessionStatus::Closed);
...
fn onboarding_external_filter_with_no_matches_has_no_target() {
    let jcode = make_session("mona_only", "mona", false, SessionStatus::Closed);
...
//
//   cargo test -p mona-tui --lib --release -- --ignored --nocapture benchmark_resume_op
//
