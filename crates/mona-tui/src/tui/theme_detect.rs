...
//!
//! 1. `MONA_THEME=dark|light` env override (also accepts `auto`).
...
//!
//! The result is stored in `mona_tui_style::theme_mode` where the renderer
//! adapts colors for light backgrounds at frame time.

use mona_tui_style::ThemeMode;
...
pub fn prewarm_theme_mode() {
    ...
    if let Ok(handle) = std::thread::Builder::new()
        .name("mona-theme-detect".to_string())
...
pub fn init_theme_mode() -> ThemeMode {
    ...
    mona_tui_style::set_theme_mode(mode);
    init_palette();
    mode
}
...
pub fn init_palette() {
    ...
    let (palette, errors) = mona_tui_style::Palette::from_pairs(
        configured
            .iter()
...
pub fn current_theme_label() -> &'static str {
    match mona_tui_style::theme_mode() {
        ThemeMode::Dark => "dark",
...
fn resolve_configured_theme(query_terminal: bool) -> ThemeMode {
    let configured = std::env::var("MONA_THEME")
        .ok()
