...
// `mona-terminal-image` crate (shared with the `read` tool). Re-exported here
// so existing `crate::tui::image` paths keep working.
pub use mona_terminal_image::display as image;
pub mod info_widget;
...
pub use mona_tui_workspace::workspace_map;
pub use mona_tui_workspace::workspace_map_widget;
...
pub trait TuiState {
    ...
    /// from the `MONA_RUNTIME_PROVIDER` env var. The header must prefer this
    /// over its own env-based heuristic: the TUI client process often does not
    /// inherit `MONA_RUNTIME_PROVIDER` (it is set inside the agent/server
    ...
    fn active_dual_credential(
        &self,
        _provider: mona_provider_core::ActiveProvider,
...
pub struct LoginImportPrompt {
    ...
    /// Whether the environment (MONA_NO_TELEMETRY / DO_NOT_TRACK) already
...
pub(crate) fn subscribe_metadata(
    ...
    if is_ssh_remote() {
        ...
        return (
            ...
            mona_selfdev_types::client_selfdev_requested().then_some(true),
        );
    }
    ...
    resolve_subscribe_metadata(
        ...
        remote_working_dir,
        mona_selfdev_types::client_selfdev_requested(),
    )
}
...
pub(crate) fn resolve_subscribe_metadata(
    ...
    if !selfdev && let Some(dir) = client_working_dir {
        ...
        while let Some(path) = current {
            if crate::build::is_mona_repo(path) {
                selfdev = true;
                break;
            }
            current = path.parent();
...
pub fn display_messages_from_session(session: &crate::session::Session) -> Vec<DisplayMessage> {
    let mut messages = mona_tui_messages::display_messages_from_rendered_messages(
