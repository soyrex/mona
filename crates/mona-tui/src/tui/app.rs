...
fn active_runtime_provider_key() -> Option<String> {
    std::env::var("MONA_RUNTIME_PROVIDER")
        .ok()
...
pub struct App {
    ...
    remote_resolved_credential: Option<mona_provider_core::ResolvedCredential>,
...
// The cache-relevant projection lives in `mona-message-types` (re-exported
// through `crate::message`) so this local path and the server event path in
// `mona-app-core::agent::kv_cache_request_event` hash messages identically.
