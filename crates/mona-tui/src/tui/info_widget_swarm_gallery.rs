...
//! [`mona_tui_render::swarm_gallery`] module so the live TUI and the
...
use mona_tui_core::keybind::alt_chord_lower;
use mona_tui_render::swarm_gallery::{
    ...
    members
        ...
        .map(|member| GalleryMember {
            ...
            todo_items: member
                .todo_items
                .iter()
                .map(|t| mona_tui_render::swarm_gallery::GalleryTodo {
                    content: t.content.clone(),
                    status: t.status.clone(),
                    tool_intents: t
                        .tool_intents
                        .iter()
                        .map(|tool| mona_tui_render::swarm_gallery::GalleryToolIntent {
...
pub(crate) fn render_swarm_chat_card_lines(
    ...
    mona_tui_render::swarm_gallery::render_swarm_chat_cards(&gallery_members, width)
}
...
mod tests {
    use super::*;
    use mona_tui_render::swarm_gallery::members_to_tiles;
