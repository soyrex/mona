...
///
/// Respects `MONA_RUNTIME_DIR` (isolated `durable-state/swarm`), then
/// `MONA_HOME` (or the default `~/.jcode`) under `state/swarm`. Missing or
...
pub fn enrich_sessions_from_local_swarm_state(sessions: &mut [SessionInfo]) {
    let dir = if let Ok(runtime) = std::env::var("MONA_RUNTIME_DIR") {
        std::path::PathBuf::from(runtime).join("durable-state")
    } else {
        let home = std::env::var_os("MONA_HOME")
