...
///
/// Run with `cargo test -p mona-harness-api -- --nocapture capability_report`.
#[test]
...
///
/// `MONA_HOME` redirects every `user_home_path` lookup into
/// `$MONA_HOME/external/`, which is what makes an instance private. The cost
...
/// single funnel for "a file in the user's home that MONA_HOME sandboxes".
...
fn the_ledger_only_names_real_daemon_requests() {
    ...
    assert!(
        ...
        "the capability ledger names requests that do not exist in \
         mona-protocol's Request enum: {unknown:?}. These are usually local \
...
fn protocol_request_variants() -> Option<BTreeSet<String>> {
    let path =
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../mona-protocol/src/wire.rs");
