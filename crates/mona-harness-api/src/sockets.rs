...
//!
//! The rules match `mona-storage::runtime_dir` so the API socket always lands
...
/// Path of the versioned harness API socket. `MONA_API_SOCKET` overrides it.
pub fn api_socket_path() -> PathBuf {
    if let Ok(custom) = std::env::var("MONA_API_SOCKET") {
        return PathBuf::from(custom);
    }
    runtime_dir().join("mona-api.sock")
}

/// Path of the internal daemon socket the bridge translates onto.
/// `MONA_SOCKET` overrides it.
...
mod tests {
    ...
    fn the_api_socket_sits_beside_the_daemon_socket() {
        ...
        let api = runtime_dir().join("mona-api.sock");
    ...
    fn socket_names_are_stable() {
        assert_eq!(
            runtime_dir().join("mona-api.sock").file_name().unwrap(),
            "mona-api.sock"
        );
    }
