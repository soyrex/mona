//! Session process-ownership markers under the historical `~/.mona/active_pids` name.
//!
...
//! lives in the storage crate because it only needs [`mona_dir`] and is a
...
pub fn streaming_pids_dir() -> Option<std::path::PathBuf> {
    mona_dir().ok().map(|d| d.join("streaming_pids"))
}
...
pub fn internal_pids_dir() -> Option<std::path::PathBuf> {
    mona_dir().ok().map(|d| d.join("internal_pids"))
}
...
/// List session IDs with ownership markers in `~/.mona/active_pids`.
...
mod tests {
    ...
    /// Serialize tests that mutate `MONA_HOME`.
