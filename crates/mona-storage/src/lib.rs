...
///
/// Can be overridden with `$MONA_RUNTIME_DIR`.
pub fn runtime_dir() -> PathBuf {
    if let Ok(dir) = std::env::var("MONA_RUNTIME_DIR") {
...
fn fallback_runtime_dir() -> PathBuf {
    std::env::temp_dir().join(format!("mona-{}", runtime_user_discriminator()))
}
...
fn ensure_private_runtime_dir(path: &Path) {
    ...
    #[cfg(unix)]
    {
        let _ = mona_core::fs::set_directory_permissions_owner_only(path);
    }
}

pub fn mona_dir() -> Result<PathBuf> {
    if let Ok(path) = std::env::var("MONA_HOME") {
...
/// Whether `MONA_HOME` redirects this process away from the user's real
/// `~/.jcode` directory.
///
/// Sandboxed runs must not consult machine-global resources, such as the macOS
/// Keychain, that cannot be redirected beneath `MONA_HOME`.
pub fn running_with_sandboxed_home() -> bool {
    let Some(configured) = std::env::var_os("MONA_HOME").map(PathBuf::from) else {
        return false;
    };
...
/// to `~/.mona/state` (respecting `MONA_HOME`).
///
/// When `MONA_RUNTIME_DIR` is set (tests and sandboxed temp servers), it
...
/// `~/.config/jcode` on Linux). When `MONA_HOME` is set, sandbox this under
/// `$MONA_HOME/config/jcode` so self-dev/tests do not leak into the user's
/// real config directory.
pub fn app_config_dir() -> Result<PathBuf> {
    if let Ok(path) = std::env::var("MONA_HOME") {
        return Ok(PathBuf::from(path).join("config").join("mona"));
    }

    let config_dir =
        dirs::config_dir().ok_or_else(|| anyhow::anyhow!("No config directory found"))?;
    Ok(config_dir.join("mona"))
}

/// Resolve a path under the user's home directory, but sandbox it under
/// `$MONA_HOME/external/` when `MONA_HOME` is set.
///
...
pub fn harden_user_config_permissions() {
    #[cfg(windows)]
    {
        if let Some(config_dir) = dirs::config_dir() {
            let mona_config_dir = config_dir.join("mona");
            if mona_config_dir.exists() {
                schedule_windows_path_hardening(&mona_config_dir, true);
            }
        }

        if let Ok(mona_home) = mona_dir()
            && mona_home.exists()
        {
            schedule_windows_path_hardening(&mona_home, true);
        }
        return;
    }
    ...
    #[cfg(not(windows))]
    {
        if let Some(config_dir) = dirs::config_dir() {
            let mona_config_dir = config_dir.join("mona");
            if mona_config_dir.exists() {
                let _ = mona_core::fs::set_directory_permissions_owner_only(&mona_config_dir);
            }
        }
        ...
        if let Ok(mona_home) = mona_dir()
            && mona_home.exists()
        {
            let _ = mona_core::fs::set_directory_permissions_owner_only(&mona_home);
        }
    }
}
...
pub fn harden_secret_file_permissions(path: &Path) {
    ...
    #[cfg(not(windows))]
    {
        if let Some(parent) = path.parent() {
            let _ = mona_core::fs::set_directory_permissions_owner_only(parent);
        }
        if path.exists() {
            let _ = mona_core::fs::set_permissions_owner_only(path);
        }
    }
}
...
fn schedule_windows_path_hardening(path: &Path, directory: bool) {
    ...
    if std::thread::Builder::new()
        .name("mona-windows-acl-harden".to_string())
        .spawn(|| {
...
fn run_windows_hardening_worker() {
    loop {
        ...
        for path in &directories {
            let succeeded = mona_core::fs::set_directory_permissions_owner_only(path).is_ok();
            directory_results.push((path.clone(), succeeded));
        }
        let mut file_results = Vec::with_capacity(files.len());
        for path in &files {
            let succeeded =
                !path.exists() || mona_core::fs::set_permissions_owner_only(path).is_ok();
...
pub fn ensure_dir(path: &Path) -> Result<()> {
    if !path.exists() {
        ...
        mona_core::fs::set_directory_permissions_owner_only(path)?;
    }
    Ok(())
}
...
fn write_bytes_inner(path: &Path, bytes: &[u8], durable: bool, secret: bool) -> Result<()> {
    if let Some(parent) = path.parent() {
        ...
        if secret {
            ...
            mona_core::fs::set_directory_permissions_owner_only(parent)?;
        }
    }
    ...
    let result = (|| -> Result<()> {
        ...
        if secret {
            mona_core::fs::set_permissions_owner_only(&tmp_path)?;
        }
        ...
        if path.exists() {
            ...
            if secret && bak_path.exists() {
                mona_core::fs::set_permissions_owner_only(&bak_path)?;
            }
        }
