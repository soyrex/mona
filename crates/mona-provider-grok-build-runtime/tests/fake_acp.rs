...
use mona_provider_core::Provider;
use mona_provider_grok_build_runtime::{GrokBuildProcess, GrokBuildProvider};
...
fn fake_process(log: &Path) -> GrokBuildProcess {
    ...
    env.insert(
        "MONA_FAKE_GROK_ACP_LOG".to_string(),
        log.display().to_string(),
    );
    GrokBuildProcess {
        command: env!("CARGO_BIN_EXE_mona-fake-grok-acp").into(),
        args: Vec::new(),
        env,
    }
}
...
async fn provider_surfaces_payment_failure_written_to_subprocess_stderr() {
    ...
    process
        .env
        .insert("MONA_FAKE_GROK_ACP_PAYMENT_REQUIRED".into(), "1".into());
...
async fn dropping_stream_cancels_prompt_and_terminates_subprocess() {
    ...
    process
        .env
        .insert("MONA_FAKE_GROK_ACP_HANG".to_string(), "1".to_string());
    let provider = GrokBuildProvider::with_process(process);
