...
fn append_log(value: &Value) {
    let path = std::env::var("MONA_FAKE_GROK_ACP_LOG").expect("fake log path");
...
fn main() {
    ...
    for line in BufReader::new(stdin.lock()).lines() {
        ...
        match method {
            ...
            "session/prompt" => {
                if std::env::var_os("MONA_FAKE_GROK_ACP_HANG").is_some() {
                    continue;
                }
                if std::env::var_os("MONA_FAKE_GROK_ACP_PAYMENT_REQUIRED").is_some() {
                    eprintln!(
