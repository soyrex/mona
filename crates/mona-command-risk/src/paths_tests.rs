...
fn ctx() -> RiskContext {
    RiskContext {
        scratch_dir: Some(PathBuf::from("/home/u/.mona/scratch")),
...
fn known_variable_logs_are_bounded_but_unknown_expansions_still_confirm() {
    let ctx = ctx();
    for raw in [
        "$HOME/.mona/scratch/tests.log",
        "${HOME}/proj/tests.log",
        "$MONA_SCRATCH_DIR/tests.log",
        "${MONA_SCRATCH_DIR}/backup.patch",
    ] {
    ...
    for raw in [
        ...
        "$HOME/proj/$UNKNOWN/../file*",
        "$MONA_SCRATCH_DIR_OTHER/file",
        "${HOME:-/tmp}/file",
        "$UNKNOWN/../file",
        "$MONA_SCRATCH_DIR/$UNKNOWN/../file",
        "$MONA_SCRATCH_DIR/$(pwd)/file",
    ] {
        let path = expand(raw, &ctx);
        assert_eq!(
...
fn known_variables_do_not_bypass_protected_paths() {
    ...
    let raw = "$MONA_SCRATCH_DIR/passwd";
    assert_eq!(
