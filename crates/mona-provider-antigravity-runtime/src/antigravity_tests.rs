...
fn available_models_display_seeds_from_persisted_catalog() {
    ...
    let path = mona_base::provider::antigravity::persisted_catalog_path().expect("catalog path");
...
fn antigravity_gemini_uses_the_same_dialect_as_the_native_gemini_provider() {
    ...
    assert_eq!(
        out,
        mona_provider_gemini::gemini_compatible_schema(&schema),
...
fn pseudo_tool_call_turn_only_matches_the_recovery_marker_as_a_call() {
    ...
    assert!(mona_provider_antigravity::is_pseudo_tool_call_turn(
