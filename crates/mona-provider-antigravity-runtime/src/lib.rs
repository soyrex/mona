...
//! Gemini/Claude/gpt-oss upstreams), moved out of `mona-base` so provider
...
//! [`AntigravityProvider`] with `mona_base::provider::external` at startup.

use anyhow::{Context, Result};
use async_trait::async_trait;
use mona_base::auth::antigravity as antigravity_auth;
use mona_message_types::{ConnectionPhase, Message, StreamEvent, ToolDefinition};
use mona_provider_antigravity::{
...
impl AntigravityProvider {
    fn load_persisted_catalog() -> Option<PersistedCatalog> {
        mona_base::provider::antigravity::load_persisted_catalog()
    }

    fn persist_catalog(snapshot: &CatalogSnapshot) {
        mona_base::provider::antigravity::persist_catalog(snapshot);
    }
    ...
    pub fn new() -> Self {
        let model =
            std::env::var("MONA_ANTIGRAVITY_MODEL").unwrap_or_else(|_| DEFAULT_MODEL.into());
    ...
    async fn fetch_available_models(&self) -> Result<CatalogSnapshot> {
        mona_base::provider::antigravity::fetch_catalog_snapshot(&self.client).await
    }
    ...
    /// #446). `mona-schema-dialect` instead parses the construct out of the
    ...
    async fn retry_after_schema_rejection(
        ...
        signature_policy: mona_provider_gemini::SignaturePolicy,
    ) -> Option<Result<CodeAssistGenerateResponse>> {
        let resolved = self.resolve_model_for_request(model);
        let dialect = mona_provider_antigravity::antigravity_dialect(&resolved);
        match mona_schema_dialect::recover_from_error(error, dialect) {
            mona_schema_dialect::RecoveryAction::NotSchemaRelated => None,
            mona_schema_dialect::RecoveryAction::Unrecoverable { hint } => {
                mona_base::logging::warn(&format!("Antigravity tool-schema rejection: {hint}"));
                None
            }
            mona_schema_dialect::RecoveryAction::RetryWithoutConstruct { description } => {
                mona_base::logging::warn(&format!("Antigravity {description}"));
                Some(
    ...
    async fn generate_content(
        ...
        let mut tools = mona_provider_gemini::build_tools(tools);
        ...
        let request = CodeAssistGenerateRequest {
            ...
            request: VertexGenerateContentRequest {
                contents: mona_provider_gemini::build_contents_with_signature_policy(
                    messages,
                    signature_policy,
                ),
                system_instruction: mona_provider_gemini::build_system_instruction_with_tool_guard(
                    system,
                    !tools_is_empty,
                ),
                tools,
...
impl Provider for AntigravityProvider {
    async fn complete(
        ...
        let (tx, rx) = mpsc::channel::<Result<mona_message_types::StreamEvent>>(100);
        ...
        tokio::spawn(async move {
            ...
            let mut signature_policy = mona_provider_gemini::SignaturePolicy::ReplayCarriedForward;
                ...
                .await
            {
                ...
                Err(err) => {
                    ...
                    // without it. See `mona-schema-dialect`.
                        ...
                        .await
                    {
                    ...
                    } else if !mona_provider_gemini::is_missing_thought_signature_error(
                        &err.to_string(),
                    ) {
                        ...
                        signature_policy =
                            mona_provider_gemini::SignaturePolicy::DowngradeToolCallsToText;
                        match provider
                            .generate_content(
                                &model,
                                &messages,
            ...
            if mona_provider_antigravity::is_pseudo_tool_call_turn(&response) {
            ...
            if let Some(content) = candidate.content {
                ...
                for part in content.parts {
                    ...
                    if let Some(function_call) = part.function_call {
                        ...
                        let call_id = mona_message_types::sanitize_tool_id(&raw_call_id);
                        let _ = tx
            ...
            if !produced_output {
                ...
                if abnormal {
                    ...
                    let detail = candidate
                        ...
                        .map(|msg| format!(": {}", mona_base::util::truncate_str(msg.trim(), 300)))
                        .unwrap_or_default();
                    let _ = tx
                        .send(Err(anyhow::anyhow!(
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        ...
        // resumed turn 404.
        let trimmed = mona_provider_core::strip_own_model_prefix(model, "antigravity:");
        if trimmed.is_empty() {
    ...
    fn model_routes(&self) -> Vec<mona_provider_core::ModelRoute> {
        let catalog = self.fetched_catalog();
        if !catalog.is_empty() {
            return catalog
                .into_iter()
                .map(|model| mona_provider_core::ModelRoute {
