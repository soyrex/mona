...
use mona_provider_core::anthropic_map_tool_name_for_oauth as map_tool_name_for_oauth;
use serde::Serialize;
...
/// `mona-schema-dialect` so every provider shares one implementation and one
...
fn anthropic_input_schema(schema: &Value) -> Value {
    mona_schema_dialect::normalize(schema, &mona_schema_dialect::registry::ANTHROPIC)
}
...
pub fn add_message_cache_breakpoint(messages: &mut [ApiMessage], cache_ttl_1h: bool) {
    ...
    if messages.len() < 3 {
        ...
        mona_logging::info("Conversation caching: too few messages, skipping");
        return;
    }
    ...
    if assistant_indices.is_empty() {
        mona_logging::info("Conversation caching: no assistant message found");
        return;
    }
...
mod cache_prefix_invariant_tests {
    ...
    use super::*;
    use mona_message_types::{ContentBlock, Message, Role};
