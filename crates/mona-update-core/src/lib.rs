...
mod tests {
    ...
    fn summarize_update_error_is_always_one_short_line() {
        let inputs = [
            ...
            "Checksum mismatch for mona-linux-x86_64.tar.gz: expected aaa, got bbb",
            "Failed to install /home/u/.mona/builds/versions/0.1.0/jcode: Permission denied (os error 13)",
    ...
    fn summarize_update_error_maps_known_causes() {
        ...
        assert_eq!(
            summarize_update_error("No asset found for platform: mona-linux-x86_64"),
    ...
    fn asset_name_is_supported() {
        assert_ne!(get_asset_name(), "mona-unknown");
    }
    ...
    fn sha256sums_accepts_standard_and_binary_lines() {
        let checksums = parse_sha256sums(
            "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa  mona-linux-x86_64\n\
             bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb *mona-macos-aarch64\n",
        )
        .unwrap();
        assert_eq!(
            checksums.get("mona-linux-x86_64").map(String::as_str),
            Some("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
        );
        assert_eq!(
            checksums.get("mona-macos-aarch64").map(String::as_str),
    ...
    fn checksum_verification_accepts_matching_digest() {
        ...
        let sums = format!("{}  mona-linux-x86_64\n", digest);
        verify_asset_checksum_text(&sums, "mona-linux-x86_64", bytes).unwrap();
    }

    #[test]
    fn checksum_verification_rejects_mismatch() {
        let sums = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa  mona-linux-x86_64\n";
        let err = verify_asset_checksum_text(sums, "mona-linux-x86_64", b"hello")
            .unwrap_err()
            .to_string();
