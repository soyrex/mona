...
//!
//! Run with:
//!   cargo test -p mona-embedding --test embed_latency_probe -- --ignored --nocapture
//!
//! Requires the MiniLM model to be installed (~/.mona/models/all-MiniLM-L6-v2).
...
fn model_dir() -> Option<PathBuf> {
    ...
    let dir = home
        .join(".jcode")
        .join("models")
        .join(mona_embedding::MODEL_NAME);
...
fn embed_latency_probe() {
    ...
    let embedder = mona_embedding::Embedder::load_from_dir(&dir).expect("load model");
