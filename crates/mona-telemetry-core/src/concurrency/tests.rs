...
fn lease_subprocess_helper() {
    let Some(dir) = std::env::var_os("MONA_TEST_CONCURRENCY_DIR") else {
        return;
    };
    let dir = PathBuf::from(dir);
    let mut command = [0];
    std::io::stdin().read_exact(&mut command).unwrap();
    if std::env::var_os("MONA_TEST_REGISTRY_CRASH").is_some() {
...
impl Process {
    fn spawn(dir: &Path, registry_crash: bool) -> Self {
        ...
        command
            ...
            .env("MONA_TEST_CONCURRENCY_DIR", dir)
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit());
        if registry_crash {
            command.env("MONA_TEST_REGISTRY_CRASH", "1");
        }
