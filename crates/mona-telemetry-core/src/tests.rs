...
// flip `MONA_NO_TELEMETRY` / `DO_NOT_TRACK`, while the session tests drive the
...
impl Drop for TestEnvironment {
    fn drop(&mut self) {
        restore_env_var("MONA_HOME", self.previous_home.take());
        restore_env_var("MONA_NO_TELEMETRY", self.previous_no_telemetry.take());
...
fn global_test_lock() -> TestEnvironment {
    ...
    let previous_no_telemetry = std::env::var_os("MONA_NO_TELEMETRY");
    let previous_do_not_track = std::env::var_os("DO_NOT_TRACK");
    mona_core::env::set_var("MONA_HOME", home.path());
    mona_core::env::remove_var("MONA_NO_TELEMETRY");
    mona_core::env::remove_var("DO_NOT_TRACK");
...
fn test_opt_out_env_var() {
    let _guard = lock_test_env();
    mona_core::env::set_var("MONA_NO_TELEMETRY", "1");
    assert!(!is_enabled());
    mona_core::env::remove_var("MONA_NO_TELEMETRY");
}
...
fn test_do_not_track() {
    let _guard = lock_test_env();
    mona_core::env::set_var("DO_NOT_TRACK", "1");
    assert!(!is_enabled());
    mona_core::env::remove_var("DO_NOT_TRACK");
}
...
fn false_opt_out_values_keep_telemetry_enabled() {
    ...
    for value in ["", "0", "false", "no", "off"] {
        mona_core::env::set_var("MONA_NO_TELEMETRY", value);
        mona_core::env::set_var("DO_NOT_TRACK", value);
...
fn test_is_ci_detects_ci_env() {
    ...
    mona_core::env::set_var("CI", "true");
    assert!(
        is_ci(),
        "CI env var should mark the run as CI (gates install skip)"
    );
    mona_core::env::remove_var("CI");
    assert!(!is_ci());

    mona_core::env::set_var("MONA_CI", "1");
    assert!(is_ci(), "explicit MONA_CI=1 should mark any runtime as CI");
    mona_core::env::remove_var("MONA_CI");

    mona_core::env::set_var("CI", "true");
    mona_core::env::set_var("MONA_CI", "0");
    assert!(
        !is_ci(),
        "explicit MONA_CI=0 should override inherited provider markers"
    );
    mona_core::env::remove_var("MONA_CI");
    mona_core::env::remove_var("CI");

    mona_core::env::set_var("CI", "true");
    mona_core::env::set_var("MONA_CI", "invalid");
    assert!(
        is_ci(),
    ...
    mona_core::env::set_var("NEXTEST", "1");
    assert!(is_ci(), "nextest runs should be tagged as automation");
    mona_core::env::remove_var("NEXTEST");
    assert!(!is_ci());
}
...
fn test_onboarding_step_milestone_key_includes_provider_and_method() {
    assert_eq!(
        onboarding_step_milestone_key("auth_success", Some("mona"), Some("API key")),
        "auth_success_mona_api_key"
    );
    assert_eq!(
...
fn test_install_marker_tracks_current_telemetry_id() {
    ...
    if let Some(prev_home) = prev_home {
        mona_core::env::set_var("MONA_HOME", prev_home);
    } else {
        mona_core::env::remove_var("MONA_HOME");
    }
}
...
fn test_begin_session_closes_superseded_session() {
    let _guard = lock_telemetry_test_state();
    mona_core::env::set_var("MONA_TELEMETRY_DISABLED", "1");
    ...
    mona_core::env::remove_var("MONA_TELEMETRY_DISABLED");
}
...
fn concurrency_guard_paths_never_use_untrusted_session_ids() {
    let _guard = lock_telemetry_test_state();
    let home = storage::mona_dir().unwrap();
