...
pub use mona_usage_types::{ErrorCategory, SessionEndReason};
...
/// Its presence is equivalent to setting `MONA_NO_TELEMETRY=1`, but persists
...
fn opt_out_marker_path() -> Option<std::path::PathBuf> {
    storage::mona_dir().ok().map(|d| d.join("no_telemetry"))
}
...
pub fn opt_out_forced_by_env() -> bool {
    ...
    is_truthy("MONA_NO_TELEMETRY") || is_truthy("DO_NOT_TRACK")
}
...
pub fn content_sharing_enabled() -> bool {
    ...
    if std::env::var("MONA_NO_TELEMETRY").is_ok() || std::env::var("DO_NOT_TRACK").is_ok() {
        return false;
    }
...
fn detect_project_profile() -> ProjectProfile {
    ...
    profile.repo_present = root.join(".git").exists() || is_mona_repo_dir(root);
...
fn post_payload(payload: serde_json::Value, timeout: Duration) -> bool {
    ...
    let client = TELEMETRY_HTTP_CLIENT.get_or_init(|| {
        reqwest::blocking::Client::builder()
            .user_agent(mona_provider_core::MONA_USER_AGENT)
            .build()
...
pub fn record_auth_started(provider: &str, method: &str) {
    mona_logging::auth_event("auth_started", provider, &[("method", method)]);
...
pub fn record_auth_cancelled(provider: &str, method: &str) {
    mona_logging::auth_event("auth_cancelled", provider, &[("method", method)]);
...
pub fn record_auth_surface_blocked(provider: &str, method: &str) {
    mona_logging::auth_event("auth_surface_blocked", provider, &[("method", method)]);
...
pub fn record_auth_success(provider: &str, method: &str) {
    mona_logging::auth_event("auth_success", provider, &[("method", method)]);
    if !is_enabled() {
        return;
    }
    let Some(id) = get_or_create_id() else {
...
fn show_first_run_notice() {
    ...
    let (dim, reset) = if mona_core::console::stderr_supports_ansi() {
        ("\x1b[90m", "\x1b[0m")
    } else {
        ("", "")
    };
    ...
    eprintln!("  To opt out: export MONA_NO_TELEMETRY=1");
