...
fn session_creation_does_not_wait_for_unreachable_telemetry() {
    const CHILD: &str = "MONA_TEST_SESSION_LATENCY_CHILD";
    if std::env::var_os(CHILD).is_some() {
        mona_telemetry_core::begin_session("test-provider", "test-model");
        mona_telemetry_core::record_turn();
