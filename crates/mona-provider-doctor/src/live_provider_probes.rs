...
use mona_base::message::{ContentBlock, Message, Role, StreamEvent, ToolDefinition};
use mona_base::provider::Provider;
use mona_base::provider_catalog::{OpenAiCompatibleProfile, ResolvedOpenAiCompatibleProfile};
...
/// raise it via `MONA_LIVE_SMOKE_TIMEOUT_SECS` for slow reasoning models (e.g.
...
fn smoke_timeout(default_secs: u64) -> std::time::Duration {
    let secs = std::env::var("MONA_LIVE_SMOKE_TIMEOUT_SECS")
        .ok()
...
fn apply_provider_auth(
    ...
    mona_base::provider_catalog::apply_openai_compatible_catalog_auth(
        request,
        &resolved.api_base,
        api_key,
    )
...
pub async fn fetch_live_openai_compatible_models(
    ...
    let request = mona_base::provider::shared_http_client().get(&url);
...
pub async fn run_live_openai_compatible_smoke(
    ...
    api_key: &str,
    model: &str,
) -> anyhow::Result<mona_base::live_tests::LiveVerificationStage> {
    ...
    let request = mona_base::provider::shared_http_client()
        .post(&url)
        .json(&body);
    let request = apply_provider_auth(request, &resolved, api_key);
    ...
    let mut stage = mona_base::live_tests::LiveVerificationStage::passed(
...
mod tests {
    use super::*;
    use mona_base::provider_catalog::resolve_openai_compatible_profile;
    use mona_provider_metadata::{
...
pub async fn run_live_openai_compatible_tool_smoke(
    ...
    let tool_calls = parsed
        ...
        resolved.display_name,
        mona_base::util::truncate_str(text.trim(), 1200)
    );
    ...
    let parsed_arguments = mona_base::message::ToolCall::parse_streamed_input_to_object(arguments);
    ensure!(
...
pub async fn run_live_claude_native_smoke(
    ...
    .await?;
        ...
        mona_base::util::truncate_str(outcome.text.trim(), 200),
        outcome.diagnostics()
    );
...
/// `tool_execution_loop`, `tool_result_followup`, and `real_mona_tool_smoke`
...
pub async fn run_live_claude_native_tool_smoke(
    ...
    .await?;
        ...
        first.stop_reason,
        mona_base::util::truncate_str(first.text.trim(), 200)
    );
    ...
    let parsed_arguments = mona_base::message::ToolCall::parse_streamed_input_to_object(
    ...
    .await?;
        ...
        mona_base::util::truncate_str(second.text.trim(), 200)
    );
...
/// `real_mona_tool_smoke` checkpoints.
...
pub async fn run_live_native_provider_reasoning_smoke(
    ...
    let mut stage = mona_base::live_tests::LiveVerificationStage::passed(
        mona_base::live_tests::checkpoints::REASONING_CAPABILITY,
    )
    .with_duration_ms(started.elapsed().as_millis() as u64)
    .with_evidence("model", serde_json::json!(model))
...
fn parse_tool_arguments(input_json: &str) -> serde_json::Value {
    mona_base::message::ToolCall::parse_streamed_input_to_object(if input_json.trim().is_empty() {
        "{}"
    } else {
        input_json.trim()
    })
