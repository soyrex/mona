...
use mona_base::protocol::{AuthChanged, CatalogNamespace, RuntimeProviderKey};
...
pub enum DoctorTier {
    /// No key, no network, no spend. Validates mona-side wiring only.
    Offline,
...
const FULL_PIPELINE_LABELS: &[(&str, &str)] = &[
    ...
    (checkpoints::REAL_MONA_TOOL_SMOKE, "Real Jcode tool smoke"),
...
fn tool_stage_detail(stage: &mona_base::live_tests::LiveVerificationStage) -> String {
...
/// doctor report.
fn reasoning_stage_detail(stage: &mona_base::live_tests::LiveVerificationStage) -> String {
    match stage
        .evidence
...
const API_DEPENDENT_CHECKPOINTS: &[&str] = &[
    ...
    checkpoints::REAL_MONA_TOOL_SMOKE,
    checkpoints::REASONING_CAPABILITY,
];
...
///
/// The predicate itself lives in `mona_base::auth::doctor` so base-internal
...
pub use mona_base::auth::doctor::native_doctor_supports_provider;
...
pub async fn run_claude_native_e2e(
    ...
    use mona_base::provider::Provider;
    use mona_provider_anthropic_runtime::AnthropicProvider;

    let normalized = mona_base::auth::lifecycle::normalized_auth_provider_id(Some(provider_id))
        .unwrap_or("claude");
    let provider_label = mona_base::auth::lifecycle::provider_display_label(Some(normalized))
    ...
    // `MONA_RUNTIME_PROVIDER=claude-api` would silently test the API-key path
    ...
    let catalog_models: Vec<String> = if tier.requires_api_key() {
        match provider_runtime.fetch_live_model_ids_for_doctor().await {
            ...
            Ok(_) => {
                ...
                let fallback = mona_base::provider::known_anthropic_model_ids();
                checks.push(DoctorCheck::passed(
                    checkpoints::MODEL_CATALOG_LIVE_ENDPOINT,
                    label_for(checkpoints::MODEL_CATALOG_LIVE_ENDPOINT),
                    format!(
...
pub async fn run_antigravity_native_e2e(
    ...
    use mona_base::provider::Provider;
    use mona_provider_antigravity_runtime::AntigravityProvider;
    ...
    let _ = mona_base::auth::lifecycle::normalized_auth_provider_id(Some(provider_id));
    let provider_label = mona_base::auth::lifecycle::provider_display_label(Some("antigravity"))
...
impl NativeProviderKind {
    ...
    fn spec(self) -> NativeProviderSpec {
        match self {
            ...
            Self::Jcode => NativeProviderSpec {
                ...
                contract: WiringContract {
                    api_method: mona_base::subscription_catalog::MONA_ROUTE_API_METHOD
                        .to_string(),
                    route_provider: mona_base::subscription_catalog::MONA_PROVIDER_DISPLAY_NAME
                        .to_string(),
                    expected_runtime: "mona",
                    expected_namespace: None,
                    switch_prefix: String::new(),
                },
                auth_source: "Jcode subscription API key (MONA_API_KEY)",
                auth_env_key: Some("MONA_API_KEY"),
    ...
    fn build_runtime(self) -> anyhow::Result<std::sync::Arc<dyn mona_base::provider::Provider>> {
        use anyhow::Context as _;
        use mona_base::provider::Provider;
        let runtime: std::sync::Arc<dyn Provider> = match self {
            Self::OpenAi => {
                let credentials =
                    mona_base::auth::codex::load_credentials().unwrap_or_else(|_| {
                        mona_base::auth::codex::CodexCredentials {
                ...
                std::sync::Arc::new(mona_provider_openai_runtime::OpenAIProvider::new(
                    credentials,
                ))
            }
            Self::Gemini => {
            ...
            Self::Copilot => {
                ...
                mona_base::env::set_var("MONA_COPILOT_PREFETCH_STARTUP_GRACE_MS", "0");
                let runtime = match mona_provider_copilot_runtime::CopilotApiProvider::new() {
                    Ok(runtime) => runtime,
                    Err(_) => mona_provider_copilot_runtime::CopilotApiProvider::new_with_token(
                        String::new(),
                    ),
                };
            ...
            Self::Bedrock => {
                std::sync::Arc::new(mona_base::provider::bedrock::BedrockProvider::new())
            }
            Self::Jcode => std::sync::Arc::new(mona_base::provider::jcode::JcodeProvider::new()),
            Self::Azure => {
                ...
                mona_base::auth::azure::apply_runtime_env()
                    .context("apply Azure OpenAI runtime env")?;
                let runtime = mona_provider_openrouter_runtime::OpenRouterProvider::new()
                ...
                if let Some(model) = mona_base::auth::azure::load_model() {
    ...
    async fn resolve_credential(self) -> anyhow::Result<String> {
        ...
        match self {
            Self::OpenAi => {
                let credentials = mona_base::auth::codex::load_credentials()
            ...
            Self::Gemini => {
                let tokens = mona_base::auth::gemini::load_or_refresh_tokens()
                    .await
            ...
            Self::Cursor => {
                let key = mona_base::auth::cursor::load_api_key()
            ...
            Self::Copilot => {
                let token = mona_base::auth::copilot::load_github_token()
            ...
            Self::Bedrock => {
                if !mona_base::provider::bedrock::BedrockProvider::has_credentials() {
                    anyhow::bail!(
            ...
            Self::Jcode => {
                if !mona_base::subscription_catalog::has_credentials() {
                    anyhow::bail!(
                        "no Jcode subscription credential found (set MONA_API_KEY or run \
            ...
            Self::Azure => {
                ...
                Ok(format!(
                    ...
                    mona_base::auth::azure::method_detail()
                ))
            }
        }
    }
...
struct NativeProviderSpec {
    ...
    label: &'static str,
    /// mona-side routing/activation contract for the wiring checkpoints.
    contract: WiringContract,
...
async fn run_generic_native_api_checks(
    provider: &dyn mona_base::provider::Provider,
    selected: &str,
    label: &str,
    checks: &mut Vec<DoctorCheck>,
    spend: &mut DoctorSpend,
...
/// The mona-side wiring a given compat profile is expected to activate.
///
...
fn wiring_contract(profile: OpenAiCompatibleProfile) -> WiringContract {
    match mona_base::auth::lifecycle::normalized_auth_provider_id(Some(profile.id)) {
...
fn run_wiring_checks_for_contract(
    ...
    let auth = AuthChanged {
        provider: mona_base::protocol::AuthProviderId::new(auth_provider_id),
...
mod tests {
    ...
    /// `native_doctor_supports_provider` lives in `mona_base::auth::doctor`
    ...
    #[test]
    fn native_mona_contract_uses_managed_subscription_identity() {
        let contract = NativeProviderKind::Jcode.spec().contract;
        assert_eq!(
            contract.api_method,
            mona_base::subscription_catalog::MONA_ROUTE_API_METHOD
        );
        assert_eq!(
            contract.route_provider,
            mona_base::subscription_catalog::MONA_PROVIDER_DISPLAY_NAME
        );
        assert_eq!(contract.expected_runtime, "mona");
        assert!(contract.expected_namespace.is_none());
