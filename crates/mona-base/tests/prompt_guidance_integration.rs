...
use mona_base::prompt::{build_system_prompt_full, build_system_prompt_split};
use std::path::Path;
...
fn public_prompt_builders_deduplicate_home_and_aliases_but_preserve_distinct_files() {
    ...
    for (case, cwd) in [("home", &home), ("alias", &alias), ("distinct", &distinct)] {
        ...
        let output = Command::new(std::env::current_exe().unwrap())
            ...
            .env_remove("MONA_HOME")
            .env("MONA_PROMPT_ACCEPTANCE_CASE", case)
            .output()
            .unwrap();
        assert!(
...
fn prompt_guidance_child() {
    let case = std::env::var("MONA_PROMPT_ACCEPTANCE_CASE").unwrap();
