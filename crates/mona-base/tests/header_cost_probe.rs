...
//
//   cargo test -p mona-base --test header_cost_probe -- --nocapture --ignored
//
...
fn measure_auth_status_probe_cost() {
    ...
    let _ = mona_base::auth::AuthStatus::check_fast();
...
///
/// This lives in `mona-base` (its own test binary) rather than beside the
/// header tests because `invalidate_cached_status()` clears a process-global
/// cache that sibling tests in the `mona-tui` binary share.
#[test]
fn invalidate_cached_status_bumps_the_auth_generation() {
    let before = mona_base::auth::auth_status_generation();
    mona_base::auth::AuthStatus::invalidate_cached_status();
    let after = mona_base::auth::auth_status_generation();
