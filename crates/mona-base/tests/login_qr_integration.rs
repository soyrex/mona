...
fn openai_browserless_login_prints_qr() {
    if std::env::var_os("MONA_QR_LOGIN_TEST_CHILD").is_some() {
        let runtime = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .build()
            .unwrap();
        let error = runtime
            .block_on(mona_base::auth::oauth::login_openai(true))
            .unwrap_err();
    ...
    let output = Command::new(std::env::current_exe().unwrap())
        ...
        .env("MONA_QR_LOGIN_TEST_CHILD", "1")
        .env("MONA_HOME", home.path())
        .env_remove("MONA_SHOW_LOGIN_QR")
        .env_remove("MONA_LOGIN_QR")
        .stdin(Stdio::null())
        .output()
        .unwrap();
