...
use mona_tool_core::{Tool, ToolContext, ToolExecutionMode};
...
const CHILD_MARKER: &str = "MONA_MCP_STDIO_TEST_CHILD";
...
fn real_stdio_collision_aliases_preserve_original_targets() {
    if std::env::var_os(CHILD_MARKER).is_none() {
        ...
        // inherited. In particular, no provider credentials or MONA_* config.
        ...
        let status = child
            .env(CHILD_MARKER, "1")
            .env("MONA_HOME", sandbox.path().join("mona"))
            .env("MONA_RUNTIME_DIR", sandbox.path().join("runtime"))
