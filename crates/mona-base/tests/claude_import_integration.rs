...
//!
//! These drive the real `mona_base::import` pipeline against synthetic
//! `.jsonl` fixtures written into a sandboxed `MONA_HOME`, so they never touch
...
//! environment (`MONA_HOME`).

use mona_base::import::{
    import_session, import_session_from_file, list_claude_code_sessions,
    list_claude_code_sessions_lazy,
};
use mona_base::message::ContentBlock;
use mona_base::session::Session;
use mona_import_core::imported_claude_code_session_id;
use std::path::{Path, PathBuf};
use std::sync::Mutex;

/// Serializes tests that mutate `MONA_HOME`. This test binary has its own
...
impl Drop for EnvVarGuard {
    fn drop(&mut self) {
        if let Some(prev) = self.prev.take() {
            mona_base::env::set_var(self.key, prev);
        } else {
...
/// Create the sandboxed Claude project dir under `$MONA_HOME/external/...`.
...
fn import_array_tool_result_with_image_uses_placeholder_not_base64() {
    ...
    let saved = std::fs::read_to_string(mona_session_file(temp.path(), "img-session")).unwrap();
    assert!(
        !saved.contains(&big),
...
/// With `MONA_HOME` set, `storage::mona_dir()` returns it directly, and
/// sessions live at `<MONA_HOME>/sessions/<id>.json`.
fn mona_session_file(home: &Path, claude_id: &str) -> PathBuf {
