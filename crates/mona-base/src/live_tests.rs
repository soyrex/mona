...
const LEDGER_ENV: &str = "MONA_LIVE_TEST_LEDGER";
const COVERAGE_ENV: &str = "MONA_LIVE_TEST_COVERAGE";
...
pub mod checkpoints {
    ...
    pub const REAL_MONA_TOOL_SMOKE: &str = "real_mona_tool_smoke";
...
const END_TO_END_CHECKPOINTS: &[LiveVerificationCheckpointDefinition] = &[
    ...
    LiveVerificationCheckpointDefinition {
        id: checkpoints::REAL_MONA_TOOL_SMOKE,
...
pub struct LiveVerificationBuild {
    pub mona_version: String,
    pub mona_git_hash: String,
    pub mona_git_date: String,
    pub mona_git_dirty: bool,
    pub mona_semver: String,
    pub os: String,
    pub arch: String,
    pub pid: u32,
}
...
impl LiveVerificationBuild {
    pub fn current() -> Self {
        let version = mona_build_meta::version().to_string();
        Self {
            mona_git_dirty: version.contains("dirty"),
            mona_version: version,
            mona_git_hash: mona_build_meta::git_hash().to_string(),
            mona_git_date: mona_build_meta::git_date().to_string(),
            mona_semver: mona_build_meta::semver().to_string(),
...
pub struct LiveProviderModelCoveragePair {
    ...
    #[serde(default)]
    pub latest_mona_version: String,
...
fn update_coverage(event: &LiveVerificationEvent, path: &Path) -> Result<()> {
    ...
    coverage.latest.insert(
        ...
        LiveVerificationCoverageEntry {
            ...
            mona_version: event.build.mona_version.clone(),
            mona_git_hash: event.build.mona_git_hash.clone(),
            mona_git_dirty: event.build.mona_git_dirty,
...
pub fn format_provider_test_coverage_report(
    ...
    out.push_str(&format!(
        ...
        coverage_actor_label(entry.mona_git_dirty, &entry.mona_version),
    ));
    ...
    out.push_str(&format!(
        ...
        entry.mona_version,
        entry.mona_git_hash,
        if entry.mona_git_dirty { ", dirty" } else { "" }
    ));
...
fn provider_test_coverage_checkpoint_label(checkpoint: &str) -> String {
    match checkpoint {
        ...
        checkpoints::REAL_MONA_TOOL_SMOKE => "Real jcode tool smoke".to_string(),
...
struct ProviderModelCoverageBuilder {
    ...
    latest_mona_version: String,
    latest_mona_dirty: bool,
    entries: usize,
...
impl ProviderModelCoverageBuilder {
    fn record(&mut self, entry: &LiveVerificationCoverageEntry) {
            ...
            .unwrap_or(true)
        {
            ...
            self.latest_mona_version = entry.mona_version.clone();
            self.latest_mona_dirty = entry.mona_git_dirty;
        }
    ...
    fn finish(self) -> LiveProviderModelCoveragePair {
        ...
        LiveProviderModelCoveragePair {
            ...
            latest_mona_version: self.latest_mona_version,
            latest_mona_dirty: self.latest_mona_dirty,
...
pub fn strict_live_provider_model_coverage_summary(
        ...
        .map(ProviderModelCoverageBuilder::finish)
    {
        ...
        match pair.checkpoint_status(checkpoints::REAL_MONA_TOOL_SMOKE) {
...
const STRICT_PIPELINE_STAGES: &[(&str, &str)] = &[
    ...
    (checkpoints::REAL_MONA_TOOL_SMOKE, "real tool smoke"),
];
...
fn doctor_tier_for_stage(stage_id: &str) -> &'static str {
    match stage_id {
        ...
        | checkpoints::TOOL_EXECUTION_LOOP
        | checkpoints::TOOL_RESULT_FOLLOWUP
        | checkpoints::REAL_MONA_TOOL_SMOKE => "full",
    ...
    if all_pairs.is_empty() {
        ...
        for pair in &shown_pairs {
            ...
            let tested = format!(
                ...
                coverage_actor_label(pair.latest_mona_dirty, &pair.latest_mona_version),
            );
...
mod tests {
    ...
    fn coverage_entry(
        ...
        LiveVerificationCoverageEntry {
            ...
            mona_git_hash: "test".to_string(),
            mona_git_dirty: false,
            user_ready: false,
