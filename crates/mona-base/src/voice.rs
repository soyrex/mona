...
mod tests {
    ...
    async fn configured_public_api_requires_key_and_uses_configured_base() {
        ...
        let _restore = Restore(
            ["MONA_API_KEY", "MONA_API_BASE"]
                .into_iter()
                .map(|key| (key, std::env::var(key).ok()))
                .collect(),
        );
        crate::env::remove_var("MONA_API_KEY");
        crate::env::set_var("MONA_API_BASE", "http://127.0.0.1:1");
        assert!(!subscription_voice_available().await);
        assert_eq!(
            transcribe_wav(wav(), None).await,
            Err(VoiceError::NotConfigured)
        );
        crate::env::set_var("MONA_API_KEY", "public-api-test-key");
        ...
        crate::env::set_var("MONA_API_BASE", &base);
        assert!(subscription_voice_available().await);
        assert!(
    ...
    async fn multipart_and_bearer_auth_match_backend_contract() {
        ...
        assert!(text.contains("authorization: Bearer mona-secret-key\r\n"));
        ...
        assert_eq!(text.matches("mona-secret-key").count(), 1);
    }
