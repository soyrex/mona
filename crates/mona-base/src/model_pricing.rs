...
//!
//! jcode's static pricing tables (`mona_provider_core::pricing`) only cover
...
//!   - a 24h disk cache under `~/.mona/cache/models_dev_pricing.json`,
...
/// from, so changing `MONA_HOME` (tests, multi-home setups) never serves
...
pub fn models_dev_provider_id(mona_provider: &str) -> Option<&'static str> {
    let key = mona_provider
        .trim()
        .strip_prefix("openai-compatible:")
        .unwrap_or_else(|| mona_provider.trim());
    Some(match key {
...
/// Strip mona-local suffixes/prefixes a model id may carry before catalog
/// lookup (`[1m]` long-context alias, `provider/` prefixes for OpenRouter ids).
fn normalize_model_id(model: &str) -> &str {
    let model = mona_provider_core::model_id::strip_long_context_suffix(model).trim();
    model
        .rsplit_once('@')
...
pub fn lookup(mona_provider: &str, model: &str) -> Option<ModelCost> {
    let provider_id = models_dev_provider_id(mona_provider)?;
...
pub fn schedule_refresh() {
    ...
    // MONA_FORCE_PRICING_REFRESH=1 re-enables the fetch for manual e2e checks
    // (e.g. `cargo run --example pricing_e2e_check`, which builds with
    // test-support unified in).
    let forced = std::env::var_os("MONA_FORCE_PRICING_REFRESH").is_some();
    if !forced
        && (cfg!(any(test, feature = "test-support"))
            || std::env::var_os("MONA_DISABLE_PRICING_REFRESH").is_some())
    {
        return;
    }
...
mod tests {
    ...
    #[test]
    fn provider_key_mapping_covers_mona_providers() {
