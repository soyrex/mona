...
pub fn configured_api_base() -> String {
    subscription_catalog::configured_api_base()
        .unwrap_or_else(|| subscription_catalog::DEFAULT_MONA_API_BASE.to_string())
        .trim_end_matches('/')
        .to_string()
}
...
pub async fn request_device_authorization(
    ...
    let mut payload = serde_json::json!({ "client_name": "mona-cli" });
...
mod tests {
    ...
    async fn metered_device_request_does_not_select_a_subscription_tier() {
        ...
        assert!(request.contains(r#"{"client_name":"mona-cli"}"#));
