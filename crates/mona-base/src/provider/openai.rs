...
//! downstream `mona-provider-openai-runtime` crate so provider edits do not
...
pub use mona_provider_core::CredentialMode as OpenAICredentialMode;
...
pub fn prompt_cache_retention_from_env() -> Option<String> {
    let raw = std::env::var("MONA_OPENAI_PROMPT_CACHE_RETENTION").ok()?;
...
mod cache_tests {
    ...
    fn cache_ttl_provider_routes_and_environment_override() {
        ...
        let key = "MONA_OPENAI_PROMPT_CACHE_RETENTION";
