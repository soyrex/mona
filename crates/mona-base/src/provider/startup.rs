...
impl MultiProvider {
    ...
    pub(super) fn new_with_auth_status(auth_status: auth::AuthStatus) -> Self {
        ...
        if initial_provider.is_none()
            && std::env::var_os("MONA_PROVIDER_PROFILE_ACTIVE").is_none()
        ...
        let use_claude_cli = std::env::var("MONA_USE_CLAUDE_CLI")
            .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
            .unwrap_or(false);
        if use_claude_cli {
            crate::logging::warn(
                "MONA_USE_CLAUDE_CLI is deprecated and will be removed. Direct Anthropic API transport is the default.",
            );
        }

        let claude = if has_claude_creds && use_claude_cli {
            crate::logging::info(
                "Using deprecated Claude CLI provider (forced by MONA_USE_CLAUDE_CLI=1)",
            );
        ...
        let anthropic = if has_claude_creds && !use_claude_cli {
            ...
            let active_profile_is_anthropic = std::env::var("MONA_NAMED_PROVIDER_PROFILE")
                .ok()
        ...
        let active_named_profile_is_anthropic = std::env::var("MONA_NAMED_PROVIDER_PROFILE")
            .ok()
        ...
        let openrouter = if has_openrouter_creds && !active_named_profile_is_anthropic {
            let named_profile = std::env::var("MONA_NAMED_PROVIDER_PROFILE")
                .ok()
                .or_else(|| default_named_provider_profile.clone());
