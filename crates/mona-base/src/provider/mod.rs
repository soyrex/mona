...
#[cfg(test)]
use mona_provider_core::FailoverDecision;
...
pub use mona_provider_core::attempt_tracker;
pub use mona_provider_core::cli_provider_arg_for_session_key;
pub use mona_provider_core::{
    ...
    MONA_USER_AGENT, ModelCapabilities, ModelCatalogRefreshSummary, ModelRoute,
...
pub use mona_provider_core::{ProviderFailoverPrompt, parse_failover_prompt_message};
...
pub(crate) fn cached_live_models_for_openai_compatible_profile(
    ...
    let cache_is_stale = mona_provider_openrouter::current_unix_secs()
...
fn configured_standard_openrouter_profile_routes() -> Vec<ModelRoute> {
    let Some(cache) = mona_provider_openrouter::load_disk_cache_entry_for_namespace("openrouter")
    else {
        return Vec::new();
    };
...
pub struct MultiProvider {
    ...
    /// `mona-provider-copilot-runtime` and is instantiated through
    ...
    /// `mona-provider-antigravity-runtime` and is instantiated through
    /// `external::instantiate_external_provider`.
    antigravity: RwLock<Option<Arc<dyn Provider>>>,
    /// Gemini provider (hot-swappable after login). Held as `dyn Provider`:
    /// the concrete runtime lives downstream in `mona-provider-gemini-runtime`
    ...
    /// `dyn Provider`: the concrete runtime lives downstream in
    /// `mona-provider-cursor-runtime` and is instantiated through
    /// `external::instantiate_external_provider`.
...
impl MultiProvider {
    ...
    fn routes_memo_key(&self) -> String {
        ...
        format!(
            "{}|{}|{}|{:?}|{}|{}|{}|{}",
            // Scope by home so sandboxes (tests, MONA_HOME switches) never
            // share catalogs that were built from different credential files.
            std::env::var("MONA_HOME").unwrap_or_default(),
    ...
    fn activity_source_key(&self, provider: ActiveProvider) -> String {
        match provider {
            ...
            ActiveProvider::OpenRouter => {
                ...
                let runtime = std::env::var("MONA_RUNTIME_PROVIDER").ok();
    ...
    fn set_model_on_mona_subscription(&self, model: &str) -> Result<()> {
        let model = model.trim();
        if model.is_empty() {
            anyhow::bail!("Model cannot be empty");
        }
        ...
        crate::provider::activation::ProviderActivation::mona_subscription(model).apply_env()?;
    ...
    fn set_model_on_provider_with_credential_modes(
        ...
        match provider {
            ActiveProvider::Claude => {
                let switching_from_named_anthropic = std::env::var("MONA_NAMED_PROVIDER_PROFILE")
                    .ok()
    ...
    pub(super) fn set_config_default_model(
            ...
            Self::resolve_config_provider_selection(pref, crate::config::config())
        {
            ...
            let pinned = mona_provider_core::AuthRoute::parse_explicit_credential_prefix(pref);
            let anthropic_credential_mode = pinned.and_then(|route| {
                ...
                .then(|| match route.mode {
                    mona_provider_core::AuthMode::ApiKey => {
                        anthropic::AnthropicCredentialMode::ApiKey
                    }
                    mona_provider_core::AuthMode::Oauth => {
            ...
            let openai_credential_mode = pinned.and_then(|route| {
                matches!(
                    route.provider,
                    mona_provider_core::DualAuthProvider::OpenAI
                )
                .then(|| match route.mode {
                    mona_provider_core::AuthMode::ApiKey => openai::OpenAICredentialMode::ApiKey,
                    mona_provider_core::AuthMode::Oauth => openai::OpenAICredentialMode::OAuth,
                })
            });
        ...
        let prefix = match active {
            ActiveProvider::Claude => {
                if let Some(anthropic) = self.anthropic_provider() {
                    ...
                    anthropic
                        .credential_mode()
                        .auth_route(mona_provider_core::DualAuthProvider::Anthropic)
            ...
            ActiveProvider::OpenAI => {
                if let Some(openai) = self.openai_provider() {
                    openai
                        .credential_mode()
                        .auth_route(mona_provider_core::DualAuthProvider::OpenAI)
                        .map(|route| route.model_prefix())
                        .unwrap_or("openai")
                } else {
                    "openai"
...
impl Provider for MultiProvider {
        ...
        match self.active_provider() {
            ActiveProvider::Claude => {
                ...
                if let Some(anthropic) = self.anthropic_provider() {
                    ...
                    mona_provider_core::DEFAULT_CLAUDE_MODEL.to_string()
                }
            }
            ActiveProvider::OpenAI => self
                .openai_provider()
                .map(|o| o.model())
                .unwrap_or_else(|| mona_provider_core::DEFAULT_OPENAI_MODEL.to_string()),
    ...
    fn active_resolved_credential(&self) -> Option<mona_provider_core::ResolvedCredential> {
        use mona_provider_core::ResolvedCredential;
        match self.active_provider() {
            ActiveProvider::Claude => {
    ...
    fn active_explicit_credential(&self) -> Option<mona_provider_core::ResolvedCredential> {
        use mona_provider_core::ResolvedCredential;
    ...
    fn set_model(&self, model: &str) -> Result<()> {
            ...
            explicit_model_provider_prefix(requested_model)
        {
            ...
            let pinned = mona_provider_core::AuthRoute::parse_explicit_credential_prefix(prefix);
            let openai_credential_mode = pinned.and_then(|route| {
                matches!(
                    route.provider,
                    mona_provider_core::DualAuthProvider::OpenAI
    ...
    fn set_route_selection(&self, selection: &RouteSelection) -> Result<()> {
        ...
        if selection.runtime_key == RuntimeKey::JcodeSubscription {
            return self.set_model_on_mona_subscription(&selection.model);
        }
    ...
    fn uses_mona_compaction(&self) -> bool {
        match self.active_provider() {
            ActiveProvider::Claude => {
                if self.anthropic_provider().is_some() {
                    true
...
pub fn cache_ttl_is_estimate(provider: &str) -> bool {
    mona_provider_core::AuthRoute::parse(provider)
        .is_some_and(|route| route.provider == mona_provider_core::DualAuthProvider::OpenAI)
        || matches!(
...
pub fn cache_ttl_for_provider_model(provider: &str, model: Option<&str>) -> Option<u64> {
    if let Some(route) = mona_provider_core::AuthRoute::parse(provider)
        && route.provider == mona_provider_core::DualAuthProvider::OpenAI
    {
        ...
        return (route.mode == mona_provider_core::AuthMode::ApiKey).then(|| {
