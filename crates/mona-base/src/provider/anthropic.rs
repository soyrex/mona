...
//! in the downstream `mona-provider-anthropic-runtime` crate so provider
...
pub use mona_provider_core::CredentialMode as AnthropicCredentialMode;
...
pub fn load_anthropic_api_key() -> Result<String> {
    if std::env::var("MONA_ANTHROPIC_AUTH")
        .ok()
        .is_some_and(|value| value.eq_ignore_ascii_case("none"))
    {
        return Ok(String::new());
    }
    if let Ok(env_name) = std::env::var("MONA_ANTHROPIC_API_KEY_NAME") {
        ...
        if !env_name.is_empty() {
            ...
            if let Ok(env_file) = std::env::var("MONA_ANTHROPIC_ENV_FILE")
