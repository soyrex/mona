...
impl JcodeProvider {
    fn runtime_model_spec(model: &str) -> String {
        ...
        // as `mona-subscription`, for every curated model.
    ...
    fn apply_runtime_profile() {
        let _ = crate::provider::activation::ProviderActivation::mona_subscription(
...
impl Provider for JcodeProvider {
    ...
    fn name(&self) -> &str {
        crate::subscription_catalog::MONA_PROVIDER_DISPLAY_NAME
    }
    ...
    fn uses_mona_compaction(&self) -> bool {
        self.inner.uses_mona_compaction()
    }
...
mod tests {
    ...
    #[test]
    fn mona_provider_enables_subscription_runtime_mode() {
    ...
    #[test]
    fn mona_provider_name_and_default_model_are_curated() {
        let _guard = crate::storage::lock_test_env();
        crate::subscription_catalog::clear_runtime_env();
        let runtime = tokio::runtime::Runtime::new().expect("tokio runtime");
    ...
    #[test]
    fn mona_models_are_pinned_to_the_managed_transport() {
        assert_eq!(
    ...
    #[test]
    fn mona_provider_exposes_only_explicit_subscription_routes() {
        ...
        let route_selection = mona_provider_core::RouteSelection::from_model_route(gpt_route);
        ...
        assert!(plus_routes.iter().all(|route| {
            route.provider == "Jcode Subscription"
                && route.api_method == "mona-subscription"
                && route.available
        }));
        assert_eq!(
        ...
        assert_eq!(
            ...
            mona_provider_core::RuntimeKey::JcodeSubscription
        );
        assert_eq!(route_selection.api_method, "mona-subscription");
