...
pub async fn run_configured() -> Result<DictationRun> {
    ...
    if command.is_empty() {
        anyhow::bail!(
            "Dictation is not configured. Set `[dictation].command` in `~/.mona/config.toml`."
        );
    }
...
pub fn focused_mona_session() -> Result<Option<String>> {
...
fn last_focused_session_path() -> Result<std::path::PathBuf> {
    Ok(crate::storage::mona_dir()?.join("last_focused_client_session"))
}
