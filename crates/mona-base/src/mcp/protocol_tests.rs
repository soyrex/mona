...
fn load_for_dir_expands_the_winning_merged_definition() {
    let _guard = crate::storage::lock_test_env();
    let previous_home = std::env::var_os("MONA_HOME");
    let previous_value = std::env::var_os("MONA_MCP_EXPANSION_TEST_VALUE");
    let home = tempfile::tempdir().expect("home tempdir");
    let project = tempfile::tempdir().expect("project tempdir");
    crate::env::set_var("MONA_HOME", home.path());
    crate::env::set_var("MONA_MCP_EXPANSION_TEST_VALUE", "expanded-value");
    ...
    std::fs::write(
        project.path().join(".mcp.json"),
        r#"{"mcpServers":{"same-name":{"command":"project-bin","args":["${MONA_MCP_EXPANSION_TEST_VALUE}"]}}}"#,
    )
    .unwrap();
    ...
    match previous_value {
        Some(value) => crate::env::set_var("MONA_MCP_EXPANSION_TEST_VALUE", value),
        None => crate::env::remove_var("MONA_MCP_EXPANSION_TEST_VALUE"),
    }
...
fn test_load_project_locals_merge_order() {
    ...
    std::fs::write(
        ...
        r#"{"servers":{"shared-name":{"command":"mona-bin"},"mona-only":{"command":"a"}}}"#,
    )
    .unwrap();
    std::fs::write(
        project.join(".mcp.json"),
    ...
    assert!(config.servers.contains_key("mona-only"));
...
#[test]
fn claude_json_http_entry_does_not_displace_mona_stdio_server() {
    // The exact configuration from issue #653: `github` is stdio in
    // ~/.mona/mcp.json and http in ~/.claude.json. The http entry used to win
    ...
    // `user_home_path()` maps external configs under MONA_HOME to an
...
#[test]
fn claude_only_config_never_creates_a_mona_snapshot() {
    let _guard = crate::storage::lock_test_env();
    let previous_home = std::env::var_os("MONA_HOME");
    let home = tempfile::tempdir().expect("home tempdir");
    crate::env::set_var("MONA_HOME", home.path());
...
#[test]
fn disabling_claude_mcp_skips_both_live_sources_but_preserves_mona_sources() {
    let _guard = crate::storage::lock_test_env();
    let previous_home = std::env::var_os("MONA_HOME");
    let previous_disable = std::env::var_os("MONA_DISABLE_CLAUDE_MCP");
    let home = tempfile::tempdir().expect("home tempdir");
    let project = tempfile::tempdir().expect("project tempdir");
    crate::env::set_var("MONA_HOME", home.path());
    crate::env::set_var("MONA_DISABLE_CLAUDE_MCP", "1");

    std::fs::write(
        home.path().join("mcp.json"),
        r#"{"mcpServers":{"mona-global":{"command":"mona-global"}}}"#,
    )
    ...
    std::fs::write(
        ...
        r#"{"mcpServers":{"mona-project":{"command":"mona-project"}}}"#,
    )
    ...
    let result = std::panic::catch_unwind(|| {
        let config = McpConfig::load_for_dir(Some(project.path()));
        assert!(config.servers.contains_key("mona-global"));
        assert!(config.servers.contains_key("mona-project"));
    ...
    if let Some(previous_disable) = previous_disable {
        crate::env::set_var("MONA_DISABLE_CLAUDE_MCP", previous_disable);
    } else {
        crate::env::remove_var("MONA_DISABLE_CLAUDE_MCP");
    }
...
fn mcp_source_logs_explain_provenance_without_config_values() {
    ...
    let imported =
        McpConfig::codex_import_log_message(1, 2, std::path::Path::new("/sandbox/.mona/mcp.json"));
    assert!(imported.contains("One-time imported 1 server(s) from Codex CLI"));
    assert!(imported.contains("/sandbox/.mona/mcp.json"));
