...
impl McpConfig {
    ...
    /// This only runs while ~/.mona/mcp.json does not exist.
    fn import_from_codex_once() {
        let mona_mcp = match crate::storage::mona_dir() {
        ...
        let environment_value_count = imported
            ...
            .sum();
        if let Err(e) = imported.save_to_file(&mona_mcp) {
    ...
    pub fn load_for_dir(project_dir: Option<&std::path::Path>) -> Self {
        ...
        let mut merged = Self::default();
        let claude_mcp_enabled = std::env::var_os("MONA_DISABLE_CLAUDE_MCP").is_none();

        // Load jcode's own global config (~/.mona/mcp.json)
        if let Ok(mona_dir) = crate::storage::mona_dir() {
            let mona_mcp = mona_dir.join("mcp.json");
            if mona_mcp.exists() {
                if let Ok(config) = Self::load_from_file(&mona_mcp) {
    ...
    /// working stdio server from `~/.mona/mcp.json` and then be dropped by the
