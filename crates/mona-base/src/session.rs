...
///
/// Wraps the on-disk streaming marker from `mona-storage` (cleared on every
...
fn default_is_test_session() -> bool {
    env_flag_enabled("MONA_TEST_SESSION")
}
...
pub fn derive_session_provider_key(provider_name: &str) -> Option<String> {
    ...
    if normalized_name == "mona" {
        return Some("mona".to_string());
    }
    ...
    if let Ok(active) = std::env::var("MONA_ACTIVE_PROVIDER") {
...
request in this new forked session, using the inherited conversation only as context.\n</system-reminder>",
    ...
    pub fn strip_oversized_images(&mut self, target_total_chars: usize) -> usize {
        ...
        let stripped = mona_compaction_core::strip_large_images_in_contents(
            &mut contents,
            target_total_chars,
        );
        if stripped > 0 {
