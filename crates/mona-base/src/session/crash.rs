...
///
/// Uses a fast O(n) scan of `~/.mona/active_pids/` (typically 0-5 files)
...
fn find_crashed_legacy_scan() -> Vec<(String, String)> {
    let sessions_dir = match storage::mona_dir() {
...
// Active PID tracking
// ---------------------------------------------------------------------------
// Lightweight files in ~/.mona/active_pids/<session_id> containing the PID.
...
/// was still sitting in `~/.mona/sessions` (issue #336).
///
...
mod batch_crash_tests {
    ...
    -> anyhow::Result<()> {
        ...
        // No external OpenCode store exists under MONA_HOME/external, so the only
