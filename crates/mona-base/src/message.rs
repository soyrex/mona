...
pub fn generated_image_rendered_image(
    ...
    path: &str,
    output_format: &str,
) -> Option<mona_session_types::RenderedImage> {
    let (media_type, data) = generated_image_payload(path, output_format)?;
    Some(mona_session_types::RenderedImage {
        ...
        anchor: Some(mona_session_types::RenderedImageAnchor::ToolCall { id: id.to_string() }),
    })
}
