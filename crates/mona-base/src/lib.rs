//! `mona-base`: foundational layer of the jcode application core.
//!
//! This crate holds the downward-closed set of modules that the upper
//! server/tool/agent layer (`mona-app-core`) depends on: provider, auth,
...
//!
//! `mona-app-core` re-exports this crate via `pub use mona_base::*`, so every
...
pub use mona_core::{terminal_eprint, terminal_eprintln, terminal_print, terminal_println};
