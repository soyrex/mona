...
/// [`DEFAULT_SYSTEM_PROMPT`]. Precedence: project `./.mona/system-prompt.md`,
/// then global `~/.mona/system-prompt.md`, then the built-in default.
///
...
/// creating `~/.mona/swarm-prompt.md` (global) or `./.mona/swarm-prompt.md`
...
/// project `./.mona/swarm-prompt.md`, then global `~/.mona/swarm-prompt.md`,
...
/// `mission` module in the upper `mona-app-core` layer; the asset lives here
...
pub fn build_system_prompt_full_with_capabilities(
    ...
    // Add optional prompt overlays from ~/.mona/ and ./.mona/
    ...
    // Add optional preferred-tool guidance from ~/.mona/ and ./.mona/
...
fn is_desktop_working_dir(working_dir: Option<&Path>) -> bool {
    working_dir
        .and_then(mona_selfdev_types::desktop_repo_root)
        .is_some()
}
...
/// Load optional prompt overlay markdown from ~/.mona/ and ./.mona/
fn load_prompt_overlay_files_from_dir(working_dir: Option<&Path>) -> (Option<String>, usize) {
    ...
    if let Ok(global_overlay) = crate::storage::mona_dir().map(|dir| dir.join("prompt-overlay.md"))
        && !same_canonical_path(&project_overlay, &global_overlay)
        && let Some((content, size)) = load_file(
            &global_overlay,
            "Global Prompt Overlay (~/.mona/prompt-overlay.md)",
        )
    {
        total_chars += size;
        contents.push(content);
...
/// Load optional preferred-tool guidance from ~/.mona/ and ./.mona/
fn load_preferred_tools_files_from_dir(working_dir: Option<&Path>) -> (Option<String>, usize) {
    ...
    if let Ok(global_preferred_tools) =
        crate::storage::mona_dir().map(|dir| dir.join("preferred-tools.md"))
        && !same_canonical_path(&project_preferred_tools, &global_preferred_tools)
        && let Some((content, size)) = load_file(
            &global_preferred_tools,
            "Global Preferred Tools (~/.mona/preferred-tools.md)",
        )
    {
        total_chars += size;
        contents.push(content);
