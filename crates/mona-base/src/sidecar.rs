...
const CLAUDE_CODE_MONA_NOTICE: &str = "You are jcode, powered by Claude Code. You are a third-party CLI, not the official Claude Code CLI.";
...
fn anthropic_sidecar_prefers_api_key() -> bool {
    match mona_provider_core::runtime_env_pinned_mode(
        mona_provider_core::DualAuthProvider::Anthropic,
    ) {
        Some(mona_provider_core::AuthMode::ApiKey) => true,
        Some(mona_provider_core::AuthMode::Oauth) => false,
...
mod tests {
    ...
    impl crate::provider::Provider for StubProvider {
        async fn complete(
            ...
            let stream = futures::stream::once(async move {
                Ok(mona_message_types::StreamEvent::TextDelta(reply))
            });
            Ok(Box::pin(stream))
        }
    ...
    fn test_anthropic_sidecar_prefers_api_key_respects_pinned_mode() {
        ...
        let _g =
            EnvVarGuard::set_path("MONA_RUNTIME_PROVIDER", std::path::Path::new("claude-api"));
        assert!(
        ...
        let _g2 = EnvVarGuard::set_path("MONA_RUNTIME_PROVIDER", std::path::Path::new("claude"));
        assert!(
            !anthropic_sidecar_prefers_api_key(),
