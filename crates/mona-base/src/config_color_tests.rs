...
fn configured_colors_survive_a_real_config_file_round_trip() {
    ...
    // The palette-side half of that contract is asserted in `mona-tui-style`
...
///
/// The unit tests in `mona-config-types` cover lenient enum parsing in
/// isolation. This covers the path the reporter actually took: hand-written
/// `~/.mona/config.toml`, loaded through the same global cache the running
...
fn reported_display_config_survives_a_real_config_file_round_trip() {
    ...
    assert_eq!(
        ...
        mona_config_types::ReasoningDisplayMode::Current
    );
    // "inline" is the inline-only mode: no dedicated diagram widget.
    assert_eq!(
        loaded.display.diagram_mode,
        mona_config_types::DiagramDisplayMode::None
    );
    ...
    assert_eq!(
        loaded.display.diagram_mode,
        mona_config_types::DiagramDisplayMode::default()
    );

    match prev_home {
        Some(prev) => crate::env::set_var("MONA_HOME", prev),
