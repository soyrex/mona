...
#[test]
fn mona_auth_file_default_is_empty() {
...
#[test]
fn mona_path_respects_mona_home() {
    let _lock = crate::storage::lock_test_env();
    let temp = tempfile::TempDir::new().unwrap();
    let _home = EnvVarGuard::set("MONA_HOME", temp.path());

    assert_eq!(mona_path().unwrap(), temp.path().join("auth.json"));
    assert_eq!(
...
#[test]
fn sandboxed_mona_home_is_detected_without_hiding_explicit_env_credentials() {
    let _lock = crate::storage::lock_test_env();
    let temp = tempfile::TempDir::new().unwrap();
    let _home = EnvVarGuard::set("MONA_HOME", temp.path());
...
#[test]
fn real_mona_home_is_not_treated_as_a_sandbox() {
    let _lock = crate::storage::lock_test_env();
    let real_home = dirs::home_dir().unwrap().join(".jcode");
    let _home = EnvVarGuard::set("MONA_HOME", &real_home);
...
#[test]
fn mona_auth_file_multi_account() {
    let auth = JcodeAuthFile {
...
#[test]
fn mona_auth_file_legacy_migration_format() {
    let legacy_json = r#"{
        "anthropic": {
            "access": "legacy_acc",
            "refresh": "legacy_ref",
...
///   cargo test -p mona-base --lib auth::claude::tests::live_keychain -- --ignored --nocapture
#[cfg(target_os = "macos")]
#[test]
