...
const ALLOW_LEGACY_AUTH_ENV: &str = "MONA_ALLOW_CODEX_LEGACY_AUTH";
...
fn mona_auth_path() -> Result<PathBuf> {
    Ok(crate::storage::mona_dir()?.join("openai-auth.json"))
}
...
fn load_oauth_credentials_internal(return_expired: bool) -> Result<CodexCredentials> {
    ...
    if let Ok(creds) = load_mona_credentials() {
        if creds
            .expires_at
        ...
        expired_candidates.push(("mona", creds));
    }
...
fn load_mona_credentials() -> Result<CodexCredentials> {
    let auth = load_auth_file()?;
