...
/// Cached auth status plus the `MONA_HOME` it was computed under.
///
/// Auth probes read credential files relative to `MONA_HOME`. Tests swap
/// `MONA_HOME` to per-test temp dirs, and a status computed under one home
...
/// multiple browser tabs. Set `MONA_ALLOW_BROWSER_IN_TESTS=1` to opt out
...
pub fn running_in_test_harness() -> bool {
    ...
    *IN_TEST_HARNESS.get_or_init(|| {
        if env_truthy("MONA_ALLOW_BROWSER_IN_TESTS") {
            return false;
        }
        std::env::current_exe()
            .ok()
...
fn log_auth_status_snapshot(event: &str, status: &AuthStatus) {
    crate::logging::auth_event(
        ...
        "all",
        &[
            ("mona", auth_state_label(status.jcode)),
...
impl AuthStatus {
    ...
    /// per-test `MONA_HOME` swaps would poison the shared cache.
    ...
    pub fn log_snapshot(&self, surface: &str) {
        crate::logging::event_info(
            ...
            vec![
                ...
                ("mona", self.jcode.label().to_string()),
        ...
        let (
            ...
            crate::provider_catalog::LoginProviderTarget::Jcode => {
                let (source, detail) = summarize_sources(vec![
                    env_source(crate::subscription_catalog::MONA_API_KEY_ENV),
                    config_source(
                        crate::subscription_catalog::MONA_API_KEY_ENV,
                        crate::subscription_catalog::MONA_ENV_FILE,
                        "~/.config/jcode/mona-subscription.env",
                    ),
                ]);
                (
                    source,
            ...
            crate::provider_catalog::LoginProviderTarget::ClaudeApiKey => {
                ...
                // var and *not* `~/.mona/auth.json` (which holds the separate
            ...
            crate::provider_catalog::LoginProviderTarget::Bedrock => {
                let (source, detail) = summarize_sources(vec![
                    ...
                    env_source("MONA_BEDROCK_PROFILE"),
...
fn build_auth_status_uncached(mode: AuthProbeMode) -> (AuthStatus, Vec<(&'static str, u128)>) {
    ...
    record_auth_probe_step(&mut timings, "mona", || probe_mona_status(&mut status));
...
fn probe_mona_status(status: &mut AuthStatus) {
    if crate::subscription_catalog::has_credentials() {
...
fn openai_oauth_source(status: &AuthStatus) -> Option<(AuthCredentialSource, String)> {
        ...
        .is_empty()
    {
        return Some((
            AuthCredentialSource::JcodeManagedFile,
            "~/.mona/openai-auth.json".to_string(),
        ));
    }
