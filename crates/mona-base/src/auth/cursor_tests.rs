...
fn save_and_load_api_key() {
    ...
    let file = dir.path().join("mona").join("cursor.env");
...
#[test]
fn cursor_auth_file_path_respects_mona_home() {
    // Regression: on Linux the auth.json path previously used
    // `dirs::config_dir()` directly, ignoring MONA_HOME. That leaked the real
    ...
    // provider correctly looked under `$MONA_HOME/external/...`.
    ...
    assert!(
        ...
        "cursor auth path should be under MONA_HOME/external, got {}",
        path.display()
    );
...
#[test]
fn cursor_auth_file_path_does_not_escape_mona_home_on_windows() {
    let _guard = crate::storage::lock_test_env();
    let temp = tempfile::tempdir().unwrap();
    let old_home = std::env::var_os("MONA_HOME");
...
#[test]
fn cursor_vscdb_paths_respect_mona_home() {
    let _guard = crate::storage::lock_test_env();
    let prev_home = std::env::var_os("MONA_HOME");
    let temp = TempDir::new().unwrap();
    crate::env::set_var("MONA_HOME", temp.path());
