...
pub fn mona_path() -> Result<PathBuf> {
    Ok(crate::storage::mona_dir()?.join("auth.json"))
}
...
fn load_mona_credentials() -> Result<ClaudeCredentials> {
    let auth = load_auth_file()?;
    if auth.anthropic_accounts.is_empty() {
...
/// them into a mona-managed Anthropic account so future loads and token
