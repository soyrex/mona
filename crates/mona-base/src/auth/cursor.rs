...
// `MONA_CURSOR_CLIENT_VERSION` if Cursor moves the floor again.
...
pub fn cursor_direct_client_version() -> String {
    std::env::var("MONA_CURSOR_CLIENT_VERSION")
        .ok()
...
pub fn load_api_key() -> Result<String> {
    if let Ok(key) = std::env::var("CURSOR_API_KEY") {
        let trimmed = mona_provider_env::sanitize_secret_value(&key);
        if !trimmed.is_empty() {
    ...
    if file_path.exists() {
        ...
        for line in content.lines() {
            ...
            if let Some(key) = line.strip_prefix("CURSOR_API_KEY=") {
                let key = mona_provider_env::sanitize_secret_value(key);
                if !key.is_empty() {
...
pub fn cursor_auth_file_path() -> Result<PathBuf> {
    #[cfg(target_os = "windows")]
    {
        ...
        // branch, APPDATA points at the real user profile even when MONA_HOME
    ...
    {
        // Honor MONA_HOME isolation (used by the onboarding sandbox and tests)
        ...
        // `$MONA_HOME/external/...`, so a fresh-install sandbox would show only
