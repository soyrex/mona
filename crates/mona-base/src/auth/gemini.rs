...
/// an API key exists (`MONA_GEMINI_FORCE_OAUTH`, or `[provider]
...
///
/// Preference order:
/// 1. `MONA_GEMINI_CLI_PATH` (supports a full command like `npx @google/gemini-cli`)
/// 2. `gemini` on PATH
/// 3. `npx @google/gemini-cli`
pub fn gemini_cli_command() -> GeminiCliCommand {
    resolve_gemini_cli_command_with(
        std::env::var("MONA_GEMINI_CLI_PATH").ok().as_deref(),
...
pub fn tokens_path() -> Result<std::path::PathBuf> {
    Ok(crate::storage::mona_dir()?.join("gemini_oauth.json"))
}
