...
use mona_provider_core::ActiveProvider;
...
fn globally_preferred_model_rank(model: &str) -> (u8, usize) {
    ...
    let openai_default = normalize_model_for_preference(mona_provider_core::DEFAULT_OPENAI_MODEL);
    let claude_default = normalize_model_for_preference(mona_provider_core::DEFAULT_CLAUDE_MODEL);
...
fn normalize_model_for_preference(model: &str) -> String {
    let mut id = mona_provider_core::model_id::canonical(model);
    ...
    id = mona_provider_core::model_id::strip_date_suffix(&id).to_string();
...
/// `mona_provider_core::auto_default_provider`: Claude wins when both families
...
fn normalized_login_provider_id(provider_id: &str) -> Option<&'static str> {
    match provider_id.trim().to_ascii_lowercase().as_str() {
        ...
        "mona" | "subscription" | "mona-subscription" => Some("mona"),
...
fn api_key_env_bindings_for_provider(provider_id: &str) -> Vec<(String, String)> {
    match provider_id {
        ...
        "mona" => vec![
            (
                crate::subscription_catalog::MONA_API_KEY_ENV.to_string(),
                crate::subscription_catalog::MONA_ENV_FILE.to_string(),
            ),
            (
                crate::subscription_catalog::MONA_API_BASE_ENV.to_string(),
                crate::subscription_catalog::MONA_ENV_FILE.to_string(),
            ),
        ],
        "bedrock" => vec![
...
fn direct_provider_activation(provider_id: &str) -> Option<ProviderActivation> {
    let (runtime_id, active) = match normalized_login_provider_id(provider_id)? {
        ...
        "mona" => (RuntimeProviderId::Jcode, ActiveProvider::OpenRouter),
...
mod tests {
    ...
    fn direct_login_provider_ids_are_normalized_with_display_labels() {
        for (hint, normalized, label) in [
            ...
            ("subscription", "mona", "Jcode Subscription"),
    ...
    fn direct_login_provider_activation_sets_runtime_identity_and_active_provider() {
        // Sandbox MONA_HOME so activation's env-file credential sync (#453)
        ...
        for (provider, runtime, active) in [
            ...
            assert_eq!(
                std::env::var("MONA_RUNTIME_PROVIDER").as_deref(),
                Ok(runtime)
            );
            assert_eq!(
                std::env::var("MONA_ACTIVE_PROVIDER").as_deref(),
                Ok(active)
            );
            assert_eq!(
                std::env::var("MONA_INITIAL_PROVIDER_EXPLICIT").as_deref(),
                Ok("1")
            );
        }
    }
    ...
    fn direct_login_provider_descriptor_matrix_has_full_lifecycle_parity() {
        // Sandbox MONA_HOME for the same reason as the activation matrix
    ...
    #[test]
    fn mona_auth_lifecycle_matches_only_managed_subscription_routes() {
        let activation = AuthActivationResult {
            provider_id: Some("mona".to_string()),
            provider_label: Some("Jcode Subscription".to_string()),
            activated_model: Some("gpt-5.5".to_string()),
            expected_runtime: Some("mona-subscription".to_string()),
            expected_catalog_namespace: Some("mona-subscription".to_string()),
        };
        let routes = vec![
            ...
            route(
                "gpt-5.5",
                "Jcode Subscription",
                crate::subscription_catalog::MONA_ROUTE_API_METHOD,
                true,
            ),
        ];
    ...
    fn post_auth_model_selection_preserves_configured_claude_model() {
        ...
        let routes = vec![
            route(
                mona_provider_core::DEFAULT_CLAUDE_MODEL,
                "Anthropic",
                "claude-api",
                true,
            ),
        ...
        assert_eq!(
            provider_model_to_select_after_auth_with_configured_default(
                ...
                Some(mona_provider_core::DEFAULT_CLAUDE_MODEL),
                &routes,
            )
            .as_deref(),
            Some("claude-opus-4-6")
    ...
    fn post_auth_model_selection_keeps_catalog_order_for_unranked_providers() {
        ...
        // shared test-env lock (and a private MONA_HOME) or a sibling test's
        // cached catalog decides this assertion depending on ordering.
        let _env = EnvGuard::new(&["MONA_HOME"]);
