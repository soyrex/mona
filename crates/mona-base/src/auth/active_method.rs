...
//! [`AuthStatus`] plus the `MONA_RUNTIME_PROVIDER` override on its own, keyed on
...
impl From<mona_provider_core::ResolvedCredential> for ActiveCredential {
    fn from(value: mona_provider_core::ResolvedCredential) -> Self {
        match value {
            mona_provider_core::ResolvedCredential::Oauth => Self::OAuth,
            mona_provider_core::ResolvedCredential::ApiKey => Self::ApiKey,
        }
    }
}
...
pub struct ResolvedProviderAuth {
    ...
    /// True when the active credential was pinned via `MONA_RUNTIME_PROVIDER`
...
///
/// `runtime_provider` is the raw `MONA_RUNTIME_PROVIDER` value (if any); it
...
pub fn resolve_dual_credential_auth(
    ...
    let dual = mona_provider_core::DualAuthProvider::from_active_provider(provider)?;
    ...
    let forced =
        mona_provider_core::pinned_mode_for(dual, runtime_provider).map(|mode| match mode {
            mona_provider_core::AuthMode::Oauth => ActiveCredential::OAuth,
            mona_provider_core::AuthMode::ApiKey => ActiveCredential::ApiKey,
        });
