...
/// 8. optional `gh auth token` fallback when MONA_COPILOT_ALLOW_GH_AUTH_TOKEN=1
pub fn load_github_token() -> Result<String> {
    ...
    anyhow::bail!(
        "GitHub Copilot token not found. \
         ...
         or set MONA_COPILOT_ALLOW_GH_AUTH_TOKEN=1 to explicitly reuse `gh auth token`."
    )
}

fn allow_gh_cli_fallback() -> bool {
    std::env::var("MONA_COPILOT_ALLOW_GH_AUTH_TOKEN")
        .ok()
        .map(|value| {
