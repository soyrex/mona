...
pub async fn login_claude(no_browser: bool) -> Result<OAuthTokens> {
    ...
    if let Ok(code) = std::env::var("MONA_CLAUDE_AUTH_CODE") {
        let trimmed = code.trim();
        if trimmed.is_empty() {
            anyhow::bail!("MONA_CLAUDE_AUTH_CODE is set but empty");
        }
    ...
    if !std::io::stdin().is_terminal() {
        anyhow::bail!(
            "Claude login needs an authorization code from stdin. Re-run in an interactive terminal, or set MONA_CLAUDE_AUTH_CODE."
        );
    }
