...
#[test]
fn test_prompt_overlay_files_are_loaded_from_project_and_global_mona_dirs() {
    let _guard = crate::storage::lock_test_env();
    let prev_home = std::env::var_os("MONA_HOME");
    let temp = tempfile::TempDir::new().unwrap();
    crate::env::set_var("MONA_HOME", temp.path());
...
#[test]
fn test_preferred_tools_files_are_loaded_from_project_and_global_mona_dirs() {
    let _guard = crate::storage::lock_test_env();
    let prev_home = std::env::var_os("MONA_HOME");
    let temp = tempfile::TempDir::new().unwrap();
    crate::env::set_var("MONA_HOME", temp.path());
    ...
    assert!(
        direct_content.contains("Global Preferred Tools (~/.mona/preferred-tools.md)"),
...
fn project_system_prompt_file_replaces_default_base_prompt() {
    ...
    let dir = std::env::temp_dir().join(format!("mona-sysprompt-{}", std::process::id()));
    let mona_dir = dir.join(".jcode");
    std::fs::create_dir_all(&mona_dir).unwrap();
    std::fs::write(
        mona_dir.join("system-prompt.md"),
    ...
    std::fs::write(mona_dir.join("system-prompt.md"), "   \n").unwrap();
...
fn desktop_prompt_auto_detects_and_overrides_cli_in_full_and_split_modes() {
    ...
    for relative in ["", "crates/mona-desktop-ui/src"] {
...
fn desktop_prompt_detects_symlinked_nested_working_directory() {
    ...
    std::os::unix::fs::symlink(root.path().join("crates/mona-desktop-ui/src"), &link).unwrap();
...
// Restore MONA_HOME even when a regression assertion panics.
fn with_prompt_guidance_home(test: impl FnOnce(&Path)) {
    ...
    let mona_dir = home.path().join(".jcode");
    std::fs::create_dir(&mona_dir).unwrap();
    crate::env::set_var("MONA_HOME", &mona_dir);
    test(home.path());
}
