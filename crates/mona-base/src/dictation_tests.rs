#[cfg(target_os = "linux")]
use super::focused_mona_session;
use super::{
...
#[test]
fn extract_session_short_name_from_mona_window_title() {
    assert_eq!(
...
fn remember_and_read_last_focused_session() {
    ...
    let prev = std::env::var_os("MONA_HOME");
...
#[cfg(target_os = "linux")]
#[test]
fn focused_mona_session_uses_niri_window_title_when_process_name_is_generic() {
    let _guard = crate::storage::lock_test_env();
    let temp = tempfile::TempDir::new().expect("tempdir");
    ...
    let focused_process = ChildGuard::spawn_named("mona");
    ...
    assert_eq!(
        focused_mona_session().expect("resolve focused session"),
