...
async fn account_only_start_and_approval_use_expected_wire_contract() {
    ...
    assert_eq!(body, serde_json::json!({"client_name":"mona-cli"}));
...
#[tokio::test]
#[ignore = "set MONA_ACCOUNT_TEST_API_BASE to opt into live device start/pending validation"]
async fn deployed_account_start_and_pending_are_compatible() {
    let base = std::env::var("MONA_ACCOUNT_TEST_API_BASE").expect("explicit account API base");
