...
use crate::storage;
use mona_gateway_types::{PairedDevice, PairingCode};

// ---------------------------------------------------------------------------
// Device registry (persisted to ~/.mona/devices.json)
// ---------------------------------------------------------------------------
...
impl DeviceRegistry {
    /// Load from ~/.mona/devices.json
    pub fn load() -> Self {
        let path = match storage::mona_dir() {
    ...
    /// Save to ~/.mona/devices.json
    pub fn save(&self) -> Result<()> {
        let path = storage::mona_dir()?.join("devices.json");
