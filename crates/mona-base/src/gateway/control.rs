        ...
        if self.enabled && self.host_unknown {
            out.push_str(
                "\nCould not determine a reachable address for this machine. \
                 Set `MONA_GATEWAY_HOST` to a hostname or IP the other machine can reach.\n",
            );
        }
...
mod tests {
    use super::*;
    use mona_gateway_types::PairedDevice;
    ...
    /// These tests mutate the process-global `MONA_HOME`, so they must
    ...
    /// `MONA_HOME` out from under a provider test mid-assertion, failing it
