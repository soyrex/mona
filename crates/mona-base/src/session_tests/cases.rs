...
fn derive_session_provider_key_prefers_runtime_identity_over_transport() {
    let _lock = lock_env();
    let _runtime = EnvVarGuard::set("MONA_RUNTIME_PROVIDER", "azure-openai");
    let _namespace = EnvVarGuard::set("MONA_OPENROUTER_CACHE_NAMESPACE", "azure-cache");
    let _active = EnvVarGuard::set("MONA_ACTIVE_PROVIDER", "openrouter");
...
fn derive_session_provider_key_falls_back_to_openrouter_namespace() {
    let _lock = lock_env();
    let _runtime = EnvVarGuard::remove("MONA_RUNTIME_PROVIDER");
    let _namespace = EnvVarGuard::set("MONA_OPENROUTER_CACHE_NAMESPACE", "azure-openai");
    let _active = EnvVarGuard::set("MONA_ACTIVE_PROVIDER", "openrouter");
...
fn derive_session_provider_key_keeps_openai_compatible_profile_namespace() {
    let _lock = lock_env();
    let _runtime = EnvVarGuard::set("MONA_RUNTIME_PROVIDER", "openai-compatible");
    let _namespace = EnvVarGuard::set("MONA_OPENROUTER_CACHE_NAMESPACE", "zai");
    let _active = EnvVarGuard::set("MONA_ACTIVE_PROVIDER", "openrouter");
...
fn initial_session_context_preserves_explicitly_bound_cwd_when_inserted() -> Result<()> {
    ...
    let first_dir = tempfile::Builder::new()
        .prefix("mona-session-context-first-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let second_dir = tempfile::Builder::new()
        .prefix("mona-session-context-second-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
...
fn initial_session_context_can_refresh_before_real_conversation() -> Result<()> {
    ...
    let original_cwd = std::env::current_dir().map_err(|e| anyhow!(e))?;
    let first_dir = tempfile::Builder::new()
        .prefix("mona-session-context-stale-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let second_dir = tempfile::Builder::new()
        .prefix("mona-session-context-real-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
...
fn initial_session_context_does_not_refresh_after_real_conversation() -> Result<()> {
    ...
    let original_cwd = std::env::current_dir().map_err(|e| anyhow!(e))?;
    let first_dir = tempfile::Builder::new()
        .prefix("mona-session-context-original-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let second_dir = tempfile::Builder::new()
        .prefix("mona-session-context-late-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
...
fn load_startup_stub_preserves_metadata_but_skips_heavy_vectors() -> Result<()> {
    ...
    let temp_home = tempfile::Builder::new()
        .prefix("mona-startup-stub-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("MONA_HOME", temp_home.path().as_os_str());
    ...
    session.record_env_snapshot(EnvSnapshot {
        ...
        mona_version: "test".to_string(),
        mona_git_hash: Some("abc123".to_string()),
        mona_git_dirty: Some(false),
        os: "linux".to_string(),
...
fn load_for_remote_startup_preserves_messages_and_replay_but_skips_heavy_vectors() -> Result<()> {
    let _env_lock = lock_env();
    let temp_home = tempfile::Builder::new()
        .prefix("mona-remote-startup-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("MONA_HOME", temp_home.path().as_os_str());
...
fn test_create_marks_debug_when_test_session_env_enabled() {
    let _env_lock = lock_env();
    let _test_flag = EnvVarGuard::set("MONA_TEST_SESSION", "1");
...
fn test_create_not_debug_when_test_session_env_disabled() {
    let _env_lock = lock_env();
    let _test_flag = EnvVarGuard::set("MONA_TEST_SESSION", "0");
...
fn test_recover_crashed_sessions_preserves_debug_flag() -> Result<()> {
    let _env_lock = lock_env();
    let temp_home = tempfile::Builder::new()
        .prefix("mona-recover-debug-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("MONA_HOME", temp_home.path().as_os_str());
    let _test_flag = EnvVarGuard::set("MONA_TEST_SESSION", "0");
...
fn test_recover_crashed_sessions_by_ids_restores_only_selected_group() -> Result<()> {
    let _env_lock = lock_env();
    let temp_home = tempfile::Builder::new()
        .prefix("mona-recover-selected-crash-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("MONA_HOME", temp_home.path().as_os_str());
    let _test_flag = EnvVarGuard::set("MONA_TEST_SESSION", "0");
...
fn untouched_session_is_not_persisted_until_real_conversation_starts() -> Result<()> {
    let _env_lock = lock_env();
    let temp_home = tempfile::Builder::new()
        .prefix("mona-session-lazy-save-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("MONA_HOME", temp_home.path().as_os_str());
...
fn session_created_with_title_is_persisted_before_first_visible_message() -> Result<()> {
    ...
    let _env_lock = lock_env();
    let temp_home = tempfile::Builder::new()
        .prefix("mona-session-titled-save-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("MONA_HOME", temp_home.path().as_os_str());
...
fn test_save_persists_compaction_state() -> Result<()> {
    let _env_lock = lock_env();
    let temp_home = tempfile::Builder::new()
        .prefix("mona-session-compaction-save-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("MONA_HOME", temp_home.path().as_os_str());
...
fn test_save_persists_provider_key() -> Result<()> {
    let _env_lock = lock_env();
    let temp_home = tempfile::Builder::new()
        .prefix("mona-session-provider-key-save-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("MONA_HOME", temp_home.path().as_os_str());
...
fn test_save_persists_reasoning_effort() -> Result<()> {
    let _env_lock = lock_env();
    let temp_home = tempfile::Builder::new()
        .prefix("mona-session-reasoning-effort-save-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("MONA_HOME", temp_home.path().as_os_str());
...
fn test_save_appends_journal_and_load_replays_it() -> Result<()> {
    let _env_lock = lock_env();
    let temp_home = tempfile::Builder::new()
        .prefix("mona-session-journal-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("MONA_HOME", temp_home.path().as_os_str());
...
fn test_save_checkpoints_after_full_mutation_and_clears_journal() -> Result<()> {
    let _env_lock = lock_env();
    let temp_home = tempfile::Builder::new()
        .prefix("mona-session-checkpoint-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("MONA_HOME", temp_home.path().as_os_str());
...
fn test_journal_replay_skips_corrupt_line_and_keeps_tail() -> Result<()> {
    let _env_lock = lock_env();
    let temp_home = tempfile::Builder::new()
        .prefix("mona-session-journal-corrupt-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("MONA_HOME", temp_home.path().as_os_str());
...
fn test_journal_replay_salvages_glued_entries_on_torn_line() -> Result<()> {
    let _env_lock = lock_env();
    let temp_home = tempfile::Builder::new()
        .prefix("mona-session-journal-glued-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("MONA_HOME", temp_home.path().as_os_str());
...
fn test_corrupt_journal_heals_via_checkpoint_on_next_save() -> Result<()> {
    let _env_lock = lock_env();
    let temp_home = tempfile::Builder::new()
        .prefix("mona-session-journal-heal-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("MONA_HOME", temp_home.path().as_os_str());
...
fn test_render_messages_renders_reasoning_before_answer_in_stored_order() {
    ...
    use mona_render_core::REASONING_SENTINEL;

    let _env_lock = lock_env();
    let _mode = EnvVarGuard::set("MONA_REASONING_DISPLAY", "full");
...
fn test_render_messages_hides_persisted_reasoning_in_current_mode() {
    ...
    let _env_lock = lock_env();
    let _mode = EnvVarGuard::set("MONA_REASONING_DISPLAY", "current");
    crate::config::invalidate_config_cache();
...
fn test_render_messages_hides_persisted_reasoning_in_off_mode() {
    ...
    let _env_lock = lock_env();
    let _mode = EnvVarGuard::set("MONA_REASONING_DISPLAY", "off");
    crate::config::invalidate_config_cache();
...
fn reasoning_trace_survives_session_save_and_load() -> Result<()> {
    let _env_lock = lock_env();
    let temp_home = tempfile::Builder::new()
        .prefix("mona-reasoning-persist-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("MONA_HOME", temp_home.path().as_os_str());
