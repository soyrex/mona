...
use anyhow::Result;
use mona_provider_core::{ModelRoute, ModelRouteApiMethod, Provider, ResolvedCredential};
use mona_usage_types::ModelUsage;
...
fn key(model: &str, provider: &str, api_method: &str) -> RouteKey {
    let provider = mona_provider_core::normalize_model_route_provider_label(provider);
    ...
    (
        ...
        provider.to_string(),
        mona_provider_core::RuntimeKey::from_api_method(
...
fn path() -> Result<std::path::PathBuf> {
    Ok(crate::storage::mona_dir()?.join("model-usage-v1.sqlite3"))
}
...
pub fn serving_route(provider: &dyn Provider, selected_method: Option<&str>) -> Option<ModelRoute> {
    ...
    let provider_name = mona_provider_core::normalize_model_route_provider_label(provider.name());
    ...
    let mut candidates: Vec<_> = provider
        ...
        .filter(|route| {
            ...
            let same_provider = if let Some(pin) = pin.as_ref() {
                ...
                mona_provider_core::model_route_provider_labels_match(&route.provider, &display)
            };
            same_model
                && same_provider
