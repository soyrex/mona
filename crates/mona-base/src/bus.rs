...
pub use mona_batch_types::{BatchProgress, BatchSubcallProgress, BatchSubcallState};
...
pub struct ModelRefreshCompleted {
    pub session_id: String,
    pub result: std::result::Result<mona_provider_core::ModelCatalogRefreshSummary, String>,
}
...
/// background tasks, without re-coupling `mona-base` to swarm internals.
#[derive(Clone, Debug)]
...
pub enum BusEvent {
    ...
    UsageReport(Vec<mona_usage_types::ProviderUsage>),
    /// Progressive usage report update while providers are still loading
    UsageReportProgress(mona_usage_types::ProviderUsageProgress),
