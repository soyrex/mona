...
#[tokio::test]
#[ignore = "requires MONA_MEMORY_JEV_LIVE_TEST=1 and live Jev credentials"]
async fn live_synthetic_relevance_acceptance() -> Result<()> {
    ensure!(
        std::env::var("MONA_MEMORY_JEV_LIVE_TEST").as_deref() == Ok("1"),
        "Live Jev acceptance not run: set MONA_MEMORY_JEV_LIVE_TEST=1 explicitly"
    );
    ensure!(
