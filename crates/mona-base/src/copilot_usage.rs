...
//! doesn't expose a usage API. Data persists to ~/.mona/copilot_usage.json.
...
pub use mona_usage_types::{AllTimeUsage, CopilotUsageTracker, DayUsage, MonthUsage};
...
mod tests {
    ...
    #[test]
    fn usage_path_respects_mona_home() {
        let _env_lock = lock_env();
        clear_tracker();
        let temp = tempfile::tempdir().expect("tempdir");
        let _home = EnvVarGuard::set("MONA_HOME", temp.path().as_os_str());

        assert_eq!(usage_path(), temp.path().join("copilot_usage.json"));
    }

    #[test]
    fn save_and_load_roundtrip_under_mona_home() {
        let _env_lock = lock_env();
        clear_tracker();
        let temp = tempfile::tempdir().expect("tempdir");
        let _home = EnvVarGuard::set("MONA_HOME", temp.path().as_os_str());
