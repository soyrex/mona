...
pub const MONA_API_KEY_ENV: &str = "MONA_API_KEY";
pub const MONA_API_BASE_ENV: &str = "MONA_API_BASE";
pub const MONA_ACCOUNT_ID_ENV: &str = "MONA_ACCOUNT_ID";
pub const MONA_ACCOUNT_EMAIL_ENV: &str = "MONA_ACCOUNT_EMAIL";
pub const MONA_TIER_ENV: &str = "MONA_TIER";
pub const MONA_ENV_FILE: &str = "mona-subscription.env";
pub const MONA_CACHE_NAMESPACE: &str = "mona-subscription";
pub const MONA_SUBSCRIPTION_ACTIVE_ENV: &str = "MONA_SUBSCRIPTION_ACTIVE";
pub const DEFAULT_MONA_API_BASE: &str = "https://api.jcode.sh/v1";
pub const MONA_PRICING_URL: &str = "https://jcode.sh/pricing";
pub const MONA_ACCOUNT_URL: &str = "https://jcode.sh/account";
...
pub const MONA_PROVIDER_DISPLAY_NAME: &str = "Jcode Subscription";
pub const MONA_ROUTE_API_METHOD: &str = "mona-subscription";
...
fn normalize_model_key(model: &str) -> String {
    ...
    mona_provider_core::model_id::canonical(base)
}
...
pub fn cached_tier() -> Option<JcodeTier> {
    provider_catalog::load_env_value_from_env_or_config(MONA_TIER_ENV, MONA_ENV_FILE)
        .as_deref()
...
pub fn configured_api_key() -> Option<String> {
    provider_catalog::load_env_value_from_env_or_config(MONA_API_KEY_ENV, MONA_ENV_FILE)
}

pub fn configured_api_base() -> Option<String> {
    provider_catalog::load_env_value_from_env_or_config(MONA_API_BASE_ENV, MONA_ENV_FILE)
}
...
pub fn persist_account_credentials(
    ...
    for (key, value) in [
        (MONA_API_KEY_ENV, Some(api_key)),
        (MONA_ACCOUNT_ID_ENV, nonempty(account_id)),
        (MONA_ACCOUNT_EMAIL_ENV, nonempty(email)),
        (MONA_TIER_ENV, nonempty(tier)),
    ] {
        provider_catalog::save_env_value_to_env_file(key, MONA_ENV_FILE, value)?;
    }
...
pub fn clear_account_credentials() -> anyhow::Result<()> {
    for key in [
        MONA_API_KEY_ENV,
        MONA_ACCOUNT_ID_ENV,
        MONA_ACCOUNT_EMAIL_ENV,
        MONA_TIER_ENV,
    ] {
        provider_catalog::save_env_value_to_env_file(key, MONA_ENV_FILE, None)?;
    }
    clear_runtime_env();
    ensure_account_credential_permissions()
}
...
pub fn account_credential_path() -> anyhow::Result<std::path::PathBuf> {
    Ok(crate::storage::app_config_dir()?.join(MONA_ENV_FILE))
}
...
pub fn is_runtime_mode_enabled() -> bool {
    std::env::var(MONA_SUBSCRIPTION_ACTIVE_ENV)
        .ok()
        .map(|value| {
            matches!(
...
pub fn apply_runtime_env() {
    crate::env::set_var(MONA_SUBSCRIPTION_ACTIVE_ENV, "1");
    crate::env::set_var(
        "MONA_OPENROUTER_API_BASE",
        configured_api_base().unwrap_or_else(|| DEFAULT_MONA_API_BASE.to_string()),
    );
    crate::env::set_var("MONA_OPENROUTER_API_KEY_NAME", MONA_API_KEY_ENV);
    crate::env::set_var("MONA_OPENROUTER_ENV_FILE", MONA_ENV_FILE);
    crate::env::set_var("MONA_OPENROUTER_CACHE_NAMESPACE", MONA_CACHE_NAMESPACE);
    crate::env::set_var("MONA_OPENROUTER_PROVIDER_FEATURES", "0");
    crate::env::set_var("MONA_OPENROUTER_TRANSPORT_STATE", "mona-subscription");
    crate::env::remove_var("MONA_OPENROUTER_ALLOW_NO_AUTH");
    crate::env::remove_var("MONA_OPENROUTER_PROVIDER");
    crate::env::remove_var("MONA_OPENROUTER_NO_FALLBACK");
}

pub fn clear_runtime_env() {
    crate::env::remove_var(MONA_SUBSCRIPTION_ACTIVE_ENV);
    crate::env::remove_var("MONA_OPENROUTER_API_BASE");
    crate::env::remove_var("MONA_OPENROUTER_API_KEY_NAME");
    crate::env::remove_var("MONA_OPENROUTER_ENV_FILE");
    crate::env::remove_var("MONA_OPENROUTER_CACHE_NAMESPACE");
    crate::env::remove_var("MONA_OPENROUTER_PROVIDER_FEATURES");
    crate::env::remove_var("MONA_OPENROUTER_TRANSPORT_STATE");
    crate::env::remove_var("MONA_OPENROUTER_ALLOW_NO_AUTH");
    crate::env::remove_var("MONA_OPENROUTER_PROVIDER");
    crate::env::remove_var("MONA_OPENROUTER_NO_FALLBACK");
}
...
mod tests {
    ...
    fn hosted_catalog_access_does_not_depend_on_legacy_cached_tier() {
        ...
        crate::env::remove_var(MONA_TIER_ENV);
        let temp = tempfile::tempdir().expect("temp home");
        crate::env::set_var("MONA_HOME", temp.path().to_string_lossy().to_string());
        ...
        crate::env::set_var(MONA_TIER_ENV, "mystery");
        assert_eq!(cached_tier(), None);
        assert_eq!(effective_tier(), JcodeTier::Plus);

        for tier in [JcodeTier::Pro, JcodeTier::Max] {
            crate::env::set_var(MONA_TIER_ENV, tier.as_str());
        ...
        crate::env::set_var(MONA_TIER_ENV, JcodeTier::Ultra.as_str());
