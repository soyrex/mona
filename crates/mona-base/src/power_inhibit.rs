...
const DISABLE_ENV: &str = "MONA_DISABLE_POWER_INHIBIT";
...
impl PowerInhibitor {
    ...
    /// unless the legacy `MONA_DISABLE_POWER_INHIBIT` env escape hatch is set.
    ///
...
impl WindowsPowerGuard {
    fn acquire() -> io::Result<Self> {
        ...
        let thread = thread::Builder::new()
            .name("mona-power-inhibit".to_string())
...
impl Drop for WindowsPowerGuard {
    fn drop(&mut self) {
        if let Err(error) = self.stop() {
            mona_logging::warn(&format!("failed to stop Windows power guard: {error}"));
        }
    }
}
...
fn run_windows_power_guard(
    ...
    if ready_tx.send(Ok(())).is_err() {
        if unsafe { SetThreadExecutionState(windows_clear_execution_state_flags()) } == 0 {
            mona_logging::warn("failed to clear Windows power guard after receiver disconnect");
        }
        return;
    }
