...
impl SkillRegistry {
    ...
    ///
    /// Holds GLOBAL skills only (plugins, `~/.mona/skills/`,
    ...
    /// Only runs if ~/.mona/skills/ doesn't exist yet.
    fn import_from_external() {
        let mona_skills = match crate::storage::mona_dir() {
            Ok(dir) => dir.join("skills"),
            Err(_) => return,
        };

        if mona_skills.exists() {
        ...
        {
            let count = Self::copy_skills_dir(&claude_skills, &mona_skills);
            if count > 0 {
                sources.push(format!("{} from Claude Code", count));
                copied.extend(Self::list_skill_names(&mona_skills));
            }
        }
        ...
        if let Ok(codex_skills) = crate::storage::user_home_path(".codex/skills")
            && codex_skills.is_dir()
        {
            let count = Self::copy_skills_dir(&codex_skills, &mona_skills);
            if count > 0 {
    ...
    pub fn load_global() -> Result<Self> {
        ...
        // Load from ~/.mona/skills/ (jcode's own global skills)
        if let Ok(mona_dir) = crate::storage::mona_dir() {
            let mona_skills = mona_dir.join("skills");
            if mona_skills.exists() {
                registry.load_from_dir(&mona_skills)?;
            }
        }
    ...
    /// `./.mona/skills/`, `./.agents/skills/`, and `./.claude/skills/`.
    ///
    ...
    fn load_project_local_dirs(&mut self, working_dir: Option<&Path>) -> Result<()> {
        // Load from ./.mona/skills/ (project-local jcode skills)
    ...
    /// Reload the shared global skill sources (plugins, `~/.mona/skills/`,
    ...
    pub fn reload_global(&mut self) -> Result<usize> {
        ...
        if let Ok(mona_dir) = crate::storage::mona_dir() {
            let mona_skills = mona_dir.join("skills");
            if mona_skills.exists() {
                count += self.load_from_dir_count(&mona_skills)?;
            }
        }
...
pub struct EndorsedSkill {
    ...
    /// Grouping label used to organize the endorsed list (e.g. "mona",
    /// "NVIDIA CUDA-X").
...
mod tests {
    ...
    #[test]
    fn load_for_working_dir_reads_project_local_mona_skills() {
    ...
    #[test]
    fn explicit_mona_skill_wins_over_plugin_skill_with_same_name() {
        let temp = tempfile::tempdir().expect("tempdir");
