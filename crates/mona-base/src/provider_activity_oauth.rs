...
fn path() -> anyhow::Result<std::path::PathBuf> {
    Ok(crate::storage::mona_dir()?.join("openai_oauth_usage.json"))
}
...
fn priced_usage(
    ...
    let cost =
        mona_provider_core::pricing::openai_api_pricing_with_tier(model, tier).and_then(|price| {
            if !complete {
                return None;
            }
...
mod tests {
    ...
    fn process_writer_child() {
        let Some(path) = std::env::var_os("MONA_OAUTH_TEST_LEDGER") else {
            return;
        };
        for _ in 0..10 {
            update(
        ...
        for _ in 0..4 {
            children.push(
                std::process::Command::new(&executable)
                    ...
                    .env("MONA_OAUTH_TEST_LEDGER", &path)
