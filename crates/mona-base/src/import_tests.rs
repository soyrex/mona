...
fn imported_tool_history_is_provider_neutral() {
    ...
    let _home = EnvVarGuard::set_path("MONA_HOME", temp.path());
...
fn repeated_external_resume_reuses_the_imported_snapshot() {
    ...
    .unwrap();
    let target = mona_session_types::ResumeTarget::ClaudeCodeSession {
...
fn ordinary_resume_never_stops_a_live_claude_process() {
    ...
    assert!(matches!(
        resolved,
        mona_session_types::ResumeTarget::JcodeSession { .. }
    ));
...
fn explicit_takeover_preserves_history_and_stops_only_matching_process() {
    ...
    let mona_session_types::ResumeTarget::JcodeSession { session_id } = resolved else {
...
#[cfg(target_os = "linux")]
#[test]
fn takeover_timeout_preserves_the_staged_mona_session_after_sigterm() {
    use std::process::{Command, Stdio};
...
fn cached_imported_session_preserves_existing_history_verbatim() {
    ...
    legacy.append_stored_message(StoredMessage {
        id: "mona-continuation".to_string(),
        role: Role::Assistant,
        content: vec![ContentBlock::Text {
    ...
    let resolved =
        resolve_resume_target_to_jcode(&mona_session_types::ResumeTarget::CodexSession {
...
#[test]
fn test_import_pi_session_creates_mona_snapshot() {
    let _guard = crate::storage::lock_test_env();
    let temp = tempfile::TempDir::new().unwrap();
    let _home = EnvVarGuard::set_path("MONA_HOME", temp.path());
...
#[test]
fn test_import_opencode_session_creates_mona_snapshot() {
    let _guard = crate::storage::lock_test_env();
    let temp = tempfile::TempDir::new().unwrap();
    let _home = EnvVarGuard::set_path("MONA_HOME", temp.path());
...
#[test]
fn test_resolve_resume_target_to_mona_imports_codex_session() {
    let _guard = crate::storage::lock_test_env();
    let temp = tempfile::TempDir::new().unwrap();
    let _home = EnvVarGuard::set_path("MONA_HOME", temp.path());
...
/// had coverage here).
#[test]
fn test_resolve_resume_target_to_mona_imports_claude_code_session() {
    let _guard = crate::storage::lock_test_env();
    let temp = tempfile::TempDir::new().unwrap();
    let _home = EnvVarGuard::set_path("MONA_HOME", temp.path());
    ...
    let resolved =
        resolve_resume_target_to_jcode(&mona_session_types::ResumeTarget::ClaudeCodeSession {
    ...
    assert_eq!(
        imported_session_id_for_target(&mona_session_types::ResumeTarget::ClaudeCodeSession {
            session_id: "claude-resolve-test".to_string(),
            session_path: transcript_path.to_string_lossy().to_string(),
        }),
...
/// blind re-import previously overwrote the snapshot and dropped the mona-side
/// messages. The continuation must be preserved instead.
#[test]
fn test_reimporting_claude_session_preserves_mona_continuation() {
    let _guard = crate::storage::lock_test_env();
    let temp = tempfile::TempDir::new().unwrap();
    let _home = EnvVarGuard::set_path("MONA_HOME", temp.path());
    ...
    // User resumes inside jcode and appends a mona-only follow-up message.
    let mut session = Session::load(&imported_id).unwrap();
    session.append_stored_message(StoredMessage {
        id: "mona-continuation".to_string(),
        role: Role::User,
        content: vec![ContentBlock::Text {
            text: "mona-only follow up".to_string(),
            cache_control: None,
        }],
        display_role: None,
        timestamp: None,
    ...
    assert_eq!(
        after.messages.len(),
        3,
        "mona-side continuation must not be clobbered by re-import"
    );
    let preserved = after.messages.iter().flat_map(|m| m.content.iter()).any(
        |block| matches!(block, ContentBlock::Text { text, .. } if text == "mona-only follow up"),
    );
    assert!(
        preserved,
        "the mona-only follow up message must be preserved"
    );
}

#[test]
fn test_import_cursor_session_creates_mona_snapshot() {
    let _guard = crate::storage::lock_test_env();
    let temp = tempfile::TempDir::new().unwrap();
    let _home = EnvVarGuard::set_path("MONA_HOME", temp.path());
    ...
    let resumed = crate::import::resolve_resume_target_to_jcode(
        &mona_session_types::ResumeTarget::CursorSession {
