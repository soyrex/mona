...
# Set false here or set MONA_NO_EMOJI=1 for ASCII fallbacks.
emoji = true
...
# bing_api_key_env = "MONA_BING_API_KEY"
# bing_api_key = ""
...
# this. Configure here or via the MONA_SEARXNG_URL environment variable, then
...
# Env overrides: MONA_MCP_TOOLS, MONA_MCP_TOOLS_TOKEN_THRESHOLD.
mcp_tools = "auto"
...
#   claude        = Claude via OAuth/subscription (token in ~/.mona/auth.json)
...
# to match this value. Also overridable per-launch via MONA_STREAM_IDLE_TIMEOUT_SECS.
...
# Env overrides: MONA_MAX_RETRIES, MONA_RETRY_BACKOFF_CAP_SECS.
# max_retries = 8
...
# Env override: MONA_SWARM_EFFORT
# swarm_effort = "medium"
#
...
# Env overrides: MONA_SWARM_ROOT_EFFORT, MONA_SWARM_DEEP_ROOT_EFFORT
...
# Env override: MONA_SWARM_SPAWN_MODE
...
# Env override: MONA_SWARM_MAX_CONCURRENT_AGENTS
...
# Env override: MONA_SWARM_STRIP_LAYOUT
...
# Env override: MONA_MEMORY_JEV_PROVIDER
...
# Env overrides: MONA_MEMORY_SIDECAR_ENABLED, MONA_MEMORY_MODEL
...
# Set MONA_TERMINAL to force a supported terminal emulator instead.
...
#
# When set, jcode runs `<spawn_hook> <mona-binary> <args...>` instead of
# opening a terminal emulator itself. The hook receives MONA_SPAWN_* env vars
# describing the spawn so multiplexers/wrappers can decide where it appears:
#   MONA_SPAWN_KIND        - "swarm-agent", "resume", "selfdev", "restart", ...
#   MONA_SPAWN_SESSION_ID  - session the window will run
#   MONA_SPAWN_TITLE       - suggested window/tab title
#   MONA_SPAWN_CWD         - session working directory (also the hook's cwd)
#   MONA_SPAWN_PROGRAM     - jcode binary path
#   MONA_SPAWN_COMMAND     - full shell-escaped command line
#   MONA_SPAWN_SWARM_ID / MONA_SPAWN_COORDINATOR_SESSION_ID (swarm spawns)
# If the hook fails to start, jcode falls back to built-in terminal detection.
# Env override: MONA_SPAWN_HOOK (set empty to disable a config hook).
#
# Examples:
#   spawn_hook = "tmux new-window"                # tmux window per agent
#   spawn_hook = "kitty @ launch --type=tab --"   # kitty tab per agent
#   spawn_hook = "~/bin/mona-spawn-router"       # custom placement script
# spawn_hook = ""
#
# External command used to focus/raise an existing session window, replacing
# the built-in wmctrl/xdotool title search. Receives MONA_FOCUS_SESSION_ID
# and MONA_FOCUS_TITLE env vars. Pair with spawn_hook so the program that
# placed the window also brings it to the front.
# Env override: MONA_FOCUS_HOOK (set empty to disable a config hook).
#
# Example:
#   focus_hook = "~/bin/mona-focus-router"
# focus_hook = ""
#
...
# ~/.mona/preferred_terminal.json file. After changing this, re-run
...
# (quotes work) but executed directly, with MONA_HOOK_* env vars describing
# the event:
#   MONA_HOOK_EVENT       - "turn_start", "turn_end", "session_start",
#                            "session_end", "pre_tool", "post_tool"
#   MONA_HOOK_SESSION_ID  - the session the event belongs to
#   MONA_HOOK_CWD         - session working directory (also the hook's cwd)
#   MONA_HOOK_PAYLOAD     - JSON mirror of all fields
# Hook processes get MONA_HOOKS_DISABLED=1 so nested jcode calls don't recurse.
#
# All hooks except pre_tool are observers: detached, fire-and-forget, failures
# only logged. Env overrides: MONA_HOOK_TURN_START, MONA_HOOK_TURN_END,
# MONA_HOOK_SESSION_START, MONA_HOOK_SESSION_END, MONA_HOOK_PRE_TOOL,
# MONA_HOOK_POST_TOOL (set empty to disable a config hook).
#
...
# think/stream window before any tool call. Extra fields: MONA_HOOK_MODEL,
# MONA_HOOK_SOURCE ("chat"/"resume"/"ambient").
# turn_start = "~/bin/mona-turn-start"
#
# Runs when an agent turn completes. Extra fields: MONA_HOOK_STATUS
# ("ok"/"error"), MONA_HOOK_DURATION_MS, MONA_HOOK_MODEL,
# MONA_HOOK_LAST_ASSISTANT_TEXT (first 4000 chars), MONA_HOOK_ERROR.
# turn_end = "~/bin/mona-turn-notify"
#
# Runs when a session becomes active. Extra: MONA_HOOK_SOURCE
# ("create"/"attach"/"resume").
# session_start = ""
#
# Runs when a session closes normally. Extra: MONA_HOOK_SOURCE ("close").
# session_end = ""
#
# Gate hook before every tool call. Receives MONA_HOOK_TOOL_NAME and the tool
# input JSON on stdin (truncated copy in MONA_HOOK_TOOL_INPUT). Exit 0 allows
...
# pre_tool = "~/bin/mona-tool-policy"
#
# Max milliseconds to wait for pre_tool before failing open (default: 5000).
# pre_tool_timeout_ms = 5000
#
# Runs after each tool call. Extra fields: MONA_HOOK_TOOL_NAME,
# MONA_HOOK_STATUS, MONA_HOOK_DURATION_MS, MONA_HOOK_OUTPUT_BYTES,
# MONA_HOOK_ERROR.
# post_tool = ""
...
# Set MONA_DISABLE_POWER_INHIBIT=1 to force-disable regardless of this setting.
...
# ntfy_topic = "mona-ambient-your-secret-topic"
...
# email_smtp_port = 587
# Password via env: MONA_SMTP_PASSWORD (preferred) or config below
# email_password = ""
...
# telegram_bot_token = ""  # From @BotFather (prefer MONA_TELEGRAM_BOT_TOKEN env var)
...
# discord_bot_token = ""     # From Discord Developer Portal (prefer MONA_DISCORD_BOT_TOKEN env var)
...
# MONA_JADE_RELAY_API_BASE, MONA_JADE_RELAY_TOKEN, MONA_JADE_RELAY_TOKEN_ID,
# MONA_JADE_RELAY_USER_ID, MONA_JADE_RELAY_SESSION_ID.
...
# endpoint = "https://api.jcode.sh/v1/discovery"
	"##;
        ...
        let p = mona_config_types::KeybindingPlatform::current();
        let effort_increase =
            mona_config_types::default_binding("effort_increase", p).unwrap_or("alt+right");
        let effort_decrease =
            mona_config_types::default_binding("effort_decrease", p).unwrap_or("alt+left");
        default_content
...
mod tests {
    ...
    fn documented_color_example_is_valid_when_uncommented() {
        ...
        for (role, value) in &parsed.display.colors {
            assert!(
                mona_config_types::DisplayConfig::default()
                    .colors
                    .is_empty(),
