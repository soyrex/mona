use super::*;
use crate::storage::mona_dir;
use std::path::PathBuf;

impl Config {
    /// Get the config file path
    pub fn path() -> Option<PathBuf> {
        mona_dir().ok().map(|d| d.join("config.toml"))
    }
    ...
    ///
    /// Scans `~/.mona/sessions` for the directories the user works in most,
    ...
    pub fn bake_launch_hotkeys_once() -> bool {
        use mona_import_core::repo_ranking;

        let cfg = Self::load();
        if cfg.launch_hotkeys.imported {
            return false;
        }
        let Ok(mona_dir) = mona_dir() else {
            return false;
        };
        let sessions_dir = mona_dir.join("sessions");
        ...
        let entries: Vec<mona_config_types::LaunchHotkeyEntry> = plan
            .into_iter()
            .map(|p| mona_config_types::LaunchHotkeyEntry {
                chord: p.chord,
    ...
    pub fn external_auth_source_allowed_for_path_cached(
        ...
        // a new path-bound trust decision, or before tests switch MONA_HOME. Fall
...
mod issue_1056_tests {
    ...
    fn effort_update_preserves_profile_with_capitalized_bearer_auth() {
        ...
        let _home = EnvGuard::set("MONA_HOME", home.path());
