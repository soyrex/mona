...
const REQUIRED_BRIDGE_ACTION_PROBES: &[(&str, &str)] = &[
    ...
    (
        "uploadFile",
        r#"{"selector":"input[type=file]","filePath":"/tmp/mona-browser-capability-probe"}"#,
    ),
];

fn mona_dir() -> PathBuf {
    storage::mona_dir().unwrap_or_else(|_| {
        dirs::home_dir()
...
async fn download_browser_binary() -> Result<()> {
    ...
    let client = mona_provider_core::shared_http_client();
...
///
/// Set `MONA_BROWSER_AUTOLAUNCH=0` to keep jcode from starting Firefox on its
/// own. Tests also use this to stay hermetic.
pub fn firefox_autolaunch_disabled() -> bool {
    matches!(
        std::env::var("MONA_BROWSER_AUTOLAUNCH").as_deref(),
