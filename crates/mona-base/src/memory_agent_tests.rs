...
async fn missing_jev_access_clears_pending_without_fallback() {
    ...
    let _env = TestEnv::set(&[("MONA_MEMORY_JEV_PROVIDER", "disabled-for-test")]);
...
async fn automatic_recall_uses_jev_http_without_embeddings_or_sidecar() {
    ...
    let _env = TestEnv::set(&[
        ("MONA_HOME", dir.path().to_str().unwrap()),
        ("MONA_MEMORY_JEV_PROVIDER", "mona"),
        ("MONA_API_KEY", "mona_test_only_never_a_real_key"),
        ("MONA_API_BASE", &base),
    ]);
