...
pub use mona_import_core::repo_ranking;
...
fn truncate_import_text(text: String, max_bytes: usize) -> String {
    ...
    format!(
        "{}{}",
        mona_core::util::truncate_str(&text, prefix_budget),
        suffix
    )
}
...
fn reuse_existing_imported_session(session_id: &str) -> bool {
    ...
    // doing so can discard mona-only turns and journal state. New imports are
...
pub fn imported_session_id_for_target(
    target: &mona_session_types::ResumeTarget,
) -> Option<String> {
    match target {
        mona_session_types::ResumeTarget::JcodeSession { session_id } => Some(session_id.clone()),
        mona_session_types::ResumeTarget::ClaudeCodeSession { session_id, .. } => {
            Some(imported_claude_code_session_id(session_id))
        }
        mona_session_types::ResumeTarget::CodexSession { session_id, .. } => {
            Some(imported_codex_session_id(session_id))
        }
        mona_session_types::ResumeTarget::PiSession { session_path } => {
            Some(imported_pi_session_id(session_path))
        }
        mona_session_types::ResumeTarget::OpenCodeSession { session_id, .. } => {
            Some(imported_opencode_session_id(session_id))
        }
        mona_session_types::ResumeTarget::CursorSession { session_id, .. } => {
...
pub fn resolve_resume_target_to_jcode(
    target: &mona_session_types::ResumeTarget,
) -> Result<mona_session_types::ResumeTarget> {
    use mona_session_types::ResumeTarget;
...
fn import_session_from_file_with_target(
    ...
    let mut session = Session::create_with_id(mona_session_id, None, title);
...
fn take_over_live_claude_session_with_timeout(
    ...
    crate::logging::info(&format!(
        "Claude takeover complete: source_session={session_id} source_pid={} mona_session={takeover_id}",
        live.pid
    ));
    crate::session_list_cache::invalidate();
...
fn finalize_imported_session(
    ...
    // Never overwrite a mona-side continuation with a shorter external
...
pub fn import_cursor_session_from_path(
    ...
    let created_at =
        mona_import_core::file_modified_datetime(session_path).unwrap_or_else(Utc::now);
