...
///
/// Resolution order: `MONA_SPAWN_HOOK` env (handled by config env overrides;
...
///
/// When a spawn hook is configured (`[terminal] spawn_hook` / `MONA_SPAWN_HOOK`),
/// the hook takes over the spawn: jcode runs `<hook> <program> <args...>` with
/// `MONA_SPAWN_*` metadata env vars so external programs (tmux, kitty remote,
...
mod tests {
    ...
    fn spawn_via_hook_runs_hook_with_metadata_env() {
        ...
        std::fs::write(
            &hook_path,
            format!(
                "#!/bin/sh\nprintf '%s|%s|%s|%s' \"$MONA_SPAWN_KIND\" \"$MONA_SPAWN_SESSION_ID\" \"$MONA_SPAWN_SWARM_ID\" \"$*\" > {}\n",
        ...
        .spawn_env("MONA_SPAWN_SWARM_ID", "swarm-7");
