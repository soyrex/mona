...
impl GmailBackend {
    ...
    ///
    /// Defaults to `Direct`. Set `MONA_GMAIL_BACKEND=composio` (with
    /// `COMPOSIO_API_KEY` present) to broker Gmail through Composio.
    pub fn from_env() -> Self {
        let selection = std::env::var("MONA_GMAIL_BACKEND")
            .unwrap_or_default()
            .trim()
            .to_lowercase();
        if selection == "composio" {
            ...
            eprintln!(
                "MONA_GMAIL_BACKEND=composio but COMPOSIO_API_KEY is not set; falling back to direct Gmail backend"
            );
        }
        GmailBackend::Direct
    }
...
impl ComposioConnection {
    pub fn path() -> Result<std::path::PathBuf> {
        Ok(crate::storage::mona_dir()?.join("composio_gmail.json"))
    }
...
impl GmailClient {
    ...
    pub async fn connect(&self, open_browser: bool) -> Result<ComposioConnection> {
        let cfg = match &self.backend {
            ...
            GmailBackend::Direct => {
                anyhow::bail!(
                    "The Composio connect flow is only available when MONA_GMAIL_BACKEND=composio."
                )
            }
        };
