...
/// e.g. `/run/user/1000/jcode/myserver.sock` -> `\\.\pipe\mona-myserver`
fn path_to_pipe_name(path: &Path) -> String {
    let stem: String = path
        ...
        .take(32)
        .collect();
    let stem = if stem.is_empty() { "mona" } else { &stem };
    let normalized = path
        .to_string_lossy()
        .replace('\\', "/")
        .to_ascii_lowercase();
...
impl Stream {
    ...
    pub fn pair() -> io::Result<(Self, Self)> {
        ...
        let pipe_name = format!(r"\\.\pipe\mona-pair-{}-{}", std::process::id(), counter);
...
mod tests {
    ...
    fn pipe_name_matches_the_typescript_sdk() {
        for (path, expected) in [
            (
                r"C:\Users\jeremy\AppData\Local\jcode\run\mona-api.sock",
                r"\\.\pipe\mona-api-5e00c01702e8cfe4",
            ),
            (r"C:\a\b\jcode.sock", r"\\.\pipe\mona-52dfdb00b2f35a71"),
        ] {
            assert_eq!(
