...
impl LiveOpenAITestEnv {
    fn new() -> Result<Option<Self>> {
        let lock = mona_base::storage::lock_test_env();
        let Some(source_auth) = real_codex_auth_path() else {
            return Ok(None);
        };

        let temp = tempfile::Builder::new()
            .prefix("mona-openai-live-")
            .tempdir()?;
        ...
        let mona_home = EnvVarGuard::set_path("MONA_HOME", temp.path());
        let transport = EnvVarGuard::set("MONA_OPENAI_TRANSPORT", "https");

        Ok(Some(Self {
            _lock: lock,
            _mona_home: mona_home,
...
async fn live_openai_catalog() -> Result<Option<mona_base::provider::OpenAIModelCatalog>> {
    let Some(_env) = LiveOpenAITestEnv::new()? else {
        return Ok(None);
    };
    let creds = mona_base::auth::codex::load_credentials()?;
...
fn openai_credential_mode_runtime_provider_identity_round_trips() {
    ...
    mona_base::env::set_var("MONA_RUNTIME_PROVIDER", "openai");
    assert_eq!(
    ...
    mona_base::env::set_var("MONA_RUNTIME_PROVIDER", "openai-api");
    assert_eq!(
        OpenAICredentialMode::from_runtime_env(mona_provider_core::DualAuthProvider::OpenAI),
