...
async fn websocket_v2_prewarm_is_adopted_by_complete_without_losing_request_state() {
    ...
    let _base = EnvVarGuard::set("MONA_OPENAI_API_BASE", &format!("http://{addr}/v1"));
    let _transport = EnvVarGuard::set("MONA_OPENAI_TRANSPORT", "websocket");
    let _prewarm = EnvVarGuard::set("MONA_OPENAI_PREWARM", "1");
