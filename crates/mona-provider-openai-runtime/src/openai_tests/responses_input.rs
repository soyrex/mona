...
fn test_build_responses_input_replaces_oversized_native_compaction_with_text() {
    let oversized =
        "x".repeat(mona_base::provider::openai_request::OPENAI_ENCRYPTED_CONTENT_SAFE_MAX_CHARS + 1);
...
fn test_build_response_request_cache_policy_matches_shared_ttl() {
    for (model, retention, expected_field, expected_ttl) in [
        ...
        assert_eq!(
            mona_base::provider::openai::prompt_cache_ttl_for_model(Some(model), retention),
            expected_ttl,
            "{model}"
        );
    }
