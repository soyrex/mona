...
pub(super) async fn stream_response(
    ...
    if !response.status().is_success() {
        ...
        {
            mona_base::provider::record_model_unavailable_for_account(model_name, &reason);
            mona_base::logging::warn(&format!(
        ...
        return Err(OpenAIStreamFailure::Other(
            mona_provider_core::retry_after::error_with_retry_after(msg, retry_after),
        ));
    }
...
async fn continue_persistent_ws_locked(
    ...
    if state.connected_at.elapsed() >= Duration::from_secs(WEBSOCKET_PERSISTENT_MAX_AGE_SECS) {
        mona_base::logging::info("Persistent WS connection too old; forcing reconnect");
        *guard = None;
        log_openai_stream_lifecycle(
            mona_base::logging::LogLevel::Info,
            "persistent_state_reset",
    ...
    if incremental_items.is_empty() {
        mona_base::logging::info("No incremental items to send; need fresh request");
        *guard = None;
        log_openai_stream_lifecycle(
            mona_base::logging::LogLevel::Info,
            "persistent_state_reset",
    ...
    let continuation_payload = serde_json::json!({
        ...
        "previous_response_id_hash": mona_provider_core::fingerprint::stable_hash_str(&previous_response_id),
    ...
    emit_connection_phase(tx, mona_message_types::ConnectionPhase::SendingRequest).await;
    ...
    emit_connection_phase(tx, mona_message_types::ConnectionPhase::WaitingForResponse).await;
    ...
    if let Some(resp_id) = new_response_id {
        ...
        mona_base::logging::warn("Persistent WS: no response_id in response; breaking chain");
        *guard = None;
        log_openai_stream_lifecycle(
            mona_base::logging::LogLevel::Warn,
            "persistent_state_reset",
...
fn maybe_record_runtime_model_unavailable_from_stream_error(model: &str, message: &str) {
    ...
    if let Some(reason) = reason {
        mona_base::provider::record_model_unavailable_for_account(model, &reason);
        mona_base::logging::warn(&format!(
