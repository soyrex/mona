...
use super::*;
use mona_base::auth::codex::OpenAiAccount;
...
impl OAuthUsageRecorder {
    pub(super) fn capture(creds: &CodexCredentials, request: &Value) -> Self {
        let label = if OpenAIProvider::is_chatgpt_mode(creds) {
            mona_base::auth::codex::list_accounts()
                .ok()
    ...
    pub(super) async fn observe(&mut self, event: &StreamEvent) {
        ...
        if let Err(error) = tokio::task::spawn_blocking(move || {
            mona_base::provider_activity::record_openai_oauth_usage(
                &label,
                &model,
                tier.as_deref(),
                input,
        ...
        .await
        {
            mona_base::logging::warn(&format!("ChatGPT usage recorder task failed: {error}"));
        }
    }
}
...
mod tests {
    ...
    async fn terminal_usage_recorded_once_before_delivery() {
        ...
        let old = std::env::var_os("MONA_HOME");
        ...
        let rows = mona_base::provider_activity::openai_oauth_usage_summary("captured");
        assert!(
            rows[1]
                .1
        ...
        if let Some(old) = old {
            mona_base::env::set_var("MONA_HOME", old);
        } else {
