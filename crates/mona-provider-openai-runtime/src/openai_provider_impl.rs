...
fn catalog_error_is_auth_rejection(err: &anyhow::Error) -> bool {
    err.downcast_ref::<mona_base::provider::ModelCatalogHttpStatus>()
...
impl Provider for OpenAIProvider {
    ...
    fn set_credential_mode(&self, mode: mona_provider_core::CredentialMode) -> anyhow::Result<()> {
    ...
    async fn prewarm(&self, tools: &[ToolDefinition], system: &str) {
        if self.is_browser_only()
            ...
            || std::env::var("MONA_OPENAI_PREWARM").is_ok_and(|value| {
                matches!(
    ...
    async fn complete(
        ...
        tokio::spawn(async move {
                ...
                for attempt in 0..MAX_RETRIES {
                    if attempt > 0 {
                        emit_connection_phase(
                            &tx,
                            mona_message_types::ConnectionPhase::Retrying {
                                attempt: attempt + 1,
                                max: MAX_RETRIES,
                            },
                        )
                    ...
                    match result {
                        ...
                        Err(OpenAIStreamFailure::Other(error)) => {
                            ...
                            log_openai_stream_lifecycle(
                                mona_base::logging::LogLevel::Error,
                                "attempt_failed",
                                vec![
                                    ("model", model_for_transport.clone()),
                                    ("attempt", (attempt + 1).to_string()),
                            ...
                            let error = match mona_schema_dialect::learn_from_error(
                                &error.to_string(),
                                &mona_schema_dialect::registry::OPENAI,
                            ) {
                                Some(explanation) => {
                                    mona_base::logging::warn(&format!(
            ...
            if let Err(panic_payload) = result {
                ...
                mona_base::logging::error(&format!(
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        ...
        if mona_provider_core::is_openai_api_only_pro_model(model) {
        ...
        if let Ok(mut current) = self.model.try_write() {
            ...
            *current = model.to_string();
            mona_base::provider::clear_model_unavailable_for_account(model);
            drop(current);
            if changed {
    ...
    fn available_models(&self) -> Vec<&'static str> {
        ...
        mona_provider_core::ALL_OPENAI_MODELS.to_vec()
    }
    ...
    fn available_models_for_switching(&self) -> Vec<String> {
        ...
        let mut models =
            mona_base::provider::cached_openai_model_ids().unwrap_or_else(|| vec![self.model()]);
        ...
        if mona_base::provider::openai_platform_api_key_configured() {
            for pro in mona_provider_core::OPENAI_API_ONLY_PRO_MODELS {
    ...
    async fn prefetch_models(&self) -> Result<()> {
        ...
        let catalog = if is_chatgpt_mode {
            ...
            match mona_base::provider::fetch_openai_model_catalog(&access_token).await {
                ...
                Err(err) if catalog_error_is_auth_rejection(&err) => {
                    ...
                    mona_base::provider::fetch_openai_model_catalog(&refreshed).await?
                }
                Err(err) => return Err(err),
            }
        } else {
            mona_base::provider::fetch_openai_api_key_model_catalog(&access_token).await?
        };
        ...
        if !catalog.available_models.is_empty() {
            mona_base::provider::populate_account_models(catalog.available_models);
        }
        Ok(())
    }
    ...
    fn set_reasoning_effort(&self, effort: &str) -> Result<()> {
        ...
        if !requested.is_empty()
            && mona_provider_core::canonical_reasoning_effort(&requested).is_none()
            && !mona_base::prompt::is_swarm_effort(&requested)
        {
            anyhow::bail!(
            ...
            && !mona_base::prompt::is_swarm_effort(requested)
        {
            ...
            if !available.contains(&requested) {
                anyhow::bail!(
                    ...
                    available
                        .into_iter()
                        .filter(|effort| !mona_base::prompt::is_swarm_effort(effort))
                        .collect::<Vec<_>>()
                        .join(", ")
                );
            }
    ...
    fn available_efforts(&self) -> Vec<&'static str> {
        let model = mona_provider_core::model_id::canonical(&self.model());
        ...
        if let Some(advertised) = advertised {
            let mut efforts: Vec<&'static str> = advertised
                .iter()
                .filter_map(|effort| mona_provider_core::canonical_reasoning_effort(effort))
                .collect();
    ...
    async fn native_compact(
    ...
    ) -> Result<mona_provider_core::NativeCompactionResult> {
        ...
        if let Some(encrypted_content) = existing_openai_encrypted_content {
            if !mona_base::provider::openai_request::openai_encrypted_content_is_sendable(
                encrypted_content,
            ) {
                anyhow::bail!(
                    "OpenAI native compaction payload is too large to replay ({} chars > safe limit {} chars)",
                    encrypted_content.len(),
                    mona_base::provider::openai_request::OPENAI_ENCRYPTED_CONTENT_SAFE_MAX_CHARS,
                );
            }
        ...
        Ok(mona_provider_core::NativeCompactionResult {
            summary_text: None,
    ...
    fn context_window(&self) -> usize {
        let model = self.model();
        mona_provider_core::context_limit_for_model_with_provider(&model, Some(self.name()))
            .unwrap_or(mona_provider_core::DEFAULT_CONTEXT_LIMIT)
    }
