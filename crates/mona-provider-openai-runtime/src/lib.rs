...
//! persistent WebSocket), moved out of `mona-base` so provider edits compile
...
//! [`OpenAIProvider`] with `mona_base::provider::external` at startup.
//!
//! Model-catalog/account-availability state stays in `mona_base::provider`
//! (it is shared vocabulary for routing), as does the pure request shaping in
//! `mona_base::provider::openai_request`.
...
use mona_base::provider::openai_request::{build_responses_input, build_tools};
#[cfg(test)]
use mona_message_types::TOOL_OUTPUT_MISSING_TEXT;
use mona_message_types::{Message as ChatMessage, StreamEvent, ToolDefinition};
...
const DEFAULT_MODEL: &str = mona_provider_core::DEFAULT_OPENAI_MODEL;
const ORIGINATOR: &str = "codex_cli_rs";

pub(crate) const CHATGPT_WEB_MODEL: &str = mona_provider_core::CHATGPT_WEB_MODEL;
...
/// `MONA_OPENAI_WS_IDLE_RECONNECT_SECS` (0 disables the idle reconnect entirely,
...
static WEBSOCKET_PERSISTENT_IDLE_RECONNECT_SECS: LazyLock<Option<u64>> = LazyLock::new(|| {
    match std::env::var("MONA_OPENAI_WS_IDLE_RECONNECT_SECS") {
        Ok(raw) => match raw.trim().parse::<u64>() {
            Ok(0) => {
                mona_base::logging::info(
                    "OpenAI persistent WS idle reconnect disabled (MONA_OPENAI_WS_IDLE_RECONNECT_SECS=0); relying on healthcheck + max-age",
                );
                None
            }
            Ok(secs) => {
                mona_base::logging::info(&format!(
                    "OpenAI persistent WS idle reconnect threshold set to {}s (MONA_OPENAI_WS_IDLE_RECONNECT_SECS)",
                    secs
                ));
                Some(secs)
            }
            Err(_) => {
                mona_base::logging::info(&format!(
                    "Warning: invalid MONA_OPENAI_WS_IDLE_RECONNECT_SECS '{}'; using default {}s",
        ...
        match raw.trim().to_ascii_lowercase().as_str() {
            ...
            other => {
                mona_base::logging::warn(&format!(
                    "Unknown MONA_OPENAI_TRANSPORT '{}'; using auto. Use: auto, websocket, or https.",
                    other
                ));
                Self::Auto
            }
...
/// Shared dual-auth credential pin (see `mona_provider_core::CredentialMode`).
/// The OpenAI-specific alias is kept so existing call sites read naturally.
pub(crate) use mona_provider_core::CredentialMode as OpenAICredentialMode;
...
pub(crate) fn load_credentials_for_mode(mode: OpenAICredentialMode) -> Result<CodexCredentials> {
    match mode {
        OpenAICredentialMode::Auto => mona_base::auth::codex::load_credentials(),
        OpenAICredentialMode::OAuth => mona_base::auth::codex::load_oauth_credentials(),
        OpenAICredentialMode::ApiKey => mona_base::auth::codex::load_api_key_credentials(),
    }
}
...
fn log_openai_stream_lifecycle(
    level: mona_base::logging::LogLevel,
    phase: &str,
    ...
    mona_base::logging::event(level, "PROVIDER_STREAM_LIFECYCLE", owned);
}
...
fn persistent_ws_input_item_hashes(input: &[Value]) -> Vec<u64> {
    input
        .iter()
        .map(mona_provider_core::fingerprint::stable_hash_json)
        .collect()
}
...
async fn emit_connection_phase(
    ...
    phase: mona_message_types::ConnectionPhase,
) {
        ...
        loop {
                ...
                >= Duration::from_secs(WEBSOCKET_PERSISTENT_MAX_AGE_SECS)
            {
                log_openai_stream_lifecycle(
                    mona_base::logging::LogLevel::Info,
            ...
            match ensure_persistent_ws_is_healthy(state, false).await {
                ...
                Err(error) => {
                    ...
                    log_openai_stream_lifecycle(
                        mona_base::logging::LogLevel::Warn,
                        "persistent_state_reset",
                        vec![
                            ("model", model.clone()),
...
impl OpenAIProvider {
    fn effective_prompt_cache_retention<'a>(
        ...
        mona_base::provider::openai::effective_prompt_cache_retention(model_id, configured)
    }
    ...
    fn new_inner(credentials: CodexCredentials, browser_only: bool) -> Self {
        ...
        let prompt_cache_key = std::env::var("MONA_OPENAI_PROMPT_CACHE_KEY")
            .ok()
        ...
        {
            mona_base::logging::warn(&format!(
                "Unsupported MONA_OPENAI_PROMPT_CACHE_RETENTION '{}'; expected 'in_memory' or '24h'",
                raw.trim()
            ));
        }
        ...
        let native_compaction_threshold_tokens = mona_base::config::config()
            .provider
            .openai_native_compaction_threshold_tokens
            .max(1000);
        let model_reasoning_efforts =
            mona_base::provider::cached_openai_reasoning_efforts().unwrap_or_default();
    ...
    pub(crate) fn set_credential_mode(&self, mode: OpenAICredentialMode) -> Result<()> {
        ...
        if let Some(route) = mode.auth_route(mona_provider_core::DualAuthProvider::OpenAI) {
    ...
    fn reload_cached_reasoning_efforts(&self) {
        let cached = mona_base::provider::cached_openai_reasoning_efforts().unwrap_or_default();
    ...
    async fn clear_persistent_ws(&self, reason: &str) {
        ...
        if persistent_ws.is_some() {
            mona_base::logging::info(&format!("Clearing persistent OpenAI WS state: {}", reason));
        }
        *persistent_ws = None;
    }
    ...
    fn default_reasoning_effort_for_model(model: &str) -> Option<String> {
        let key = mona_provider_core::model_id::canonical(model);
    ...
    fn revalidate_reasoning_effort(&self) {
        ...
        if mona_base::prompt::is_swarm_effort(&current)
    ...
    fn api_reasoning_effort(&self, effort: Option<&str>) -> Option<String> {
        self.api_reasoning_effort_with_swarm_root(
            effort,
            effort.and_then(mona_base::prompt::swarm_root_reasoning_effort),
        )
    }
    ...
    fn api_reasoning_effort_with_swarm_root(
        ...
        let effort = effort?;
        if !mona_base::prompt::is_swarm_effort(effort) {
            return Some(effort.to_string());
        }
        let resolved = swarm_root.unwrap_or("max");
        let available = self.available_efforts();
        let ladder = mona_provider_core::OPENAI_SELECTABLE_EFFORTS;
        ...
        ladder[..=requested]
            ...
            .or_else(|| {
                ladder.iter().find(|candidate| {
                    !mona_base::prompt::is_swarm_effort(candidate) && available.contains(candidate)
                })
            })
    ...
    fn parse_max_output_tokens(raw: Option<&str>) -> Option<u32> {
        ...
        match raw.parse::<u32>() {
            ...
            Err(_) => {
                mona_base::logging::warn(&format!(
                    "Invalid MONA_OPENAI_MAX_OUTPUT_TOKENS='{}'; using default {}",
    ...
    fn load_max_output_tokens() -> Option<u32> {
        let raw = std::env::var("MONA_OPENAI_MAX_OUTPUT_TOKENS").ok();
        let parsed = Self::parse_max_output_tokens(raw.as_deref());
        if raw.is_some() {
            match parsed {
                Some(value) => mona_base::logging::info(&format!(
                    "OpenAI max_output_tokens configured to {}",
                    value
                )),
                None => mona_base::logging::info(
                    "OpenAI max_output_tokens disabled (MONA_OPENAI_MAX_OUTPUT_TOKENS=0)",
                ),
            }
        }
        parsed
    ...
    /// `MONA_OPENAI_API_BASE`, `OPENAI_BASE_URL`, `OPENAI_API_BASE`.
    ///
    ...
    pub(crate) fn resolve_api_base() -> String {
        mona_base::provider::openai::resolve_api_base()
    }
    ...
    fn response_request_for_model(
        ...
        let reasoning_effort = self
            ...
            .unwrap_or_else(|poisoned| poisoned.into_inner().clone())
            // No explicit user effort: fall back to the model's mona-side
    ...
    async fn model_id(&self) -> String {
        ...
        match availability.state {
            mona_base::provider::AccountModelAvailabilityState::Unavailable => {
                ...
                if let Some(fallback) = mona_base::provider::get_best_available_openai_model()
                    && fallback != current
                {
                    mona_base::logging::info(&format!(
            ...
            mona_base::provider::AccountModelAvailabilityState::Unknown => {
                if mona_base::provider::should_refresh_openai_model_catalog()
                    && mona_base::provider::begin_openai_model_catalog_refresh()
                {
                    let is_chatgpt_mode = {
            ...
            mona_base::provider::AccountModelAvailabilityState::Available => {}
        }
