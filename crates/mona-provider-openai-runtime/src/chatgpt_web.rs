...
use mona_message_types::{ContentBlock, Message, Role, StreamEvent, ToolDefinition};
use mona_provider_core::EventStream;
...
const TOOL_CALL_START: &str = "<mona_tool_call>";
const TOOL_CALL_END: &str = "</mona_tool_call>";
...
impl ChatGptWebState {
    ...
    async fn run_turn(
        ...
        let status = mona_base::browser::ensure_browser_ready_noninteractive()
            .await
            .context(
        ...
        let result = async {
            send_phase(tx, mona_message_types::ConnectionPhase::Authenticating).await?;
            ...
            send_phase(tx, mona_message_types::ConnectionPhase::SendingRequest).await?;
            if tx.is_closed() {
                anyhow::bail!("ChatGPT web response consumer was closed before submission");
            }
            ...
            send_phase(tx, mona_message_types::ConnectionPhase::WaitingForResponse).await?;
...
fn next_owned_tab_name() -> String {
    ...
    let millis = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_millis();
    format!("mona-chatgpt-web-{millis}-{sequence}")
}
...
async fn poll_for_response(tab_id: u64, tx: &mpsc::Sender<Result<StreamEvent>>) -> Result<String> {
    let timeout_secs = std::env::var("MONA_CHATGPT_WEB_TIMEOUT_SECS")
        .ok()
...
"#,
        ...
        if Instant::now() >= deadline {
            anyhow::bail!(
                "GPT-5.6 Pro web response timed out after {} seconds (override with MONA_CHATGPT_WEB_TIMEOUT_SECS)",
                timeout_secs
            );
        }
...
"#,
        ...
        if !text.is_empty() && !streaming_emitted {
            send_phase(tx, mona_message_types::ConnectionPhase::Streaming).await?;
...
<mona_tool_call>{"name":"tool_name","input":{"argument":"value"}}</mona_tool_call>
...
async fn bridge_command(action: &str, params: Value) -> Result<Value> {
    let binary = mona_base::browser::browser_binary_path();
    if !binary.exists() {
        anyhow::bail!(
...
mod tests {
    ...
    fn tool_call_parser_accepts_valid_exact_envelope() {
        let parsed = parse_tool_call(
            "  <mona_tool_call>{\"name\":\"bash\",\"input\":{\"command\":\"pwd\",\"intent\":\"Check cwd\"}}</mona_tool_call>\n",
        )
        .unwrap()
        .unwrap();
    ...
    fn tool_call_parser_rejects_incomplete_envelope() {
        let err = parse_tool_call("<mona_tool_call>{\"name\":\"bash\"}")
            .err()
    ...
    fn tool_call_parser_rejects_quoted_or_suffixed_envelope() {
        for response in [
            "Example: <mona_tool_call>{\"name\":\"bash\",\"input\":{}}</mona_tool_call>",
            "<mona_tool_call>{\"name\":\"bash\",\"input\":{}}</mona_tool_call> done",
        ] {
    ...
    fn tool_call_parser_rejects_non_object_input() {
        let err = parse_tool_call(
            "<mona_tool_call>{\"name\":\"bash\",\"input\":[\"not\",\"an\",\"object\"]}</mona_tool_call>",
        )
        .err()
    ...
    fn tool_call_parser_allows_marker_text_inside_json_string_values() {
        let parsed = parse_tool_call(
            "<mona_tool_call>{\"name\":\"bash\",\"input\":{\"command\":\"printf '</mona_tool_call>'\"}}</mona_tool_call>",
        )
        .unwrap()
        .unwrap();
        assert_eq!(parsed.input["command"], "printf '</mona_tool_call>'");
    }
