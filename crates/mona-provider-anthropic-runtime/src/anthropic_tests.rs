...
fn configured_direct_headers_parse_and_reject_invalid_values() {
    ...
    let _invalid = EnvVarGuard::set("MONA_ANTHROPIC_HEADERS", r#"{"bad header":"x"}"#);
...
fn anthropic_auth_token_selects_bearer_without_affecting_explicit_profile_auth() {
    ...
    let _explicit = EnvVarGuard::set("MONA_ANTHROPIC_AUTH", "header");
...
fn named_profile_runtime_captures_transport_and_credential_immutably() {
    let _lock = mona_base::storage::lock_test_env();
    let _base = EnvVarGuard::set("MONA_ANTHROPIC_API_BASE", "https://one.example/v1");
    let _auth = EnvVarGuard::set("MONA_ANTHROPIC_AUTH", "bearer");
    let _key_name = EnvVarGuard::set("MONA_ANTHROPIC_API_KEY_NAME", "PROFILE_ONE_KEY");
    let _key = EnvVarGuard::set("PROFILE_ONE_KEY", "one-secret");
    let provider = AnthropicProvider::new();

    let _changed_base = EnvVarGuard::set("MONA_ANTHROPIC_API_BASE", "https://two.example/v1");
...
fn named_anthropic_profile_accepts_its_configured_custom_model() {
    ...
    let _home_guard = EnvVarGuard::set("MONA_HOME", _home.path());
    std::fs::write(
    ...
    .expect("write config");
    mona_base::config::Config::invalidate_cache();
    let _profile = EnvVarGuard::set("MONA_NAMED_PROVIDER_PROFILE", "custom");
...
#[tokio::test]
#[ignore = "live smoke: requires ANTHROPIC_API_KEY, or set MONA_LIVE_ANTHROPIC_ALLOW_OAUTH=1 to use Claude OAuth credentials"]
async fn live_anthropic_reasoning_smoke() -> Result<()> {
    let _env_lock = mona_base::storage::lock_test_env();
    let using_api_key = std::env::var_os("ANTHROPIC_API_KEY").is_some();
    let allow_oauth = std::env::var_os("MONA_LIVE_ANTHROPIC_ALLOW_OAUTH").is_some();
    if !using_api_key && !allow_oauth {
        eprintln!(
            "skipping live Anthropic smoke: set ANTHROPIC_API_KEY or MONA_LIVE_ANTHROPIC_ALLOW_OAUTH=1"
        );
        return Ok(());
    }

    let _max_tokens = EnvVarGuard::set_if_missing("MONA_ANTHROPIC_MAX_TOKENS", "2048");
    ...
    let require_thinking = std::env::var_os("MONA_LIVE_ANTHROPIC_REQUIRE_THINKING").is_some();
    ...
    if require_thinking {
        assert!(
            thinking_bytes > 0,
            "live Anthropic response did not include thinking deltas despite MONA_LIVE_ANTHROPIC_REQUIRE_THINKING"
        );
    }
    Ok(())
}
...
fn credential_mode_runtime_provider_identity_round_trips() {
    ...
    let previous = std::env::var_os("MONA_RUNTIME_PROVIDER");

    mona_base::env::set_var("MONA_RUNTIME_PROVIDER", "claude");
    assert_eq!(
        AnthropicCredentialMode::from_runtime_env(mona_provider_core::DualAuthProvider::Anthropic),
        AnthropicCredentialMode::OAuth,
        "OAuth selection must surface as the OAuth runtime identity"
    );

    mona_base::env::set_var("MONA_RUNTIME_PROVIDER", "claude-api");
    assert_eq!(
        AnthropicCredentialMode::from_runtime_env(mona_provider_core::DualAuthProvider::Anthropic),
    ...
    match previous {
        Some(value) => mona_base::env::set_var("MONA_RUNTIME_PROVIDER", value),
        None => mona_base::env::remove_var("MONA_RUNTIME_PROVIDER"),
    }
}
...
async fn auto_mode_falls_back_to_api_key_when_oauth_is_expired() {
    ...
    let _home = EnvVarGuard::set("MONA_HOME", temp.path());
    let _api_key = EnvVarGuard::set("ANTHROPIC_API_KEY", "test-anthropic-api-key");
    let _runtime = EnvVarGuard::set("MONA_RUNTIME_PROVIDER", "auto");

    mona_base::auth::claude::upsert_account(mona_base::auth::claude::AnthropicAccount {
...
async fn explicit_oauth_mode_does_not_silently_fall_back_to_api_key() {
    ...
    let _home = EnvVarGuard::set("MONA_HOME", temp.path());
    let _api_key = EnvVarGuard::set("ANTHROPIC_API_KEY", "test-anthropic-api-key");
    let _runtime = EnvVarGuard::set("MONA_RUNTIME_PROVIDER", "claude");
...
fn anthropic_fallback_prefers_best_available_and_skips_tried_and_retired() {
    ...
    let _guard = mona_base::storage::lock_test_env();
    mona_base::provider::models::reset_model_catalog_services_for_tests();
    let known = mona_base::provider::known_anthropic_model_ids();
    assert!(
        !known.is_empty(),
...
fn fable_quota_fallback_selects_the_best_available_opus() {
    ...
    let candidates = mona_base::provider::cached_anthropic_model_ids()
...
fn model_scoped_usage_routes_only_exhausted_fable_to_opus() {
    let usage = mona_base::usage::UsageData {
        model_scoped: vec![mona_base::usage::ModelScopedUsageWindow {
    ...
    let available = mona_base::usage::UsageData {
        model_scoped: vec![mona_base::usage::ModelScopedUsageWindow {
            model_name: "Fable".to_string(),
            utilization: 0.98,
            resets_at: None,
