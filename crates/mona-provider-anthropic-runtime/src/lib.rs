...
//! loading, cache-TTL toggles) stay in `mona_base::provider::anthropic`;
...
use mona_base::auth;
use mona_base::auth::oauth;
use mona_provider_core::{EventStream, NativeToolResultSender, Provider};
fn oauth_beta_headers(model: &str) -> &'static str {
    mona_provider_core::anthropic_oauth_beta_headers(model)
}

use anyhow::{Context, Result};
use async_trait::async_trait;
use futures::StreamExt;
use mona_base::provider::anthropic::{
...
#[cfg(test)]
use mona_base::provider::anthropic::{OAUTH_BETA_HEADERS, effectively_1m};
#[cfg(test)]
use mona_message_types::{ContentBlock, Role};
use mona_message_types::{Message, StreamEvent, ToolDefinition};
#[cfg(test)]
use mona_provider_anthropic::{ApiContentBlock, ToolResultContent, ToolResultContentBlock};
use mona_provider_anthropic::{
...
fn direct_api_url() -> String {
    let base = std::env::var("MONA_ANTHROPIC_API_BASE")
        .ok()
...
fn configured_direct_headers() -> Result<HeaderMap> {
    let Some(raw) = std::env::var("MONA_ANTHROPIC_HEADERS")
        .ok()
    ...
    let headers: std::collections::BTreeMap<String, String> = serde_json::from_str(&raw)
        .context("MONA_ANTHROPIC_HEADERS must be a JSON object of string values")?;
...
fn direct_auth_mode() -> String {
    std::env::var("MONA_ANTHROPIC_AUTH")
        .ok()
        .filter(|value| !value.trim().is_empty())
        .unwrap_or_else(|| {
...
impl DirectTransportConfig {
    fn from_env() -> Self {
        Self {
            ...
            auth_header: std::env::var("MONA_ANTHROPIC_AUTH_HEADER")
...
fn active_anthropic_profile_models() -> Option<Vec<String>> {
    let Ok(profile_name) = std::env::var("MONA_NAMED_PROVIDER_PROFILE") else {
        return None;
    };
    let profile = mona_base::config::config().providers.get(&profile_name)?;
    if !matches!(
        profile.provider_type,
        mona_base::config::NamedProviderType::AnthropicCompatible
    ) {
        return None;
    }
...
#[cfg(test)]
pub(crate) const OAUTH_BETA_HEADERS_1M: &str = mona_provider_core::ANTHROPIC_OAUTH_BETA_HEADERS_1M;
...
fn load_official_claude_client_metadata() -> OAuthClientMetadata {
    let path = match mona_base::storage::user_home_path(".claude.json") {
        Ok(path) => path,
...
async fn oauth_preflight_get(
    ...
    if !resp.status().is_success() {
        ...
        let body = mona_base::util::http_error_body(resp, "HTTP error").await;
...
async fn ensure_oauth_preflight(
    ...
    let eval = OAuthEvalRequest {
        attributes: OAuthEvalAttributes {
            ...
            subscription_type: mona_base::auth::claude::get_subscription_type()
    ...
    if all_ok {
        mona_base::logging::info("Claude OAuth preflight completed successfully");
    }
    Ok(())
}

/// Quality-first default shared with model routing and first-run selection.
const DEFAULT_MODEL: &str = mona_provider_core::DEFAULT_CLAUDE_MODEL;
...
pub struct AnthropicProvider {
    ...
    /// Explicit `MONA_ANTHROPIC_MAX_TOKENS` override. When unset, the output
...
impl AnthropicProvider {
    fn is_usage_exhausted() -> bool {
        let usage = mona_base::usage::get_sync();
        usage.five_hour >= 0.99 && usage.seven_day >= 0.99
    }

    fn best_available_opus_model(exclude: &str) -> Option<String> {
        let mut models = mona_base::provider::cached_anthropic_model_ids()
            .unwrap_or_else(mona_base::provider::known_anthropic_model_ids);
        models.retain(|model| {
    ...
    fn fallback_for_model_scoped_usage(
        selected_model: &str,
        usage: &mona_base::usage::UsageData,
    ) -> Option<String> {
    ...
    async fn model_after_oauth_quota_check(
        ...
        let Ok(usage) = mona_base::usage::fetch_usage_for_access_token(token).await else {
            return selected_model;
        };
    ...
    /// `MONA_RUNTIME_PROVIDER` happens to be in the current process (e.g. a
    /// self-dev session may have it set to `claude-api`). This also updates
    /// `MONA_RUNTIME_PROVIDER` so any provider instances the probes build
    ...
    pub async fn fetch_live_model_ids_for_doctor(&self) -> Result<Vec<String>> {
        ...
        let catalog = if is_oauth {
            mona_base::provider::fetch_anthropic_model_catalog_oauth(&token).await?
        } else {
            mona_base::provider::fetch_anthropic_model_catalog(&token).await?
        };
        ...
        if !catalog.available_models.is_empty() {
            mona_base::provider::populate_anthropic_models(catalog.available_models.clone());
        }
        Ok(catalog.available_models)
    }

    pub fn new() -> Self {
        let model = std::env::var("MONA_ANTHROPIC_MODEL").unwrap_or_else(|_| {
        ...
        let _ = tokio::runtime::Handle::try_current().map(|_| {
            tokio::spawn(async {
                let _ = mona_base::usage::get().await;
            })
        });

        let max_tokens_override = std::env::var("MONA_ANTHROPIC_MAX_TOKENS")
            .ok()
            .and_then(|v| v.trim().parse::<u32>().ok());
        let reasoning_effort = mona_base::config::config()
            .provider
        ...
        let profile_api_key = std::env::var_os("MONA_ANTHROPIC_API_BASE")
        ...
        Self {
            ...
            credential_mode: Arc::new(RwLock::new(AnthropicCredentialMode::from_runtime_env(
                mona_provider_core::DualAuthProvider::Anthropic,
            ))),
            max_tokens_override,
    ...
    fn model_supports_output_effort(model: &str) -> bool {
        ...
        // generations); see `mona_provider_core::anthropic_reasoning_caps`.
        // Fable 5 verified live 2026-07-01; Sonnet 5 verified live 2026-07-07.
        mona_provider_core::anthropic_reasoning_caps(model).output_effort
    }

    fn model_supports_adaptive_thinking(model: &str) -> bool {
        mona_provider_core::anthropic_reasoning_caps(model).adaptive_thinking
    }

    fn model_supports_manual_thinking(model: &str) -> bool {
        mona_provider_core::anthropic_reasoning_caps(model).manual_thinking
    }

    fn model_supports_xhigh_effort(model: &str) -> bool {
        mona_provider_core::anthropic_reasoning_caps(model).xhigh_effort
    }
    ...
    fn model_supports_max_effort(model: &str) -> bool {
        mona_provider_core::anthropic_reasoning_caps(model).max_effort
    }

    fn model_supports_reasoning_effort(model: &str) -> bool {
        mona_provider_core::anthropic_reasoning_caps(model).supports_reasoning_effort()
    }
    ...
    fn actual_effort_for_model(model: &str, effort: &str) -> String {
        let effort = mona_base::prompt::swarm_root_reasoning_effort(effort).unwrap_or(effort);
    ...
    fn store_effort_for_model(model: &str, effort: &str) -> String {
        if mona_base::prompt::is_deep_swarm_effort(effort) {
            mona_base::prompt::SWARM_DEEP_EFFORT.to_string()
        } else if mona_base::prompt::is_swarm_effort(effort) {
            mona_base::prompt::SWARM_EFFORT.to_string()
        } else {
    ...
    fn max_tokens_for(&self, model: &str) -> u32 {
        self.max_tokens_override
            .unwrap_or_else(|| mona_provider_core::anthropic::anthropic_max_output_tokens(model))
    }
    ...
    fn build_reasoning_request_parts(
        ...
        let show_thinking = mona_base::config::config().display.show_thinking;
    ...
    async fn get_oauth_access_token(&self) -> Result<(String, bool)> {
        ...
        if fresh_creds.expires_at < now + 300_000 && !fresh_creds.refresh_token.is_empty() {
            ...
            {
                Ok(refreshed) => {
                    mona_base::logging::info("OAuth token refreshed successfully");
                ...
                Err(e) => {
                    mona_base::logging::error(&format!("OAuth token refresh failed: {}", e));
    ...
    pub(crate) fn set_credential_mode(&self, mode: AnthropicCredentialMode) -> Result<()> {
        ...
        *mode_guard = mode;
        ...
        if let Some(route) = mode.auth_route(mona_provider_core::DualAuthProvider::Anthropic) {
            mona_base::env::set_var("MONA_RUNTIME_PROVIDER", route.runtime_provider_key());
        }
        ...
        mona_base::auth::AuthStatus::invalidate_cached_status();
        Ok(())
    }
    ...
    fn format_messages(&self, messages: &[Message], is_oauth: bool) -> Vec<ApiMessage> {
        mona_provider_anthropic::format_messages(messages, is_oauth)
    }
    ...
    fn format_content_blocks(
        ...
        mona_provider_anthropic::format_content_blocks(blocks, is_oauth)
    }
    ...
    fn format_tools(&self, tools: &[ToolDefinition], is_oauth: bool) -> Vec<ApiTool> {
        mona_provider_anthropic::format_tools(tools, is_oauth, is_cache_ttl_1h())
    }
}
...
fn log_anthropic_canonical_input(
    ...
    mona_provider_core::fingerprint::log_provider_canonical_input(
        "anthropic",
        model,
        format,
        &payload,
...
impl Provider for AnthropicProvider {
    ...
    fn credential_mode(&self) -> mona_provider_core::CredentialMode {
        self.credential_mode_snapshot()
    }

    fn set_credential_mode(&self, mode: mona_provider_core::CredentialMode) -> Result<()> {
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        ...
        let model: &str = if is_1m_model(model)
            && matches!(
                mona_provider_core::anthropic_context_mode(model),
                mona_provider_core::AnthropicContextMode::Native1M
            ) {
        ...
        if !self
            ...
            && !mona_base::provider::known_anthropic_model_ids()
                .iter()
    ...
    fn available_models_for_switching(&self) -> Vec<String> {
        mona_base::provider::cached_anthropic_model_ids()
            .unwrap_or_else(mona_base::provider::known_anthropic_model_ids)
    }
    ...
    async fn prefetch_models(&self) -> Result<()> {
        ...
        let catalog = if is_oauth {
            match mona_base::provider::fetch_anthropic_model_catalog_oauth(&token).await {
        ...
        if !catalog.available_models.is_empty() {
            mona_base::provider::populate_anthropic_models(catalog.available_models);
        }
        Ok(())
    }
...
async fn run_stream_with_retries(
    ...
    for attempt in 0..MAX_RETRIES {
        if attempt > 0 {
            ...
            let delay = mona_provider_core::retry_after::retry_delay(
                attempt,
                RETRY_BASE_DELAY_MS,
            ...
            let _ = tx
                .send(Ok(StreamEvent::ConnectionPhase {
                    phase: mona_message_types::ConnectionPhase::Retrying {
                        attempt: attempt + 1,
                        max: MAX_RETRIES,
                    },
                }))
        ...
        let (attempt_tx, attempt_guard) =
            mona_provider_core::attempt_tracker::track_attempt_output(tx.clone());
        ...
        let attempt_client = if attempt == 0 {
            ...
            mona_provider_core::fresh_transport_client()
        };
            ...
            Err(e) => {
                ...
                if is_retryable_error(&error_str) && attempt + 1 < MAX_RETRIES {
                    if saw_output {
                        ...
                        mona_base::logging::info(&format!("Transient error, will retry: {}", e));
                    }
                    next_retry_delay = mona_provider_core::retry_after::retry_after_from_error(&e);
                    last_error = Some(e);
                    continue;
                }
...
async fn stream_response(
    ...
    if std::env::var("MONA_ANTHROPIC_DEBUG")
        .map(|v| v == "1")
        .unwrap_or(false)
        && let Ok(json) = serde_json::to_string_pretty(&request)
    {
        mona_base::logging::info(&format!("Anthropic request payload:\n{}", json));
    }
    ...
    let stream_idle_timeout = mona_base::provider::stream_idle_timeout();
        ...
        req = match direct_transport.auth_mode.as_str() {
            ...
            "header" => {
                let header = HeaderName::from_bytes(direct_transport.auth_header.trim().as_bytes())
                    .context("invalid MONA_ANTHROPIC_AUTH_HEADER")?;
                req.header(header, &token)
            }
            value => anyhow::bail!(
                "invalid MONA_ANTHROPIC_AUTH value '{value}' (expected bearer, header, or none)"
            ),
        };
        req = req.header("anthropic-beta", beta_header);
    }

    let response = mona_provider_core::transport::send_with_initial_response_timeout(
        req.json(&request),
        stream_idle_timeout,
    )
    .await
    ...
    if !response.status().is_success() {
        ...
        let retry_after = mona_provider_core::retry_after::retry_after(response.headers());
        let error_text = mona_base::util::http_error_body(response, "HTTP error").await;
        return Err(mona_provider_core::retry_after::error_with_retry_after(
    ...
    // `[provider] stream_idle_timeout_secs` / `MONA_STREAM_IDLE_TIMEOUT_SECS`
...
fn is_retryable_error(error_str: &str) -> bool {
    mona_provider_core::is_transient_transport_error(error_str)
        // Server errors (5xx)
...
fn anthropic_model_quality_rank(model: &str) -> usize {
    ...
    let normalized = mona_provider_core::model_id::strip_date_suffix(
        &mona_provider_core::model_id::canonical(model),
    )
    .to_string();
    mona_provider_core::ALL_CLAUDE_MODELS
        .iter()
        .position(|candidate| {
            mona_provider_core::model_id::strip_date_suffix(
                &mona_provider_core::model_id::canonical(candidate),
            ) == normalized
        })
        ...
        // available.
        .unwrap_or(mona_provider_core::ALL_CLAUDE_MODELS.len())
}
...
fn parse_sse_event(buffer: &mut String) -> Option<SseEvent> {
    ...
    for line in event_str.lines() {
        ...
        } else if let Some(rest) = mona_base::util::sse_data_line(line) {
            data = rest.to_string();
        }
    }
    ...
    match event.event_type.as_str() {
        "message_start" => {
            ...
            if let Ok(parsed) = serde_json::from_str::<MessageStartEvent>(&event.data) {
                ...
                // substitution (and surface it under MONA_LOG_SERVED_MODEL).
                if let Some(served) = parsed.message.model.as_deref() {
                    mona_base::logging::info(&format!("Anthropic served model={}", served));
                    if std::env::var("MONA_LOG_SERVED_MODEL").is_ok() {
                ...
                if let Some(usage) = parsed.message.usage {
                    ...
                    if let Some(tier) = usage.service_tier.as_deref() {
                        ...
                        if std::env::var("MONA_LOG_SERVICE_TIER").is_ok() {
        ...
        "ping" => {
            ...
            events.push(StreamEvent::ConnectionPhase {
                phase: mona_message_types::ConnectionPhase::Streaming,
            });
        }
        "error" => {
            mona_base::logging::error(&format!("Anthropic stream error: {}", event.data));
...
fn build_system_param(system: &str, is_oauth: bool) -> Option<ApiSystem> {
    mona_provider_anthropic::build_system_param(system, is_oauth, is_cache_ttl_1h())
}
...
fn build_system_param_split(
    ...
    mona_provider_anthropic::build_system_param_split(
        static_part,
        dynamic_part,
        is_oauth,
        is_cache_ttl_1h(),
...
fn format_messages_with_identity(messages: Vec<ApiMessage>, is_oauth: bool) -> Vec<ApiMessage> {
    mona_provider_anthropic::format_messages_with_identity(messages, is_oauth, is_cache_ttl_1h())
}

#[cfg(test)]
fn add_message_cache_breakpoint(messages: &mut [ApiMessage]) {
    mona_provider_anthropic::add_message_cache_breakpoint(messages, is_cache_ttl_1h())
}
