...
pub use mona_usage_types::{ModelUsage, compare_model_usage};
...
impl CredentialMode {
    /// Resolve the runtime-env pin (MONA_*_AUTH / route aliases) for a
...
pub const MONA_USER_AGENT: &str = concat!("jcode/", env!("CARGO_PKG_VERSION"));
...
pub struct ModelRoute {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub usage: Option<mona_usage_types::ModelUsage>,
}
...
impl RuntimeKey {
    ...
    pub fn stable_id(&self) -> String {
        match self {
            Self::JcodeSubscription => "mona-subscription".to_string(),
...
/// through. Mirrors `mona_base::provider::openrouter_catalog_model_id` but
...
impl ModelRouteApiMethod {
    ...
    pub fn parse(value: &str) -> Self {
        ...
        match lower.as_str() {
            "mona-subscription" => Self::JcodeSubscription,
...
mod tests {
    ...
    fn canonical_user_agent_identifies_jcode() {
        assert!(MONA_USER_AGENT.starts_with("jcode/"));
    }
