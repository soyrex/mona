...
//!
//! | concept              | runtime env (`MONA_RUNTIME_PROVIDER`) | route / stable-id    | CLI `--provider` | model prefix     |
//! |----------------------|----------------------------------------|----------------------|------------------|------------------|
...
impl AuthRoute {
    ...
    /// Canonical `MONA_RUNTIME_PROVIDER` value that pins this route.
...
/// Read `MONA_RUNTIME_PROVIDER` and return the dual-auth route it pins, if any.
pub fn runtime_env_auth_route() -> Option<AuthRoute> {
    let value = std::env::var("MONA_RUNTIME_PROVIDER").ok()?;
    AuthRoute::parse(&value)
}

/// Read `MONA_RUNTIME_PROVIDER` and resolve the dual-auth mode it pins for a
