...
pub struct NamedProviderConfig {
    ...
    /// Must be a JSON object; keys here override mona-generated body fields.
...
pub struct AgentsConfig {
    ...
    /// Unset = `text-embedding-3-small`. Env: `MONA_MEMORY_EMBEDDING_MODEL`.
    #[serde(default)]
    ...
    /// Env: `MONA_MEMORY_EMBEDDING_BASE_URL`.
    #[serde(default)]
...
/// pane by the built-in launcher. `MONA_TERMINAL` explicitly selects a terminal
/// emulator instead.
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
#[serde(default)]
pub struct TerminalConfig {
    ...
    /// opening a terminal emulator itself, with `MONA_SPAWN_*` metadata env
    ...
    /// `MONA_FOCUS_SESSION_ID` and `MONA_FOCUS_TITLE` env vars. Pair this
    ...
    /// `~/.mona/preferred_terminal.json` file. Re-run `jcode setup-hotkey`
...
/// with `MONA_HOOK_*` env vars describing the event (`MONA_HOOK_EVENT`,
/// `MONA_HOOK_SESSION_ID`, `MONA_HOOK_CWD`, event-specific fields, and a
/// `MONA_HOOK_PAYLOAD` JSON mirror). Hook processes get
/// `MONA_HOOKS_DISABLED=1` so nested jcode invocations don't recurse.
///
...
pub struct HooksConfig {
    ...
    /// ("chat"/"resume"/"ambient"). Env override: MONA_HOOK_TURN_START.
    ...
    /// Env override: MONA_HOOK_PRE_TOOL.
...
pub enum WebSearchEngine {
    ...
    /// `MONA_SEARXNG_URL` env var) to point at a SearXNG instance. Useful on
...
impl Default for WebSearchConfig {
    fn default() -> Self {
        Self {
            ...
            bing_api_key: None,
            bing_api_key_env: "MONA_BING_API_KEY".to_string(),
            bing_market: "en-US".to_string(),
            searxng_url: None,
            searxng_url_env: "MONA_SEARXNG_URL".to_string(),
        }
    }
}
...
pub struct ProviderConfig {
    ...
    /// key's project. `MONA_GEMINI_FORCE_OAUTH` overrides this value.
    ...
    /// automatically (see `mona_base::provider::stream_idle_timeout_for_effort`).
    /// Default: 180. Overridable via `MONA_STREAM_IDLE_TIMEOUT_SECS`.
    pub stream_idle_timeout_secs: u64,
    /// Maximum request attempts for transient provider errors, including the
    /// initial attempt. Default: 8. Overridable via `MONA_MAX_RETRIES`.
    pub max_retries: u32,
    /// Maximum exponential-backoff delay between transient-error retries.
    /// Default: 30 seconds. Overridable via `MONA_RETRY_BACKOFF_CAP_SECS`.
...
pub struct SafetyConfig {
    ...
    /// SMTP password (prefer MONA_SMTP_PASSWORD env var)
    ...
    /// Jade relay bearer token (prefer MONA_JADE_RELAY_TOKEN env var)
...
pub struct PowerConfig {
    ...
    ///
    /// Honored by the shared `jcode serve` daemon. The `MONA_DISABLE_POWER_INHIBIT`
...
pub struct LaunchHotkeyEntry {
    /// mona-style chord string, e.g. `cmd+;`, `cmd+[`, `cmd+shift+'`.
    pub chord: String,
