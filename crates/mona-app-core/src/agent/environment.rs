use super::{Agent, MONA_REPO_SOURCE_STATE, WORKING_GIT_STATE_CACHE};
use crate::logging;
...
impl Agent {
    ...
    pub(super) fn build_env_snapshot(
        ...
        let (mona_git_hash, mona_git_dirty) = match detail {
            EnvSnapshotDetail::Full => MONA_REPO_SOURCE_STATE.clone(),
        ...
        EnvSnapshot {
            ...
            mona_version: mona_build_meta::version().to_string(),
            mona_git_hash,
            mona_git_dirty,
...
mod tests {
    ...
    async fn log_context_names_the_profile_not_the_transport_slot() {
        ...
        // no `MONA_HOME`, and it must not set one either: the crate's tests
        // run in parallel.
