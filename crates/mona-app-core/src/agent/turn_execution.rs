...
impl Agent {
    ...
    /// Self-dev agents use the working tree rather than bundled `mona_docs`,
    ...
    fn apply_selfdev_tool_surface(
        ...
        if is_desktop {
            tools.retain(|tool| {
                !matches!(
                    tool.name.as_str(),
                    "selfdev" | "debug_socket" | "mona_docs"
                )
            });
            return;
        }
        ...
        tools.retain(|tool| tool.name != "mona_docs");
    ...
    pub(super) fn validate_tool_allowed(&self, name: &str) -> Result<()> {
        ...
        if (self.session.is_canary || is_desktop) && name == "mona_docs" {
            return Err(anyhow::anyhow!(
                "Tool 'mona_docs' is disabled in self-development mode. Read the working tree documentation instead."
            ));
        }
