...
use chrono::Utc;
use mona_agent_runtime::{SoftInterruptMessage, SoftInterruptQueue, SoftInterruptSource};
use std::sync::Arc;
...
impl AmbientRunnerHandle {
    ...
    pub async fn log_json(&self) -> String {
        let transcripts_dir = match crate::storage::mona_dir() {
    ...
    async fn run_cycle(&self, provider: &Arc<dyn Provider>) -> anyhow::Result<AmbientCycleResult> {
        self.run_cycle_with_visible_launcher(provider, config().ambient.visible, || {
            let mona_bin =
                std::env::current_exe().unwrap_or_else(|_| std::path::PathBuf::from("mona"));
