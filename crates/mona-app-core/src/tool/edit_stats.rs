...
use mona_harness_api::SessionEditStats;
...
pub(super) async fn record(ctx: &ToolContext, old: &str, new: &str, approximate: bool) {
    ...
    let result = tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
        let dir = crate::storage::mona_dir()?.join("sessions");
        mona_harness_api::record_session_edit(&dir, &id, changed_lines(&old, &new, approximate))?;
        Ok(())
    })
    .await;
    match result {
        ...
        impl Drop for Home {
            fn drop(&mut self) {
                if let Some(old) = self.0.take() {
                    crate::env::set_var("MONA_HOME", old);
                } else {
                    crate::env::remove_var("MONA_HOME");
                }
            }
        }
        let _home = Home(std::env::var_os("MONA_HOME"));
