...
pub(super) fn destructive_command_refusal(
    ...
    let mut risk_ctx = mona_command_risk::RiskContext::from_env(working_dir);
    // Assess the same scratch path that the child shell actually receives.
    #[cfg(not(windows))]
    {
        risk_ctx.scratch_dir = super::tool_scratch_dir();
    }
    let assessment = mona_command_risk::assess(command, &risk_ctx);
    if assessment.level.runs_immediately() {
        return None;
    }

    let justification = mona_command_risk::Justification {
        text: justification.map(str::to_string),
    };
    match mona_command_risk::gate(&assessment, &justification) {
        mona_command_risk::GateOutcome::Allow => None,
        mona_command_risk::GateOutcome::Deny { reason } => {
            crate::logging::warn(&format!("[bash] denied destructive command: {command}"));
            Some(reason)
        }
        mona_command_risk::GateOutcome::Reflect { prompt } => {
...
pub(super) fn bash_parameters_schema() -> serde_json::Value {
    let cmd_desc = if cfg!(windows) {
        ...
        "The bash command to execute. Put large temp files under `$MONA_SCRATCH_DIR`, not `/tmp`."
    };
    serde_json::json!({
        "type": "object",
        "required": ["command"],
        "properties": {
            ...
            "run_in_background": {
                "type": "boolean",
                "description": "Run in background. Emit `MONA_PROGRESS {json}` lines for progress reporting."
            },
            "notify": {
                "type": "boolean",
...
mod tests {
    ...
    fn scratch_log_and_backup_commands_do_not_require_justification() {
        ...
        for command in [
            "cargo test --lib > \"$MONA_SCRATCH_DIR/tests.log\" 2>&1",
            "git diff > \"${MONA_SCRATCH_DIR}/before.patch\"",
            "env | grep JCODE",
