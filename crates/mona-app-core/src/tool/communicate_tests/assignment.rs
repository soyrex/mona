...
async fn communicate_assign_task_can_spawn_fallback_agent() {
    ...
    let _runtime = EnvGuard::set("MONA_RUNTIME_DIR", runtime_dir.path());
    let _socket = EnvGuard::set("MONA_SOCKET", &socket_path);
    let _debug = EnvGuard::set("MONA_DEBUG_CONTROL", "1");
