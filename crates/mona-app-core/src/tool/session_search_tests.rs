...
fn with_temp_home<T>(f: impl FnOnce(&Path) -> T) -> T {
    ...
    let previous_home = std::env::var("MONA_HOME").ok();
...
fn bench_real_session_search_corpus() {
    if std::env::var("MONA_SESSION_SEARCH_BENCH_REAL")
        .ok()
        .as_deref()
        != Some("1")
    {
        eprintln!("set MONA_SESSION_SEARCH_BENCH_REAL=1 to run against the real session corpus");
        return;
    }
    ...
    for query in ["session_search", "optimization", "nonexistentneedle123"] {
        ...
        eprintln!(
            ...
            report.scanned_mona_sessions,
            report.candidate_mona_sessions,
            report.results.len(),
            report.truncated
        );
    }
...
#[test]
fn external_codex_sessions_are_searchable_without_mona_session_dir() {
    with_temp_home(|home| {
        ...
        assert_eq!(report.scanned_mona_sessions, 0);
...
#[test]
fn external_cursor_sessions_are_searchable_without_mona_session_dir() {
    with_temp_home(|home| {
