...
//! run explicitly:
//!   cargo test -p mona-app-core tool::computer::coverage -- --ignored --nocapture --test-threads=1
//!
...
use super::*;
use mona_tool_core::{ToolContext, ToolExecutionMode};
...
async fn coverage_clipboard_scripting_system() {
    ...
    ok(json!({"action":"notify","text":"jcode coverage test","title":"mona"})).await;
