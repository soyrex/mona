...
fn builds_both_desktop_packages_with_matching_profile() {
    for (profile, suffix) in [
        ...
        let mut expected = vec!["build", "-p", "mona-desktop", "-p", "mona-desktop-ui"];
        expected.extend(suffix);
        assert_eq!(spec.args, expected);
        assert!(!spec.args.iter().any(|a| a == "mona" || a == "self-dev"));
    }
}
...
fn test_and_inspection_commands_are_scoped_and_read_only() {
    ...
    custom.command = Some("cargo test -p mona-desktop-ui".into());
    let spec = command_spec(root, &custom, "debug", None).unwrap();
    assert_eq!(spec.program, "bash");
    assert_eq!(spec.args, ["-c", "cargo test -p mona-desktop-ui"]);
...
fn ambiguous_instances_and_foreign_executables_are_rejected() {
    ...
    let root = Path::new("/desktop");
    assert_eq!(
        host_profile(root, Path::new("/desktop/target/release/mona-desktop")).unwrap(),
        "release"
    );
    for exe in [
        "/other/target/debug/mona-desktop",
        "/desktop/target/debug/jcode",
        "/desktop/target/debug/deps/mona-desktop",
        "/desktop/target/../mona-desktop",
    ] {
...
async fn custom_test_executes_from_detected_repo_root() {
    ...
    let output = DesktopSelfDevTool::new()
        .execute(
            ...
            context(Some(root.path().join("crates/mona-desktop-ui/src"))),
        )
        .await
        .unwrap();
