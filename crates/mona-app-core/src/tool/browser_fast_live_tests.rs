...
async fn live_browser_handoff_completes_local_navigation() {
    ...
    let tab_id =
        local_fixture_tab_at_path("MONA_BROWSER_HANDOFF_TEST_TAB_ID", &ctx, Some("/")).await;
    let result = handoff(
        tab_id,
...
async fn live_browser_handoff_completes_search_and_nested_navigation() {
    let ctx = fixture_context();
    let tab_id =
        local_fixture_tab_at_path("MONA_BROWSER_HANDOFF_TEST_TAB_ID", &ctx, Some("/task")).await;
...
async fn live_browser_handoff_sensitive_fixture_hands_back_without_actions() {
    let ctx = fixture_context();
    let tab_id = local_fixture_tab("MONA_BROWSER_HANDOFF_TEST_BLOCKED_TAB_ID", &ctx).await;
    let result = handoff(
        tab_id,
...
async fn live_browser_handoff_requests_script_and_resumes() {
    ...
    let ctx = fixture_context();
    let tab_id = local_fixture_tab("MONA_BROWSER_HANDOFF_TEST_TAB_ID", &ctx).await;
    ...
    let ctx = fixture_context();
    assert_eq!(
        local_fixture_tab("MONA_BROWSER_HANDOFF_TEST_TAB_ID", &ctx).await,
        tab_id
    );
