...
// Stdio protocol fixture shared in spirit with mona-base/tests/mcp_stdio_collision_integration.rs.
const SERVER: &str = r#"
import json, os, sys
...
fn mcp_collision_manual_lifecycle_real_stdio() {
    const MARKER: &str = "MONA_MCP_MANAGEMENT_TEST_CHILD";
    if std::env::var_os(MARKER).is_none() {
        ...
        let status = child
            .env(MARKER, "1")
            .env("MONA_HOME", home.path().join("mona"))
            .env("MONA_RUNTIME_DIR", home.path().join("runtime"))
