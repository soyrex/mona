...
pub fn spawn_recent_index_warmup() {
    tokio::task::spawn_blocking(|| {
        ...
        remove_legacy_index();
        let empty_query = QueryProfile::new("__mona_index_warmup__");

        let mona_count = (|| -> Result<usize> {
            let sessions_dir = storage::mona_dir()?.join("sessions");
            let collection = collect_session_files(&sessions_dir, DEFAULT_MAX_SCAN_SESSIONS)?;
            if collection.files.is_empty() {
                return Ok(0);
            }
            let _ = mona_index_candidates(&collection.files, &empty_query)?;
        ...
        crate::logging::info(&format!(
            "Session search index warmup completed for {mona_count} jcode + {external_count} external session(s) in {}ms",
    ...
    fn parameters_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "source": {
                    "type": "string",
                    "enum": ["all", "mona", "claude", "codex", "pi", "opencode", "cursor"],
...
fn normalize_source_filter(raw: Option<&str>) -> std::result::Result<Option<String>, String> {
    ...
    match normalized.as_str() {
        "all" => Ok(None),
        "mona" | "claude" | "claude-code" | "codex" | "pi" | "opencode" | "cursor" => {
...
fn search_sessions_blocking(
    ...
    if source_matches_filter("mona", options) {
        ...
        if !files.is_empty() {
            ...
            report.scanned_mona_sessions = files.len();
            ...
            if !files.is_empty() {
                ...
                let mut candidates = if options.exhaustive {
                    ...
                    match mona_index_candidates(&files, query) {
                ...
                report.candidate_mona_sessions = candidates.len();
                if using_index {
...
fn index_dir() -> Result<PathBuf> {
    Ok(storage::mona_dir()?.join("cache"))
}
...
fn mona_index_candidates(
    files: &[SessionFileCandidate],
    query: &QueryProfile,
) -> Result<Vec<SessionFileCandidate>> {
    let index_path = index_dir()?.join("session_search_mona_index_v2.bin");
...
fn claude_index_candidates(
    sessions: &[mona_import_core::ClaudeCodeSessionInfo],
    query: &QueryProfile,
) -> Vec<mona_import_core::ClaudeCodeSessionInfo> {
    let index = index_dir()
                ...
                for session in chunk {
                    ...
                    records.push(ExternalSessionRecord {
                        ...
                        short_name: Some(format!(
                            "claude {}",
                            mona_core::util::truncate_str(&session.session_id, 8)
                        )),
                        title: Some(title),
...
fn external_text_matches_query(text: &str, query: &QueryProfile) -> bool {
    mona_session_types::normalized_session_search_text_matches(&text.to_lowercase(), query)
}
...
fn append_session_results(
    ...
    if !mona_session_matches_filters(session, options) {
        return;
    }
    ...
    for (message_index, msg) in session.messages.iter().enumerate() {
        ...
        results.push(SearchResult {
            ...
            matched_terms: match_score.matched_terms,
            exact_match: match_score.exact_match,
            context: build_mona_context(&session.messages, message_index, options),
        });
    }
}
...
fn mona_session_matches_filters(session: &Session, options: &SearchOptions) -> bool {
    if !source_matches_filter("mona", options) {
        return false;
    }
    if !provider_matches(session.provider_key.as_deref(), "mona", options) {
        return false;
    }
