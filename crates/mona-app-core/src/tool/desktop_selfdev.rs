...
fn desktop_root(cwd: Option<&Path>) -> Result<PathBuf> {
    cwd.and_then(mona_selfdev_types::desktop_repo_root)
...
fn command_spec(
    ...
    let (program, args, note) = match input.action.as_str() {
        "build" => {
            let mut args = vec!["build", "-p", "mona-desktop", "-p", "mona-desktop-ui"]
                .into_iter()
                .map(String::from)
                .collect::<Vec<_>>();
            if profile == "release" {
...
fn host_profile(root: &Path, exe: &Path) -> Result<String> {
    ...
    if parts.len() != 2 || parts[1].as_os_str() != "mona-desktop" {
        bail!(
