...
fn browser_handoff_disabled() -> bool {
    std::env::var("MONA_BROWSER_HANDOFF_DISABLED").is_ok_and(|value| value == "1")
}
...
impl Tool for BrowserTool {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        if params.action == "handoff" && browser_handoff_disabled() {
            anyhow::bail!(
                "Browser handoff is disabled by MONA_BROWSER_HANDOFF_DISABLED=1. Use direct browser actions instead."
            );
        }
...
fn temp_screenshot_path() -> PathBuf {
    let ts = SystemTime::now()
        ...
        .map(|d| d.as_millis())
        .unwrap_or(0);
    std::env::temp_dir().join(format!("mona-browser-{}.png", ts))
}
