...
const DISCOVERY_REQUEST_ID_HEADER: &str = "x-mona-discovery-request-id";
const DISCOVERY_CORRELATION_ID_HEADER: &str = "x-mona-session-correlation-id";
const DISCOVERY_BENCHMARK_HEADER: &str = "x-mona-discovery-benchmark";
const DISCOVERY_SESSION_ID_HEADER: &str = "x-mona-discovery-session-id";
const DISCOVERY_SESSION_METADATA_HEADER: &str = "x-mona-discovery-session-metadata";
const DISCOVERY_SELF_DEV_HEADER: &str = "x-mona-discovery-self-dev";
const DISCOVERY_DEBUG_HEADER: &str = "x-mona-discovery-debug";
const DISCOVERY_CANARY_HEADER: &str = "x-mona-discovery-canary";
const DISCOVERY_EXECUTION_MODE_HEADER: &str = "x-mona-discovery-execution-mode";
const DISCOVERY_BUILD_CHANNEL_HEADER: &str = "x-mona-discovery-build-channel";
const DISCOVERY_GIT_CHECKOUT_HEADER: &str = "x-mona-discovery-git-checkout";
const DISCOVERY_CI_HEADER: &str = "x-mona-discovery-ci";
const DISCOVERY_RAN_FROM_CARGO_HEADER: &str = "x-mona-discovery-ran-from-cargo";
const DISCOVERY_BENCHMARK_ENV: &str = "MONA_DISCOVERY_BENCHMARK";
...
impl Tool for DiscoverToolsTool {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        // metered coarsely; see mona_base::sponsors::provenance).
...
mod tests {
    ...
    fn agentmail_selection_preserves_signup_attribution_and_mcp_provenance() {
        let listing = json!({
            ...
            "tool": {
                ...
                "url": "https://www.agentmail.to/?via=mona-discovery",
                "setup": concat!(
    ...
    async fn fetch_listing_round_trips_and_sends_only_expected_params() {
        ...
        assert!(
            request
                .to_ascii_lowercase()
                .contains("x-mona-discovery-request-id: request-test-1"),
            "{request}"
        );
        assert!(
            request
                .to_ascii_lowercase()
                .contains("x-mona-discovery-benchmark: 1"),
            "{request}"
        );
        for expected in [
            "x-mona-discovery-session-id: session-test-1",
            "x-mona-session-correlation-id: aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee",
            "x-mona-discovery-session-metadata: 1",
            "x-mona-discovery-self-dev: 1",
            "x-mona-discovery-debug: 0",
            "x-mona-discovery-canary: 1",
            "x-mona-discovery-execution-mode: agent_turn",
            "x-mona-discovery-build-channel: selfdev",
            "x-mona-discovery-git-checkout: 1",
            "x-mona-discovery-ci: 0",
            "x-mona-discovery-ran-from-cargo: 1",
        ] {
    ...
    async fn submit_suggestion_posts_structured_maintainer_only_payload() {
        ...
        assert!(
            lower.contains("x-mona-discovery-request-id: aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee"),
            "{request}"
        );
        assert!(
            lower.contains("x-mona-discovery-benchmark: 1"),
            "{request}"
        );
