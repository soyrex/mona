...
impl SelfDevTool {
        ...
        match &repo_dir {
            ...
            None => {
                let target = Self::selfdev_clone_dir()
                    ...
                    .unwrap_or_else(|_| "~/.mona/source/jcode".to_string());
    ...
    pub(super) async fn do_find_config(&self, ctx: &ToolContext) -> Result<ToolOutput> {
        let mona_home = storage::mona_dir().ok();
        let config_path = mona_home.as_ref().map(|home| home.join("config.toml"));
        ...
        output.push_str(&format!(
            "**jcode home:** {}\n",
            format_path(mona_home.as_deref())
        ));
        output.push_str(&format!(
            "**Logs dir:** {}\n",
        ...
        let metadata = json!({
            ...
            "mona_home": mona_home.as_ref().map(|p| p.display().to_string()),
    ...
    pub(super) async fn do_reload_to_newer_build(&self, _ctx: &ToolContext) -> Result<ToolOutput> {
        ...
        let hash = mona_build_meta::git_hash().to_string();
    ...
    fn selfdev_clone_dir() -> Result<std::path::PathBuf> {
        Ok(storage::mona_dir()?.join("source").join("mona"))
    }
    ...
    fn clone_selfdev_source() -> Result<std::path::PathBuf> {
        ...
        if repo_dir.exists() {
            if build::is_mona_repo(&repo_dir) {
                return Ok(repo_dir);
            }
            anyhow::bail!(
