...
pub const MONA_REPO_URL: &str = "https://github.com/1jehuang/jcode.git";
...
impl BuildRequest {
    ...
    fn requests_dir() -> Result<PathBuf> {
        let dir = storage::mona_dir()?.join("selfdev-build-requests");
    ...
    fn terminal_history_limit() -> usize {
        std::env::var("MONA_SELFDEV_REQUEST_HISTORY_LIMIT")
            .ok()
...
impl SelfDevTool {
    ...
    fn reload_timeout_secs() -> u64 {
        std::env::var("MONA_SELFDEV_RELOAD_TIMEOUT_SECS")
            .ok()
    ...
    fn build_reload_wait_secs() -> u64 {
        std::env::var("MONA_SELFDEV_BUILD_WAIT_SECS")
            .ok()
            .and_then(|raw| raw.trim().parse::<u64>().ok())
            .filter(|secs| *secs > 0)
            .unwrap_or(1800)
    ...
    fn resolve_repo_dir(working_dir: Option<&std::path::Path>) -> Option<std::path::PathBuf> {
        if let Some(dir) = working_dir {
            for ancestor in dir.ancestors() {
                if build::is_mona_repo(ancestor) {
    ...
    fn build_lock_path(worktree_scope: &str) -> Result<PathBuf> {
        let dir = storage::mona_dir()?.join("selfdev-build-locks");
        storage::ensure_dir(&dir)?;
