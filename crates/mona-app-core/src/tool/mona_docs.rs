...
include!(concat!(env!("OUT_DIR"), "/mona_docs.rs"));
...
fn terms(query: &str) -> Vec<String> {
    let stop: HashSet<&str> = [
        "a", "an", "and", "about", "does", "for", "how", "i", "in", "is", "mona", "of", "on",
        "the", "to", "what", "with",
    ]
    .into_iter()
    .collect();
...
fn sections() -> Vec<Section<'static>> {
    ...
    for (path, document) in MONA_DOCS {
...
mod tests {
    ...
    fn corpus_includes_current_docs_but_not_plans() {
        assert!(MONA_DOCS.iter().any(|(path, _)| *path == "README.md"));
        assert!(MONA_DOCS.iter().any(|(path, _)| *path == "docs/README.md"));
        assert!(
            !MONA_DOCS
                .iter()
