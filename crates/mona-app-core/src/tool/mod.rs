...
pub(crate) use mona_tool_core::intent_schema_property;
pub use mona_tool_core::{StdinInputRequest, Tool, ToolContext, ToolExecutionMode};
pub use mona_tool_types::{ToolImage, ToolOutput};
...
pub(crate) fn accept_large_output_schema_property_for_test() -> Value {
    mona_tool_core::accept_large_output_schema_property()
}
...
fn accepts_large_output(input: &Value) -> bool {
    ...
    match input.get(mona_tool_core::ACCEPT_LARGE_OUTPUT_KEY) {
...
impl Registry {
    ...
    /// correctly.
    ///
    /// The canonical mapping lives in `mona-tool-types::resolve_tool_name` so
    ...
    pub(crate) fn resolve_tool_name(name: &str) -> &str {
        mona_tool_types::resolve_tool_name(name)
    }
    ...
    pub async fn execute(&self, name: &str, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        if matches!(
            resolved_name,
            "selfdev" | "debug_socket" | "desktop_selfdev" | "mona_docs"
        ) {
            let desktop = ctx
                .working_dir
                .as_deref()
                ...
                .is_some();
            if desktop && resolved_name == "mona_docs" {
                anyhow::bail!(
                    "Tool 'mona_docs' is disabled in Desktop self-development mode. Read the working tree documentation instead."
                );
            }
