...
fn repository_commands_export_a_logged_cargo_function() {
    ...
    assert!(wrapped.contains("export MONA_DEV_CARGO_SCRIPT="));
    assert!(wrapped.contains("MONA_IN_DEV_CARGO=1 \"$MONA_DEV_CARGO_SCRIPT\" \"$@\""));
    assert!(wrapped.contains("export -f cargo"));
    assert!(wrapped.ends_with("cargo test -p demo && echo done"));
}

#[test]
fn cargo_routing_is_limited_to_the_mona_repository() {
...
fn make_agent_ctx(signal: mona_agent_runtime::InterruptSignal) -> ToolContext {
    ToolContext {
        session_id: "test-session".to_string(),
        message_id: "test-msg".to_string(),
...
async fn test_reload_persistable_bash_continues_in_background() {
    let tool = BashTool::new();
    let signal = mona_agent_runtime::InterruptSignal::new();
...
fn test_parse_progress_marker_handles_percent_payloads() {
    let progress = parse_progress_marker(
        r#"MONA_PROGRESS {"percent":25,"message":"Downloading dependencies"}"#,
    )
...
async fn test_background_command_progress_marker_updates_status_and_stays_out_of_output() {
    ...
    let result = tool
            .execute(
                json!({
                    "command": "printf '%s\n' 'MONA_PROGRESS {\"current\":3,\"total\":10,\"unit\":\"steps\",\"message\":\"Building\"}'; sleep 0.1; echo done",
...
fn test_bash_tool_schema_advertises_background_progress_guidance() {
    ...
    assert!(
        command_description.contains("MONA_SCRATCH_DIR"),
        "command description should keep the scratch-dir guidance"
    );
    assert!(
        background_description.contains("MONA_PROGRESS"),
...
//
// The unit-level policy is covered in mona-command-risk. These tests pin the
...
fn parse_progress_line_classifies_markers_checkpoints_and_heuristics() {
    let update = parse_progress_line(r#"MONA_PROGRESS {"percent":40,"message":"Working"}"#)
    ...
    let update = parse_progress_line(r#"MONA_CHECKPOINT {"message":"Tests passed"}"#)
        .expect("parser should not fail")
