...
use mona_swarm_core::validate_swarm_tldr;
...
impl RunPlanReporter {
    ...
    /// Record an explicit checkpoint (a MONA_CHECKPOINT-style milestone) on
...
fn plan_status_budget_line(summary: &PlanGraphStatus, deep_cap: usize) -> Option<String> {
    ...
    let budget_label = if budget == usize::MAX {
        format!("{} (member cap)", mona_swarm_core::MAX_SWARM_MEMBERS)
    } else {
        budget.to_string()
    };
    ...
    let effective_budget = if budget == usize::MAX {
        mona_swarm_core::MAX_SWARM_MEMBERS
    } else {
        budget
    };
...
fn format_swarm_model_list(
    ...
    model_routes: &[mona_provider_core::ModelRoute],
) -> String {
...
impl CommunicateTool {
    ...
    fn new_for_working_dir(working_dir: Option<&std::path::Path>) -> Self {
        ...
        let description = if swarm_prompt.is_empty() {
            ...
            format!(
                "{BASE_DESCRIPTION}\n\nSwarm prompt (user-tunable via ~/.mona/swarm-prompt.md):\n{swarm_prompt}"
            )
        };
        Self { description }
    }
