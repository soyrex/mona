    ...
    async fn list_servers(&self) -> Result<ToolOutput> {
        ...
        if servers.is_empty() && configured.is_empty() {
            return Ok(ToolOutput::new(
                ...
                Or add servers to ~/.mona/mcp.json or .jcode/mcp.json and use {\"action\": \"reload\"}.\n\
    ...
    async fn reload_config(&self, session_id: &str) -> Result<ToolOutput> {
        ...
        if config.servers.is_empty() {
            ...
            return Ok(ToolOutput::new(
                ...
                Add servers to ~/.mona/mcp.json (global) or .jcode/mcp.json (project):\n\
...
mod tests {
    ...
    fn create_test_tool() -> McpManagementTool {
        ...
        // would load the developer's real ~/.mona/mcp.json, and list output
    ...
    fn mcp_call_allows_dynamic_argument_keys_in_provider_schemas() {
        ...
        for spec in [
            &mona_schema_dialect::registry::OPENROUTER,
            &mona_schema_dialect::registry::OPENAI,
            &mona_schema_dialect::registry::ANTHROPIC,
        ] {
    ...
    fn mcp_call_dynamic_arguments_remain_ineligible_for_openai_strict_mode() {
        let tool = McpCallTool::new(Arc::clone(create_test_tool().manager()));
        let compatible =
            mona_provider_core::openai_schema::openai_compatible_schema(&tool.parameters_schema());
        assert!(!mona_provider_core::openai_schema::schema_supports_strict(
            &compatible
        ));
        assert_eq!(
