...
async fn mpsc_preserves_signatures_and_never_rebinds_to_provider_session_id() {
    ...
    impl Drop for RestoreHome {
        fn drop(&mut self) {
            if let Some(home) = &self.0 {
                crate::env::set_var("MONA_HOME", home);
            } else {
    ...
    let _restore = RestoreHome(std::env::var_os("MONA_HOME"));
    ...
    let mona_id = agent.session_id().to_string();
    ...
    assert_eq!(agent.session_id(), mona_id);
    let saved = Session::load(&mona_id).unwrap();
    assert_eq!(
...
async fn gmail_is_exposed_by_default_and_can_be_explicitly_disabled() {
    ...
    let prev_tools = std::env::var_os("MONA_TOOLS");
    let prev_disabled_tools = std::env::var_os("MONA_DISABLED_TOOLS");
    let prev_tool_profile = std::env::var_os("MONA_TOOL_PROFILE");
    let prev_disable_base_tools = std::env::var_os("MONA_DISABLE_BASE_TOOLS");
    let temp_home = tempfile::TempDir::new().expect("temp home");

    crate::env::set_var("MONA_HOME", temp_home.path());
    crate::env::remove_var("MONA_TOOLS");
    crate::env::remove_var("MONA_DISABLED_TOOLS");
    crate::env::remove_var("MONA_TOOL_PROFILE");
    crate::env::remove_var("MONA_DISABLE_BASE_TOOLS");
    crate::config::Config::invalidate_cache();
    ...
    assert!(
        tool_names.iter().any(|name| name == "mona_docs"),
        "mona_docs must be model-visible in regular sessions"
    );
    assert!(
    ...
    agent
        .validate_tool_allowed("mona_docs")
        .expect("mona_docs must be executable in regular sessions");
    agent.set_canary("docs-tool-regression");
    let definitions = agent.tool_definitions().await;
    assert!(definitions.iter().any(|tool| tool.name == "selfdev"));
    assert!(
        !definitions.iter().any(|tool| tool.name == "mona_docs"),
        "mona_docs must not be model-visible in self-dev sessions"
    );
    assert!(
        !agent
            .tool_definitions()
    ...
    assert!(
        !agent
            ...
            .await
            .iter()
            .any(|name| name == "mona_docs"),
        "debug tool introspection must agree with provider definitions"
    );
    assert!(
        agent
            .execute_tool("mona_docs", serde_json::json!({"action": "list"}))
            .await
            .is_err(),
    ...
    assert!(
        agent
            .validate_tool_allowed("mona_docs")
            .expect_err("mona_docs must not be executable in self-dev sessions")
            .to_string()
    ...
    assert!(
        agent
            ...
            .iter()
            .any(|tool| tool.name == "mona_docs"),
        "mona_docs must remain available after leaving self-dev mode"
    );
    agent
        .validate_tool_allowed("mona_docs")
        .expect("mona_docs must be executable again outside self-dev mode");

    crate::env::set_var("MONA_DISABLED_TOOLS", tool_name);
    crate::config::Config::invalidate_cache();
    ...
    if let Some(previous) = prev_home {
        crate::env::set_var("MONA_HOME", previous);
    } else {
        crate::env::remove_var("MONA_HOME");
    }
    if let Some(previous) = prev_tools {
        crate::env::set_var("MONA_TOOLS", previous);
    } else {
        crate::env::remove_var("MONA_TOOLS");
    }
    if let Some(previous) = prev_disabled_tools {
        crate::env::set_var("MONA_DISABLED_TOOLS", previous);
    } else {
        crate::env::remove_var("MONA_DISABLED_TOOLS");
    }
    if let Some(previous) = prev_tool_profile {
        crate::env::set_var("MONA_TOOL_PROFILE", previous);
    } else {
        crate::env::remove_var("MONA_TOOL_PROFILE");
    }
    if let Some(previous) = prev_disable_base_tools {
        crate::env::set_var("MONA_DISABLE_BASE_TOOLS", previous);
    } else {
        crate::env::remove_var("MONA_DISABLE_BASE_TOOLS");
    }
    crate::config::Config::invalidate_cache();
...
async fn build_memory_prompt_nonblocking_defers_pending_memory_during_tool_loop() {
    ...
    let _restore = RestoreMemoryHome(std::env::var_os("MONA_HOME"));
    crate::env::set_var("MONA_HOME", home.path());
    crate::config::Config::invalidate_cache();
    crate::memory::clear_all_pending_memory();
...
async fn memory_injection_message_defaults_to_ephemeral_history() {
    let _guard = crate::storage::lock_test_env();
    let previous = std::env::var_os("MONA_PERSIST_MEMORY_INJECTIONS");
    crate::env::set_var("MONA_PERSIST_MEMORY_INJECTIONS", "false");
    ...
    match previous {
        Some(value) => crate::env::set_var("MONA_PERSIST_MEMORY_INJECTIONS", value),
        None => crate::env::remove_var("MONA_PERSIST_MEMORY_INJECTIONS"),
    }
    crate::config::invalidate_config_cache();
}
...
async fn memory_injection_message_can_persist_to_history() {
    let _guard = crate::storage::lock_test_env();
    let previous = std::env::var_os("MONA_PERSIST_MEMORY_INJECTIONS");
    crate::env::set_var("MONA_PERSIST_MEMORY_INJECTIONS", "true");
    crate::config::invalidate_config_cache();
...
async fn env_snapshot_detail_is_minimal_for_empty_sessions_and_full_after_history() {
    ...
    assert!(minimal.mona_git_hash.is_none());
    assert!(minimal.mona_git_dirty.is_none());
