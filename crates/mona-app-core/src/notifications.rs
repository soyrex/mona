...
use mona_notify_email::{
    ReplyAction, SendEmailRequest, build_permission_email_html, poll_imap_once, send_email,
};
pub use mona_notify_email::{extract_permission_id, parse_permission_reply};
...
#[cfg(target_os = "macos")]
const MACOS_NOTIFICATION_BROKER_EXECUTABLE: &str = "mona-notification-broker";
...
fn macos_notification_broker_app_path() -> Option<std::path::PathBuf> {
    if let Some(path) = std::env::var_os("MONA_MACOS_NOTIFICATION_BROKER_APP") {
...
pub fn macos_notification_inbox_dir() -> Option<std::path::PathBuf> {
    if let Some(path) = std::env::var_os("MONA_MACOS_NOTIFICATION_INBOX") {
        return Some(path.into());
    }
    dirs::home_dir().map(|home| {
        home.join(".jcode")
...
fn reap_notification_child(mut child: std::process::Child) {
    let _ = std::thread::Builder::new()
        .name("mona-notification-child".to_string())
        .spawn(move || {
            let _ = child.wait();
        });
}
...
mod tests {
    ...
    fn macos_envelope_roundtrip_preserves_origin_metadata() {
        let envelope = MacosNotificationEnvelope {
            schema_version: MACOS_NOTIFICATION_SCHEMA_VERSION,
            notification_id: "mona-turn-test".to_string(),
