...
//! message, memory, telemetry, ...) lives in the `mona-base` crate and is
//! re-exported here via `pub use mona_base::*`, so every existing
...
//! turn re-exports this crate via `pub use mona_app_core::*`.

// Foundational layer: re-export every `mona-base` module so `crate::<module>`
