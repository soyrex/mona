...
fn sibling_socket_path_roundtrip() {
    ...
    let debug = std::path::PathBuf::from("/tmp/mona-debug.sock");
...
fn cleanup_socket_pair_removes_main_and_debug_files() {
    ...
    let main = dir.join(format!("mona-test-{}.sock", stamp));
    let debug = dir.join(format!("mona-test-{}-debug.sock", stamp));
...
async fn reap_stale_socket_removes_dead_socket_pair_and_lock() {
    ...
    let debug = temp.path().join("mona-debug.sock");
