...
pub(crate) fn debug_control_allowed() -> bool {
    ...
    if std::env::var("MONA_DEBUG_CONTROL")
        .ok()
    ...
    if let Ok(mona_dir) = crate::storage::mona_dir()
        && mona_dir.join("debug_control").exists()
    {
        return true;
    }
    false
...
pub(crate) fn embedding_idle_unload_secs() -> u64 {
    parse_embedding_idle_unload_secs(
        std::env::var("MONA_EMBEDDING_IDLE_UNLOAD_SECS")
            .ok()
            .as_deref(),
    )
}
...
pub(crate) fn swarm_id_for_dir(dir: Option<PathBuf>) -> Option<String> {
    if let Ok(sw_id) = std::env::var("MONA_SWARM_ID") {
...
/// default. `MONA_SWARM_ID` remains an explicit opt-in to a shared swarm.
...
pub(crate) fn startup_headless_recovery_test_delay() -> Option<std::time::Duration> {
    let raw = std::env::var("MONA_TEST_HEADLESS_STARTUP_RECOVERY_DELAY_MS").ok()?;
...
mod newest_reload_candidate_integration_tests {
    ...
    //! a temp `MONA_HOME`. This reproduces the field "/update -> new client,
    ...
    /// script plus the real `mona-linux-x86_64.bin` payload, with independently
    ...
    fn install_release_style_binary(
        ...
        let dir = build::builds_dir()
            ...
            .join(version);
        std::fs::create_dir_all(&dir).expect("create version dir");
        let payload = dir.join("mona-linux-x86_64.bin");
...
mod deleted_suffix_tests {
    ...
    fn strips_linux_deleted_marker() {
        let p = PathBuf::from("/home/u/.mona/builds/versions/abc/jcode (deleted)");
        assert_eq!(
            strip_deleted_suffix(p),
            PathBuf::from("/home/u/.mona/builds/versions/abc/jcode")
        );
    }

    #[test]
    fn leaves_normal_paths_untouched() {
        let p = PathBuf::from("/home/u/.mona/builds/versions/abc/jcode");
