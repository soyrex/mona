...
pub(super) async fn maybe_handle_server_state_command(
    ...
    shutdown_signals: &Arc<RwLock<HashMap<String, mona_agent_runtime::InterruptSignal>>>,
    ...
    if cmd == "memory-judge" || cmd == "memory:judge" || cmd == "server:memory-judge" {
        ...
        return Ok(Some(
            serde_json::to_string_pretty(&mona_base::memory_judge_metrics::snapshot())
                .unwrap_or_else(|_| "{}".to_string()),
        ));
    }
        ...
        "unattributed_live_heap" => (
            ...
            vec![
                ...
                serde_json::json!({
                    ...
                    "commands": ["jcode debug 'allocator:profile:on'", "jcode debug 'allocator:profile:dump /tmp/mona-server.heap'"]
                }),
            ],
        ),
