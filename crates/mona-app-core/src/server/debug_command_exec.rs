...
use anyhow::Result;
use mona_agent_runtime::{InterruptSignal, SoftInterruptSource};
...
pub(super) fn debug_message_timeout_secs() -> Option<u64> {
    let raw = std::env::var("MONA_DEBUG_MESSAGE_TIMEOUT_SECS").ok()?;
...
pub(super) async fn execute_debug_command(
    ...
    if trimmed == "mcp" || trimmed == "mcp:servers" {
        ...
        let config_path = if let Ok(mona_dir) = crate::storage::mona_dir() {
            let path = mona_dir.join("mcp.json");
            if path.exists() {
    ...
    if trimmed.starts_with("set_provider:") {
        ...
        let default_model = match provider.as_str() {
            ...
            "openai" | "codex" => mona_provider_core::DEFAULT_OPENAI_MODEL,
    ...
    if trimmed == "reload" {
        ...
        let mona_dir = crate::storage::mona_dir()?;
        let info_path = mona_dir.join("reload-info");
...
mod tests {
    ...
    use mona_agent_runtime::InterruptSignal;
    ...
    /// which set `MONA_HOME` under `storage::lock_test_env`). Two mutexes
    ...
    async fn debug_agent_context_inspects_modes_without_inference_or_locking() {
        ...
        let _home = EnvGuard::set("MONA_HOME", home.path().to_str().unwrap());
        ...
        std::fs::create_dir_all(desktop.join("crates/mona-desktop-ui/src")).unwrap();
        std::fs::write(
        ...
        .unwrap();
        std::fs::write(
            desktop.join("crates/mona-desktop-ui/Cargo.toml"),
            "[package]\nname = \"mona-desktop-ui\"\n",
        )
        .unwrap();
        agent
            .lock()
    ...
    async fn debug_tool_selfdev_reload_returns_promptly_for_direct_execution() {
        let _env_lock = lock_env();
        let _test_session = EnvGuard::set("MONA_TEST_SESSION", "1");
        let _debug_control = EnvGuard::set("MONA_DEBUG_CONTROL", "1");
