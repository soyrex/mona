...
pub fn is_release_build() -> bool {
    mona_build_meta::is_release_build()
}

fn current_update_semver() -> &'static str {
    mona_build_meta::update_semver()
}
...
fn source_build_root() -> Result<PathBuf> {
    Ok(storage::mona_dir()?.join("builds").join("source"))
}

fn source_build_repo_dir() -> Result<PathBuf> {
    Ok(source_build_root()?.join("mona"))
}

pub fn should_auto_update() -> bool {
    if std::env::var("MONA_NO_AUTO_UPDATE").is_ok() {
        return false;
    }
...
fn github_api_request(
    ...
    if let Some(token) = mona_base::github::github_public_api_token() {
...
fn prepare_stable_update_blocking() -> Result<PreparedUpdate> {
    let current_version = mona_build_meta::version();
...
fn prepare_main_update_blocking() -> Result<PreparedUpdate> {
    let current_hash = mona_build_meta::git_hash();
...
fn build_from_source() -> Result<PathBuf> {
    ...
    let repo_dir = build_dir.join("mona");
    ...
    if repo_dir.join(".git").exists() {
        ...
        let output = std::process::Command::new("git")
            .args([
                "clone", "--depth", "1", "--branch", "main", &clone_url, "mona",
            ])
...
pub fn download_and_install_blocking_with_progress(
    ...
    let temp_path = temp_dir.join(format!("mona-update-{}", std::process::id()));
    ...
    if asset.name.ends_with(".tar.gz") {
        ...
        for entry in archive.entries()? {
            ...
            entry.unpack(&dest)?;
            if file_name.starts_with("mona") && !file_name.ends_with(".bin") {
...
pub fn check_and_maybe_update(auto_install: bool) -> UpdateCheckResult {
    ...
    match check_for_update_blocking() {
        Ok(Some(release)) => {
            let current = mona_build_meta::version().to_string();
...
mod tests {
    use super::*;
    use mona_update_core::parse_sha256sums;
    ...
    fn test_parse_sha256sums_accepts_standard_and_binary_lines() {
        ...
        let contents = format!(
            "# generated by release workflow\n{}  mona-linux-x86_64.tar.gz\r\n{} *mona-windows-x86_64.exe\n",
            digest_a, digest_b
        );
        let parsed = parse_sha256sums(&contents).unwrap();
        assert_eq!(
            parsed.get("mona-linux-x86_64.tar.gz").map(String::as_str),
            Some(digest_a.as_str())
        );
        assert_eq!(
            parsed.get("mona-windows-x86_64.exe").map(String::as_str),
    ...
    fn test_verify_asset_checksum_text_accepts_matching_digest() {
        ...
        let contents = format!("{}  mona-linux-x86_64.tar.gz\n", digest);
        verify_asset_checksum_text(&contents, "mona-linux-x86_64.tar.gz", bytes).unwrap();
    }
    ...
    fn test_verify_asset_checksum_text_rejects_mismatch() {
        ...
        let contents = format!("{}  mona-linux-x86_64.tar.gz\n", wrong);
        let err = verify_asset_checksum_text(&contents, "mona-linux-x86_64.tar.gz", b"actual")
            .unwrap_err()
            .to_string();
    ...
    fn test_parse_sha256sums_rejects_invalid_digest() {
        let err = parse_sha256sums("not-a-sha  mona-linux-x86_64.tar.gz\n")
            .unwrap_err()
            .to_string();
    ...
    fn test_summarize_git_pull_failure_diverged() {
        ...
        assert_eq!(
            ...
            mona_update_core::GIT_PULL_DIVERGED_SUMMARY
        );
        assert!(mona_update_core::summary_is_divergence(
            &summarize_git_pull_failure(stderr)
        ));
    }
