...
impl IsolatedEnv {
    fn new() -> Self {
        ...
        let previous_opt_out = std::env::var_os("MONA_NO_TELEMETRY");
        crate::env::set_var("MONA_HOME", home.path());
        crate::env::set_var("MONA_NO_TELEMETRY", "1");
        Self {
            _home: home,
            previous_home,
            previous_opt_out,
...
impl Drop for IsolatedEnv {
    fn drop(&mut self) {
        for (key, value) in [
            ("MONA_HOME", self.previous_home.take()),
            ("MONA_NO_TELEMETRY", self.previous_opt_out.take()),
        ] {
