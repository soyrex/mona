...
fn checkout(root: &std::path::Path) -> std::path::PathBuf {
    ...
    std::fs::create_dir_all(repo.join("crates/mona-desktop-ui/src")).unwrap();
    std::fs::write(
    ...
    .unwrap();
    std::fs::write(
        repo.join("crates/mona-desktop-ui/Cargo.toml"),
...
async fn desktop_selfdev_is_automatic_separate_and_restored() {
    ...
    let cwd = repo.join("crates/mona-desktop-ui/src");
    ...
    for name in ["selfdev", "debug_socket", "mona_docs"] {
    ...
    assert!(ordinary.iter().any(|t| t.name == "mona_docs"));
