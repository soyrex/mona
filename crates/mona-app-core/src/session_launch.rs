...
/// spawn hooks and spawned terminals as `MONA_SPAWN_*` env vars so external
...
pub struct SessionSpawnContext {
    ...
    /// Extra `MONA_SPAWN_*` env entries (e.g. swarm/coordinator ids).
...
/// `MONA_FOCUS_SESSION_ID` and `MONA_FOCUS_TITLE` env vars.
...
/// var is exported natively and under a `MONA_CLIENT_<NAME>` alias.
pub fn focus_session_via_hook_with_env(
    ...
    cmd.args(args)
        .env("MONA_FOCUS_SESSION_ID", session_id)
        .env("MONA_FOCUS_TITLE", title)
...
fn focus_title_best_effort(title: &str) {
    ...
    cmd.arg("-c")
        .arg(
            "sleep 0.4; \
             if command -v wmctrl >/dev/null 2>&1; then wmctrl -a \"$MONA_WINDOW_TITLE\" >/dev/null 2>&1 && exit 0; fi; \
             if command -v xdotool >/dev/null 2>&1; then xdotool search --name \"$MONA_WINDOW_TITLE\" windowactivate >/dev/null 2>&1 && exit 0; fi; \
             exit 0",
        )
        .env("MONA_WINDOW_TITLE", title)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
...
fn resume_terminal_candidates_windows() -> Vec<String> {
    std::env::var("MONA_RESUME_TERMINAL")
        .ok()
        .map(|value| {
            value
                .split(',')
...
pub fn spawn_resume_in_new_terminal_with_context(
    ...
    let mut mona_args: Vec<String> = Vec::new();
    if let Some(provider_arg) = resume_provider_arg(provider_key) {
        mona_args.push("--provider".to_string());
        mona_args.push(provider_arg.to_string());
    }
    mona_args.push("--resume".to_string());
    mona_args.push(session_id.to_string());

    let hook_command = crate::terminal_launch::TerminalCommand::new(exe, mona_args.clone())
