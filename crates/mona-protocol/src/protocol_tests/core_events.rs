...
fn test_side_pane_images_event_roundtrip() -> Result<()> {
    let event = ServerEvent::SidePaneImages {
        ...
        images: vec![mona_session_types::RenderedImage {
            history_message_index: None,
            media_type: "image/png".to_string(),
            data: "base64-data".to_string(),
            label: Some("openclaw.png".to_string()),
            source: mona_session_types::RenderedImageSource::ToolResult {
    ...
    assert_eq!(
        images[0].source,
        mona_session_types::RenderedImageSource::ToolResult {
            tool_name: "read".to_string(),
        }
    );
    Ok(())
...
fn test_history_event_decodes_without_compaction_mode_for_older_servers() -> Result<()> {
    ...
    assert_eq!(
        compaction_mode,
        mona_config_types::CompactionMode::Reactive
    );
...
fn test_history_event_roundtrip_preserves_side_panel_snapshot() -> Result<()> {
    let event = ServerEvent::History {
        ...
        compaction_mode: mona_config_types::CompactionMode::Reactive,
        activity: None,
        side_panel: mona_side_panel_types::SidePanelSnapshot {
            focus_revision: 0,
            focused_page_id: Some("page-1".to_string()),
            pages: vec![mona_side_panel_types::SidePanelPage {
                id: "page-1".to_string(),
                title: "Notes".to_string(),
                file_path: "/tmp/notes.md".to_string(),
                format: mona_side_panel_types::SidePanelPageFormat::Markdown,
                pdf_data: None,
                source: mona_side_panel_types::SidePanelPageSource::Managed,
...
fn test_side_panel_state_event_roundtrip() -> Result<()> {
    let event = ServerEvent::SidePanelState {
        snapshot: mona_side_panel_types::SidePanelSnapshot {
            focus_revision: 0,
            focused_page_id: Some("page-1".to_string()),
            pages: vec![mona_side_panel_types::SidePanelPage {
                id: "page-1".to_string(),
