...
fn test_input_shell_result_event_roundtrip() -> Result<()> {
    let event = ServerEvent::InputShellResult {
        result: mona_message_types::InputShellResult {
...
fn test_structured_set_route_decodes_as_set_route_not_set_model() -> Result<()> {
    ...
    let request = Request::SetRoute {
        id: 7,
        selection: mona_provider_core::RouteSelection {
            model: "gpt-5.5".to_string(),
            runtime_key: mona_provider_core::RuntimeKey::OpenAIApiKey,
...
#[test]
fn test_mona_subscription_set_route_is_wire_safe() -> Result<()> {
    let request = Request::SetRoute {
        id: 8,
        selection: mona_provider_core::RouteSelection {
            model: "gpt-5.5".to_string(),
            runtime_key: mona_provider_core::RuntimeKey::JcodeSubscription,
            api_method: "mona-subscription".to_string(),
    ...
    let line = serde_json::to_string(&request)?;
    assert!(line.contains("\"type\":\"set_route\""));
    assert!(line.contains("\"kind\":\"mona-subscription\""));
...
fn test_grok_build_set_route_is_wire_safe() -> Result<()> {
    let request = Request::SetRoute {
        ...
        selection: mona_provider_core::RouteSelection {
            ...
            runtime_key: mona_provider_core::RuntimeKey::GrokBuild,
    ...
    assert_eq!(
        selection.runtime_key,
        mona_provider_core::RuntimeKey::GrokBuild
    );
...
fn test_unknown_runtime_key_other_set_route_is_wire_safe() -> Result<()> {
    let request = Request::SetRoute {
        ...
        selection: mona_provider_core::RouteSelection {
            ...
            runtime_key: mona_provider_core::RuntimeKey::Other {
    ...
    assert_eq!(
        selection.runtime_key,
        mona_provider_core::RuntimeKey::Other {
            method: "custom-acp".to_string()
        }
    );
    Ok(())
