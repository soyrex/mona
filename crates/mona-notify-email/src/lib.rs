...
pub fn poll_imap_once(host: &str, port: u16, user: &str, pass: &str) -> Result<Vec<ReplyAction>> {
    ...
    let button_search = session.search("UNSEEN SUBJECT \"[mona-perm:\"")?;
            ...
            && let Some(parsed) = mail_parser::MessageParser::default().parse(body)
        {
            ...
            let cycle_id = if in_reply_to.contains("@jcode>") {
                in_reply_to
                    ...
                    .to_string()
            } else if let Some(start) = subject.find("[mona-perm:") {
                let rest = &subject[start + "[mona-perm:".len()..];
...
pub fn build_permission_email_html(
    ...
    let approve_subj_raw = format!("[mona-perm:{}] Approved", request_id);
    let deny_subj_raw = format!("[mona-perm:{}] Denied", request_id);
