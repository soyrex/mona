...
//! renderer at a hanging toolchain via process-global `MONA_*_COMMAND`
...
fn assert_formula_resolves(text: impl Into<String>) {
    ...
    unsafe {
        std::env::remove_var("MONA_LATEX_COMMAND");
        std::env::remove_var("MONA_DVIPNG_COMMAND");
        std::env::remove_var("MONA_PDFLATEX_COMMAND");
        std::env::remove_var("MONA_PDFTOCAIRO_COMMAND");
    }

    let text = text.into();

    let render = || {
        mona_tui_mermaid::with_image_protocol_override(Some(true), || {
            mona_tui_markdown::render_markdown_with_width(&text, Some(90))
        })
    };
    ...
    let pending = first
        .iter()
        .any(mona_tui_markdown::line_is_mermaid_pending_placeholder);
    if !pending {
...
fn rendered_a_formula_image(lines: &[ratatui::text::Line<'static>]) -> bool {
    let has_placeholder = lines.iter().any(|line| {
        mona_tui_mermaid::parse_image_placeholder(line).is_some()
            || mona_tui_mermaid::parse_inline_image_placeholder(line).is_some()
    });
