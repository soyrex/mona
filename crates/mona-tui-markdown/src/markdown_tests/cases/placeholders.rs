...
fn test_blockquote_separators_preserve_image_fill_rows() {
    ...
    lines.extend(mona_tui_mermaid::inline_image_placeholder_lines(0xabcdef, 4, 40));
...
fn test_normalize_block_separators_preserves_inline_image_placeholder_body() {
    ...
    lines.extend(mona_tui_mermaid::inline_image_placeholder_lines(
        0xabcdef, rows, 40,
    ));
    lines.push(Line::from(""));
    lines.push(Line::from("after"));
    ...
    let marker_idx = lines
        .iter()
        .position(|line| mona_tui_mermaid::parse_inline_image_placeholder(line).is_some())
