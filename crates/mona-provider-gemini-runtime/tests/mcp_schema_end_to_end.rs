...
//!
//! The unit tests in `mona-schema-dialect` work on schema values, and the
//! sweep in `mona-app-core` works on jcode's own tools. Neither covers the
...
fn playwright_schema_reaches_gemini_without_the_keyword_that_caused_issue_754() {
    ...
    let built = mona_provider_gemini::build_tools(&defs).expect("gemini tools");
...
fn playwright_schema_reaches_openai_without_an_unsupported_construct() {
    let defs = playwright_tool_definitions();
    let built = mona_provider_openai::request::build_tools(&defs);
...
fn the_normalized_playwright_schema_conforms_to_every_dialect() {
    ...
    for spec in mona_schema_dialect::registry::ALL {
        let normalized = mona_schema_dialect::dialect::apply(schema, spec);
        let errors =
        ...
        let lost = mona_schema_dialect::must_preserve_meaning(schema, &normalized);
        assert!(
            lost.is_empty(),
...
fn recovery_triggers_on_the_error_string_the_runtime_really_builds() {
    ...
    let rejection = mona_schema_dialect::classify(&with_context)
