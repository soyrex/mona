...
//! key), moved out of `mona-base` so provider edits compile only this crate
...
impl GeminiProvider {
    fn persisted_catalog_path() -> Result<std::path::PathBuf> {
        Ok(mona_base::storage::app_config_dir()?.join("gemini_models_cache.json"))
    }
    ...
    pub fn new() -> Self {
        let model = std::env::var("MONA_GEMINI_MODEL").unwrap_or_else(|_| DEFAULT_MODEL.into());
        let provider = Self {
    ...
    /// cloudcode-pa tier. Set `MONA_GEMINI_FORCE_OAUTH=1` to pin OAuth even when
    /// a key is present.
    ...
    async fn post_json<T: DeserializeOwned>(
        ...
        loop {
            ...
            if !resp.status().is_success() {
                ...
                let retry_after = mona_provider_core::retry_after::retry_after(resp.headers());
    ...
    /// `mona-schema-dialect` remembers it so later requests never send it.
    ///
    ...
    async fn retry_after_schema_rejection(
        ...
        let dialect = &mona_schema_dialect::registry::GEMINI;
        match mona_schema_dialect::recover_from_error(error, dialect) {
            ...
            mona_schema_dialect::RecoveryAction::Unrecoverable { hint } => {
                mona_base::logging::warn(&format!("Gemini tool-schema rejection: {hint}"));
                None
            }
            mona_schema_dialect::RecoveryAction::RetryWithoutConstruct { description } => {
                mona_base::logging::warn(&format!("Gemini {description}"));
                Some(
...
impl Provider for GeminiProvider {
                ...
                if !produced_output {
                    ...
                    if abnormal {
                        ...
                        let detail = candidate
                            ...
                            .filter(|msg| !msg.trim().is_empty())
                            .map(|msg| {
                                format!(": {}", mona_base::util::truncate_str(msg.trim(), 300))
                            })
                            .unwrap_or_default();
                        let _ = tx
                            .send(Err(anyhow::anyhow!(
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        ...
        let trimmed = mona_provider_core::strip_own_model_prefix(model, "gemini:");
        if trimmed.is_empty() {
...
fn gemini_http_retry_delay(
    ...
    Some(mona_provider_core::retry_after::retry_delay(
        attempt,
        1500,
        retry_after,
    ))
...
fn gemini_http_client() -> reqwest::Client {
    ...
    reqwest::Client::builder()
        ...
        .build()
        .unwrap_or_else(|_| mona_provider_core::shared_http_client())
}

/// User-Agent matching the official Gemini CLI. Override with
/// `MONA_GEMINI_USER_AGENT` if Google changes the accepted format.
fn gemini_user_agent() -> String {
    if let Ok(ua) = std::env::var("MONA_GEMINI_USER_AGENT")
        && !ua.trim().is_empty()
    {
        return ua;
    }
...
fn is_transient_gemini_transport_error(err: &reqwest::Error) -> bool {
    ...
    err.is_connect()
        || err.is_timeout()
        || mona_provider_core::is_transient_transport_error(&err.to_string())
}
