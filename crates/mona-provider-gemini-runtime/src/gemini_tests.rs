use super::*;
use mona_base::message::{ContentBlock, Message, Role};
...
fn auth_mode_prefers_api_key_when_present() {
    ...
    let _force = EnvVarGuard::unset("MONA_GEMINI_FORCE_OAUTH");
...
fn auth_mode_force_oauth_overrides_api_key() {
    ...
    let _google = EnvVarGuard::unset("GOOGLE_API_KEY");
    let _key = EnvVarGuard::set_value("GEMINI_API_KEY", "test-developer-key");
    let _force = EnvVarGuard::set_value("MONA_GEMINI_FORCE_OAUTH", "1");
    mona_base::config::Config::invalidate_cache();
...
fn fully_unsigned_history_has_no_signature_to_replay() {
    ...
    let contents = mona_provider_gemini::build_contents(&messages);
...
fn downgrade_policy_removes_every_function_call_part() {
    let messages = unsigned_tool_history();
    let contents = mona_provider_gemini::build_contents_with_signature_policy(
        &messages,
...
fn missing_thought_signature_errors_are_recognized_from_backend_bodies() {
    ...
    assert!(mona_provider_gemini::is_missing_thought_signature_error(
...
#[test]
fn gemini_uses_mona_compaction_so_long_sessions_have_a_safety_net() {
    ...
    assert!(
        provider.uses_mona_compaction(),
        "uses_mona_compaction() inherits supports_compaction(); both must be true \
...
fn runtime_state_key_changes_when_configured_project_or_auth_route_changes() {
    ...
    let _force = EnvVarGuard::unset("MONA_GEMINI_FORCE_OAUTH");
    let _key = EnvVarGuard::set_value("GEMINI_API_KEY", "test-developer-key");
    let mut cfg = mona_base::config::Config::default();
...
fn post_json_retries_transient_http_responses_against_local_server() {
    ...
    let _home = EnvVarGuard::set_path("MONA_HOME", home.path());
...
fn ensure_state_replaces_oauth_cache_when_switching_to_api_key() {
    ...
    let _home = EnvVarGuard::set_path("MONA_HOME", home.path());
    let _key = EnvVarGuard::set_value("GEMINI_API_KEY", "test-developer-key");
    let _force = EnvVarGuard::set_value("MONA_GEMINI_FORCE_OAUTH", "false");
    mona_base::config::Config::invalidate_cache();
    let runtime = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
