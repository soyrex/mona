...
//!
//! Usage: mona-harness-api-bridge [api_socket] [legacy_socket]
...
async fn main() -> anyhow::Result<()> {
    ...
    let api_socket = args
        ...
        .unwrap_or_else(mona_harness_api_server::api_socket_path);
    let legacy_socket = args
        .next()
        .map(std::path::PathBuf::from)
        .unwrap_or_else(mona_harness_api_server::legacy_socket_path);
    mona_harness_api_server::run_bridge(api_socket, legacy_socket).await
}
