...
//! - Listens on `~/.mona/mona-api.sock` (or `MONA_API_SOCKET`).
//! - For each API client, dials the legacy daemon socket (`MONA_SOCKET` or
//!   `~/.mona/jcode.sock`) and speaks `subscribe`/`message`/... on its
//!   behalf.
...
use mona_harness_api::{API_VERSION_MAJOR, ApiEvent, ErrorCode, ServerFrame};
...
// all.
use mona_transport::{Listener, Stream};

// Socket paths live in `mona-harness-api` so clients and the bridge can never
// resolve different directories (they once did, and the desktop app could not
// connect as a result).
pub use mona_harness_api::{api_socket_path, legacy_socket_path};
    ...
    W: tokio::io::AsyncWrite + Unpin,
{
    ...
    let hello_ok = ServerFrame::reply(
        ...
        ApiEvent::HelloOk {
            ...
            server: format!("mona-harness-api-bridge/{}", env!("CARGO_PKG_VERSION")),
            capabilities: [
                "sessions",
                "streaming",
                "text_framing",
    ...
    let mut state =
        translate::BridgeState::with_crash_on_disconnect(client_name.starts_with("mona-desktop-"));
...
mod public_acceptance_tests {
    ...
    async fn stdio_does_not_confirm_handshake_when_native_daemon_is_missing() {
        ...
        let task = tokio::spawn(run_bridge_stream(
            read,
            write,
            PathBuf::from("/nonexistent-mona-stdio-test/daemon.sock"),
        ));
        write_json_line(
            &mut client,
    ...
    async fn public_socket_keeps_its_attachment_after_another_sessions_state() {
        let _home_lock = translate::mona_home_test_lock();
        ...
        unsafe { std::env::set_var("MONA_HOME", &root) };
...
mod single_instance_tests {
    ...
    fn a_second_bridge_cannot_take_the_socket() {
        let dir = std::env::temp_dir().join(format!("mona-bridge-lock-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let socket = dir.join("mona-api.sock");
...
mod socket_permission_tests {
    ...
    async fn the_api_socket_is_owner_only() {
        ...
        let dir = std::env::temp_dir().join(format!("mona-api-perm-{}", std::process::id()));
...
mod fragmented_reply_tests {
    ...
    async fn concurrent_history_replies_survive_api_requests_between_fragments() {
        tokio::time::timeout(std::time::Duration::from_secs(10), async {
            let path = std::env::temp_dir().join(format!("mona-fragments-{}-{}.sock", std::process::id(), std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos()));
            let listener = UnixListener::bind(&path).unwrap();
