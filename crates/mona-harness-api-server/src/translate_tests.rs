...
impl ScopedJcodeHome {
    fn new(label: &str) -> Self {
        ...
        let path = std::env::temp_dir().join(format!(
            "mona-harness-api-{label}-{}-{unique}",
            std::process::id()
        ));
        std::fs::create_dir_all(&path).expect("create isolated MONA_HOME");
        // SAFETY: all tests in this module that mutate MONA_HOME share `LOCK`,
        // and this guard restores the prior value before it is released.
        unsafe { std::env::set_var("MONA_HOME", &path) };
        Self {
            path,
            previous,
            _guard: guard,
...
impl Drop for ScopedJcodeHome {
    fn drop(&mut self) {
        match self.previous.take() {
            Some(value) => unsafe { std::env::set_var("MONA_HOME", value) },
            None => unsafe { std::env::remove_var("MONA_HOME") },
        }
...
#[test]
fn create_session_in_a_mona_checkout_requests_selfdev() {
...
fn model_usage_before_catalog_is_retained_and_legacy_routes_stay_optional() {
    ...
    let legacy: mona_harness_api::ModelRouteInfo = serde_json::from_value(route).unwrap();
...
///
/// `launch()` gives an embedded instance its own `MONA_HOME` precisely so it
...
fn session_records_are_read_from_the_instance_home() {
    ...
    assert!(
        ...
        "MONA_HOME must scope session records, got {}",
        path.display()
    );
}
...
fn credential_provisioning_normalizes_gemini_and_supports_jcode() {
    ...
    assert_eq!(notify["provider"], "mona");
    assert_eq!(
        std::fs::read_to_string(config.join("mona-subscription.env")).unwrap(),
        "MONA_API_KEY=mona-secret\n"
    );
...
fn owner_only_writes_refuse_symlink_targets_and_directories() {
    ...
    assert!(!outside_dir.join("mona-subscription.env").exists());
}
...
fn list_and_attach_expose_swarm_ownership_without_nesting_forks() {
    ...
    // that already set MONA_RUNTIME_DIR.
    let previous_runtime = std::env::var_os("MONA_RUNTIME_DIR");
    struct RuntimeGuard(Option<OsString>);
    impl Drop for RuntimeGuard {
        fn drop(&mut self) {
            match self.0.take() {
                Some(value) => unsafe { std::env::set_var("MONA_RUNTIME_DIR", value) },
                None => unsafe { std::env::remove_var("MONA_RUNTIME_DIR") },
            }
        }
    }
    let _runtime = RuntimeGuard(previous_runtime);
    let runtime = home.path.join("runtime");
    unsafe { std::env::set_var("MONA_RUNTIME_DIR", &runtime) };
...
fn history_response_stats_cross_real_render_protocol_and_sdk_boundary() {
    let mut session = mona_base::session::Session::create(None, None);
    ...
    let legacy: Vec<_> = mona_base::session::render_messages(&session)
        .into_iter()
        .map(|row| mona_base::protocol::HistoryMessage {
            role: row.role,
            content: row.content,
            tool_calls: None,
...
fn session_list_exposes_durable_edit_stats_without_phantom_sidecar_sessions() {
    ...
    let stats = mona_harness_api::SessionEditStats {
        added: 42,
        removed: 9,
        approximate: false,
    };
    mona_harness_api::record_session_edit(&home.path.join("sessions"), "session_edits", stats)
        .unwrap();
    let mut state = BridgeState::default();
