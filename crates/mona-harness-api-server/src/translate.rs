...
#[cfg(test)]
pub(crate) fn mona_home_test_lock() -> std::sync::MutexGuard<'static, ()> {
...
pub struct BridgeState {
    ...
    model_usage_updates: BTreeMap<(String, String, String), mona_harness_api::ModelUsage>,
}
...
impl BridgeState {
        ...
        match req {
            ...
            "create_session" | "attach_session" => {
                ...
                if req == "create_session" {
                    ...
                    if working_dir
                        .as_deref()
                        .is_some_and(Self::path_is_inside_mona_repo)
                    {
            ...
            "list_sessions" => {
                ...
                let mut sessions: Vec<_> = ids
                    ...
                    .collect();
                mona_harness_api::enrich_sessions_from_local_swarm_state(&mut sessions);
                mona_harness_api::enrich_sessions_from_local_edit_stats(&mut sessions);
    ...
    fn side_panel_frame(session_id: &str, snapshot: &Value) -> Option<ServerFrame> {
        ...
        let snapshot = if snapshot.is_null() {
            mona_harness_api::SidePanelSnapshot::default()
        } else {
    ...
    pub fn legacy_event_to_api(&mut self, event: &Value) -> Vec<ServerFrame> {
        ...
        let mut frames = match kind {
            ...
            "model_usage_updated" => {
                ...
                let (Some(model), Some(provider), Some(api_method), Ok(usage)) = (
                    ...
                    serde_json::from_value::<mona_harness_api::ModelUsage>(route["usage"].clone()),
                ) else {
                    return vec![];
                };
                let observed = self
        ...
        for frame in &mut frames {
            if let ApiEvent::Attached { session } | ApiEvent::SessionForked { session } =
                &mut frame.event
            {
                mona_harness_api::enrich_sessions_from_local_edit_stats(std::slice::from_mut(
                    session,
                ));
                mona_harness_api::enrich_sessions_from_local_swarm_state(std::slice::from_mut(
                    session,
                ));
            }
        }
    ...
    fn path_is_inside_mona_repo(path: &str) -> bool {
        let mut current = Some(std::path::Path::new(path));
        while let Some(dir) = current {
            if dir.join("Cargo.toml").is_file() && dir.join("crates/mona-base").is_dir() {
                return true;
            }
            current = dir.parent();
        }
    ...
    /// plain session id.
    ///
    /// One funnel for three reasons. It honours `MONA_HOME`, without which a
    ...
    fn session_record_path(session_id: &str) -> Option<std::path::PathBuf> {
        ...
        let home = match std::env::var_os("MONA_HOME") {
    ...
    fn mona_home() -> Option<std::path::PathBuf> {
    ...
    fn recent_session_index_path() -> Option<std::path::PathBuf> {
        Some(Self::mona_home()?.join("session-metadata-v1.sqlite3"))
    }
    ...
    fn stored_session_ids(limit: Option<usize>) -> Vec<String> {
        ...
        let Some(home) = Self::mona_home() else {
            return Vec::new();
        };
    ...
    fn archive_state_path() -> Option<std::path::PathBuf> {
        Some(Self::mona_home()?.join("sdk-archive.json"))
    }
    ...
    fn app_config_dir() -> Option<std::path::PathBuf> {
        if let Some(home) = std::env::var_os("MONA_HOME") {
        ...
        return Some(std::path::Path::new(&std::env::var_os("APPDATA")?).join("mona"));
    ...
    fn credential_binding(
        ...
        match provider.trim().to_ascii_lowercase().as_str() {
            ...
            "mona" | "subscription" | "mona-subscription" => {
                Some(("mona", &["MONA_API_KEY"], "mona-subscription.env"))
            }
            _ => None,
        }
    }
