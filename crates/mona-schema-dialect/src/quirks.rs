...
//! knowledge dies with the process. Writing it to `~/.mona/schema-quirks.json`
...
fn store_path() -> Option<PathBuf> {
    ...
    // developer's real `~/.mona/schema-quirks.json`, silently stripping that
    ...
    {
        if let Ok(explicit) = std::env::var("MONA_SCHEMA_QUIRKS_PATH") {
            return Some(PathBuf::from(explicit));
        }
        let home = if let Ok(mona_home) = std::env::var("MONA_HOME") {
            PathBuf::from(mona_home)
        } else {
...
/// developer's real `~/.mona/schema-quirks.json`.
#[cfg(any(test, feature = "test-support"))]
...
/// alternate `MONA_HOME`) never read each other's state.
