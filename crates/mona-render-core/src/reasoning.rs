...
//! consumes them. These live in `mona-render-core` (a backend-neutral, pure
//! crate) rather than in `mona-tui-markdown` so the foundation/streaming layer
//! can format reasoning lines without depending on any `mona-tui-*` crate.
...
///
/// Lives here (a backend-neutral, pure crate) rather than in `mona-tui-markdown`
/// so the foundation/streaming layer can format the summary without depending on
/// any `mona-tui-*` crate. Re-exported from `mona-tui-markdown` for the
/// existing `mona_tui_markdown::reasoning_summary_line_markup` path.
