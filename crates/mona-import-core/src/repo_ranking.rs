...
pub struct PlannedHotkey {
    /// mona-style chord string, e.g. `cmd+;` or `cmd+[`.
    pub chord: String,
...
pub fn collect_mona_session_locations(sessions_dir: &Path) -> Vec<SessionLocation> {
    use std::fs;
...
pub fn plan_launch_hotkeys_from_sessions(
    ...
    let locations = collect_mona_session_locations(sessions_dir);
...
mod tests {
    ...
    fn collect_reads_working_dirs_and_skips_junk() {
        ...
        write(
            "session_a.json",
            r#"{"working_dir":"/Users/jeremy/mona-github","id":"a"}"#,
        );
        write(
            "session_b.json",
    ...
    fn collect_missing_dir_is_empty() {
        let locs = collect_mona_session_locations(Path::new("/nonexistent/jcode/sessions"));
