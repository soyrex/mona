...
pub struct TerminalCommand {
    ...
    /// Exported as `MONA_SPAWN_KIND` to spawn hooks and spawned terminals.
    pub kind: Option<String>,
    /// The jcode session this terminal will run, when known.
    /// Exported as `MONA_SPAWN_SESSION_ID`.
    pub session_id: Option<String>,
    /// Extra metadata env entries (e.g. `MONA_SPAWN_SWARM_ID`) exported to
    /// spawn hooks and spawned terminals. Applied after the first-class
    /// `MONA_SPAWN_*` keys, so entries here win on key collisions.
    ...
    /// and are also exported under a `MONA_CLIENT_*` prefix so spawn/focus
...
pub fn apply_client_terminal_env(cmd: &mut Command, env: &[(String, String)]) {
    for key in CLIENT_TERMINAL_ENV_VARS {
        cmd.env_remove(key);
        cmd.env_remove(format!("MONA_CLIENT_{key}"));
    }
    for (key, value) in env {
        if CLIENT_TERMINAL_ENV_VARS.contains(&key.as_str()) {
            cmd.env(key, value);
            cmd.env(format!("MONA_CLIENT_{key}"), value);
        }
    }
}
...
fn resume_terminal_candidates_with_client_env(
    ...
    // MONA_TERMINAL and configured spawn hooks still take precedence.
...
pub fn resume_terminal_candidates() -> Vec<String> {
    let configured_terminal = std::env::var("MONA_TERMINAL").ok();
...
pub fn resume_terminal_candidates() -> Vec<String> {
    let mut candidates = Vec::new();
    if let Ok(term) = std::env::var("MONA_TERMINAL") {
        push_unique_terminal(&mut candidates, term);
    }
...
/// The `MONA_SPAWN_*` metadata env exported to spawn hooks and to terminals
/// launched by the built-in fallback:
///
/// - `MONA_SPAWN_KIND`: why this spawn happened ("resume", "selfdev",
///   "swarm-agent", ...), when known.
/// - `MONA_SPAWN_SESSION_ID`: the jcode session the window will run.
/// - `MONA_SPAWN_TITLE`: the suggested window/tab title.
/// - `MONA_SPAWN_CWD`: the working directory for the session.
/// - `MONA_SPAWN_PROGRAM`: path of the jcode binary to execute.
/// - `MONA_SPAWN_COMMAND`: the full command line, shell-escaped, for hooks
///   (like tmux) that take a single shell-command string.
///
/// `TerminalCommand::extra_env` entries (e.g. `MONA_SPAWN_SWARM_ID`,
/// `MONA_SPAWN_COORDINATOR_SESSION_ID`) are appended last and win collisions.
fn spawn_metadata_env(command: &TerminalCommand, cwd: &Path) -> Vec<(String, String)> {
    let mut env: Vec<(String, String)> = Vec::new();
    if let Some(kind) = &command.kind {
        env.push(("MONA_SPAWN_KIND".to_string(), kind.clone()));
    }
    if let Some(session_id) = &command.session_id {
        env.push(("MONA_SPAWN_SESSION_ID".to_string(), session_id.clone()));
    }
    if let Some(title) = &command.title {
        env.push(("MONA_SPAWN_TITLE".to_string(), title.clone()));
    }
    env.push((
        "MONA_SPAWN_CWD".to_string(),
        cwd.to_string_lossy().into_owned(),
    ));
    env.push((
        "MONA_SPAWN_PROGRAM".to_string(),
        command.program.to_string_lossy().into_owned(),
    ));
    env.push((
        "MONA_SPAWN_COMMAND".to_string(),
    ...
    // `MONA_CLIENT_<NAME>` alias so hooks can explicitly distinguish the
    // client's terminal from the server's.
    for (key, value) in &command.client_terminal_env {
        env.push((key.clone(), value.clone()));
        env.push((format!("MONA_CLIENT_{key}"), value.clone()));
    }
...
/// `MONA_SPAWN_*` metadata env set (see [`spawn_metadata_env`]); hooks that
/// need a single shell-command string (tmux, kitty `@ launch`) can use
/// `$MONA_SPAWN_COMMAND` instead of the appended argv.
...
fn build_spawn_command(term: &str, command: &TerminalCommand, cwd: &Path) -> Option<Command> {
    let title = command.title.as_deref().unwrap_or("mona");
    ...
    match term {
        ...
        "herdr" => {
            ...
            cmd.current_dir(cwd)
                ...
                .stdout(Stdio::null())
                .stderr(Stdio::null())
                .args(["-c", script, "mona-herdr-spawn", &herdr])
                .arg(cwd)
                .arg(shell);
            if command.fresh_spawn {
                cmd.env("MONA_FRESH_SPAWN", "1");
    ...
    // not inherit this env, matching the existing MONA_FRESH_SPAWN caveat.
    for (key, value) in spawn_metadata_env(command, cwd) {
        cmd.env(key, value);
    }
...
mod tests {
    ...
    fn client_terminal_env_replaces_inherited_identity_and_exports_aliases() {
        ...
        assert_eq!(
            env["MONA_CLIENT_HERDR_PANE_ID"].as_deref(),
            Some("client-pane")
        );
        assert_eq!(env["TMUX_PANE"], None);
        assert!(!env.contains_key("UNTRUSTED_CLIENT_VAR"));
        assert!(!env.contains_key("MONA_CLIENT_UNTRUSTED_CLIENT_VAR"));
    }
    ...
    fn spawn_metadata_env_reexports_client_terminal_env_with_native_and_client_keys() {
        ...
        // inherited/stale value) and under a `MONA_CLIENT_*` alias so hooks can
        ...
        assert_eq!(
            lookup("MONA_CLIENT_ZELLIJ_SESSION_NAME").as_deref(),
            Some("sessionB")
        );
        assert_eq!(lookup("DISPLAY").as_deref(), Some(":1"));
        assert_eq!(lookup("MONA_CLIENT_DISPLAY").as_deref(), Some(":1"));
        // The first-class spawn metadata still flows through.
        assert_eq!(lookup("MONA_SPAWN_KIND").as_deref(), Some("swarm-agent"));
    }
    ...
    fn spawn_metadata_env_without_client_env_has_no_client_keys() {
        ...
        let env = spawn_metadata_env(&command, Path::new("/tmp/work"));
        assert!(
            !env.iter().any(|(k, _)| k.starts_with("MONA_CLIENT_")),
            "no client terminal env should produce no MONA_CLIENT_* keys"
        );
    }
    ...
    fn missing_tmux_binary_falls_back_to_detected_emulator() {
        let _guard = ENV_LOCK.lock().unwrap();
        let previous_terminal = std::env::var_os("MONA_TERMINAL");
        unsafe {
            std::env::remove_var("MONA_TERMINAL");
        }
        let command =
        ...
        match previous_terminal {
            Some(value) => unsafe { std::env::set_var("MONA_TERMINAL", value) },
            None => unsafe { std::env::remove_var("MONA_TERMINAL") },
        }
    ...
    fn herdr_spawn_splits_calling_pane_and_runs_resume_command() {
        ...
        assert_eq!(args[2], "mona-herdr-spawn");
    ...
    fn shell_command_quotes_arguments() {
        let shell = shell_command(&["mona".to_string(), "it's ok".to_string()]);
        #[cfg(unix)]
    ...
    #[test]
    #[cfg(not(unix))]
    fn windows_cmd_fallback_runs_mona_under_cmd_k() {
        let command = TerminalCommand::new(
        ...
        let args: Vec<String> = cmd
            ...
            .map(|arg| arg.to_string_lossy().into_owned())
            .collect();
        assert_eq!(&args[..5], ["/C", "start", "mona", "cmd.exe", "/K"]);
    ...
    fn macos_terminal_inner_script_injects_fresh_spawn() {
        ...
        assert_eq!(
            script,
            "cd '/tmp' && exec env MONA_FRESH_SPAWN=1 '/usr/local/bin/jcode'"
        );
    }
    ...
    fn hook_spawn_command_appends_program_args_and_exports_metadata() {
        ...
        .kind("swarm-agent")
        .session_id("ses_abc")
        .spawn_env("MONA_SPAWN_SWARM_ID", "swarm-1")
        .fresh_spawn();
        ...
        assert_eq!(
            env_value(&cmd, "MONA_SPAWN_KIND").as_deref(),
            Some("swarm-agent")
        );
        assert_eq!(
            env_value(&cmd, "MONA_SPAWN_SESSION_ID").as_deref(),
            Some("ses_abc")
        );
        assert_eq!(
            env_value(&cmd, "MONA_SPAWN_TITLE").as_deref(),
            Some("🦊 jcode ses_abc")
        );
        assert_eq!(
            env_value(&cmd, "MONA_SPAWN_CWD").as_deref(),
            Some("/work/dir")
        );
        assert_eq!(
            env_value(&cmd, "MONA_SPAWN_PROGRAM").as_deref(),
            Some("/usr/local/bin/jcode")
        );
        #[cfg(unix)]
        assert_eq!(
            env_value(&cmd, "MONA_SPAWN_COMMAND").as_deref(),
            Some("'/usr/local/bin/jcode' '--resume' 'ses_abc'")
        );
        assert_eq!(
            env_value(&cmd, "MONA_SPAWN_SWARM_ID").as_deref(),
            Some("swarm-1")
        );
        assert_eq!(env_value(&cmd, "MONA_FRESH_SPAWN").as_deref(), Some("1"));
    }
