...
use mona_provider_gemini::CodeAssistGenerateResponse;
...
const VERSION_ENV: &str = "MONA_ANTIGRAVITY_VERSION";
...
pub fn antigravity_dialect(model: &str) -> &'static mona_schema_dialect::DialectSpec {
    if model_is_gemini(model) {
        &mona_schema_dialect::registry::GEMINI
    } else if model_is_claude(model) {
        &mona_schema_dialect::registry::ANTIGRAVITY_CLAUDE
    } else {
        &mona_schema_dialect::registry::ANTIGRAVITY_BRIDGE
    }
}
...
/// and `mona-schema-dialect` for why the subsets are allow-lists.
pub fn antigravity_compatible_schema(schema: &Value, model: &str) -> Value {
    mona_schema_dialect::normalize(schema, antigravity_dialect(model))
}
