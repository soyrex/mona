...
///
/// * `probe_override` is the parsed value of `MONA_MERMAID_PICKER_PROBE`
...
/// Parse only the explicit `MONA_MERMAID_PICKER_PROBE` override into a mode,
...
pub(super) fn infer_protocol_from_env(
    ...
    if term_program.contains("iterm") || term.contains("iterm") || lc_terminal.contains("iterm") {
        ...
        // MONA_ITERM2_IMAGES=1 to opt back in.
...
pub(super) fn iterm2_images_opt_in() -> bool {
    std::env::var("MONA_ITERM2_IMAGES")
        .ok()
        .as_deref()
...
pub(crate) fn prewarm_svg_font_db_async() {
    SVG_FONT_DB_PREWARM_STARTED.get_or_init(|| {
        let _ = std::thread::Builder::new()
            .name("mona-mermaid-fontdb-prewarm".to_string())
            .spawn(|| {
                let _ = &*SVG_FONT_DB;
            });
    });
...
/// on terminal capability responses. Set MONA_MERMAID_PICKER_PROBE=1 to run an
...
pub fn init_picker() {
    PICKER.get_or_init(|| {
        ...
        let probe_override = std::env::var("MONA_MERMAID_PICKER_PROBE")
            .ok()
            .as_deref()
            .and_then(parse_env_bool);
...
mod tests {
    ...
    fn rediscovery_fails_when_the_external_image_file_is_gone() {
        let path = std::env::temp_dir().join(format!(
            "mona-missing-external-image-{}-{:?}.png",
            std::process::id(),
            std::thread::current().id()
        ));
