//! Stress probe: build the mermaid source that `mona_plan::mermaid::
...
//!
//!   scripts/dev_cargo.sh run -p mona-tui-mermaid --features renderer \
//!       --example swarm_plan_stress
//!
//! Cases:
//!   (a) real deep-mode plan fixture (examples/swarm_plan_fixture.json,
//!       snapshot of ~/.mona/state/swarm/_home_jeremy_mona__git.json)
...
use mona_plan::PlanItem;
use mona_plan::mermaid::swarm_plan_mermaid;
...
fn probe(name: &str, src: &str) -> bool {
    match mona_tui_mermaid::render_mermaid_untracked(src, Some(100)) {
        ...
        mona_tui_mermaid::RenderResult::Error(err) => {
