...
//! legacy SVG-retarget backend (`MONA_MMDR_SIZE_API_DISABLE=1` is a
//! compile-time toggle consumed by build.rs, so it triggers a rebuild):
//!   cargo test -p mona-tui-mermaid --test layout_cache_cross_width_parity -- --ignored --nocapture
//!   MONA_MMDR_SIZE_API_DISABLE=1 cargo test -p mona-tui-mermaid --test layout_cache_cross_width_parity -- --ignored --nocapture
