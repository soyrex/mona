...
//!
//! Run with:
//!   cargo test --release -p mona-tui-mermaid --test layout_cache_resize_probe -- --ignored --nocapture
//!
...
use mona_tui_mermaid::{RenderResult, debug_stats, render_mermaid_untracked};

/// Deterministic ~40-node / 45-edge flowchart. There is no `infra` fixture in
/// this crate (checked `crates/mona-tui-mermaid/` for fixtures/), so this is
