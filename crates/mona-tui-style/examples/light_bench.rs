...
//! analyzer. Run with:
//!   cargo run -p mona-tui-style --example light_bench

use mona_tui_style::palette::{ALL_ROLES, Palette, Role, parse_hex, to_hex};
use mona_tui_style::{HarmonyReport, Oklab, analyze_harmony};
...
fn main() {
    ...
    let generated =
        mona_tui_style::harmony::generate_from_seed(Role::User.default_rgb(), LIGHT_BG);
