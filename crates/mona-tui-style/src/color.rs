...
/// and Apple Terminal; see `mona_app_core::perf` and issue #330). These
...
/// full truecolor.
///
/// Overridable with `MONA_GLYPH_SAFE_MODE=on|off` (shared with the perf
...
fn fragile_glyph_cache_terminal() -> bool {
    ...
    // Mirror of `mona_app_core::perf::detect_terminal` for the two affected
...
mod tests {
    ...
    fn test_fragile_terminal_override_on_forces_quantize() {
        temp_env_scope(&[("MONA_GLYPH_SAFE_MODE", Some("on"))], || {
            assert!(fragile_glyph_cache_terminal());
        });
    }
