...
- `~/.mona/builds/current/jcode` is the active local/source-build channel; self-dev builds and `scripts/install_release.sh` point the launcher here.
- `~/.mona/builds/stable/jcode` is the stable release channel; `scripts/install.sh` installs this and points the launcher here.
- `~/.mona/builds/versions/<version>/jcode` stores immutable binaries.
- `~/.mona/builds/canary/jcode` still exists for canary/testing flows, but it is not the primary self-dev install path.
...
`~/.mona/builds/shared-server/jcode`, which is a symlink into
`~/.mona/builds/versions/<version>/`. Until that symlink is repointed and the
...
./target/selfdev/jcode run --no-update --socket /run/user/1000/mona-mytest.sock '<prompt>'
```
