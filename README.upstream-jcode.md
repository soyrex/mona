...
<a href="https://github.com/1jehuang/jcode/releases/download/readme-assets/mona-yc-launch.mp4">
  <img src="https://github.com/1jehuang/jcode/releases/download/readme-assets/mona-yc-launch.webp" alt="jcode YC launch video" width="800">
</a>
...
<div align="center">
  ...
  <a href="https://github.com/1jehuang/jcode/releases/download/readme-assets/mona-performance-demo.mp4">
    <img src="https://github.com/1jehuang/jcode/releases/download/readme-assets/mona-performance-demo.webp" alt="jcode performance demonstration" width="900">
  </a>
...
<div align="center">
  ...
  <a href="https://github.com/1jehuang/jcode/releases/download/readme-assets/mona-memory-demo.mp4">
    <img src="https://github.com/1jehuang/jcode/releases/download/readme-assets/mona-memory-demo.webp" alt="jcode memory demonstration" width="900">
  </a>
...
To disable emoji globally in TUI and CLI output, set `emoji = false` under `[display]` in `~/.mona/config.toml`, or launch with `MONA_NO_EMOJI=1`. Jcode replaces emoji with compact ASCII markers while preserving other Unicode text.
...
<div align="center">
  ...
  <a href="https://github.com/1jehuang/jcode/releases/download/readme-assets/swarm-demo.mp4">
    <img src="https://github.com/1jehuang/jcode/releases/download/readme-assets/mona-swarm-demonstration.webp" alt="jcode swarm demonstration" width="900">
  </a>
...
`swarm_effort`. Environment overrides are `MONA_SWARM_ROOT_EFFORT` and
...
- `MONA_STREAM_IDLE_TIMEOUT_SECS` — raise the base streaming idle timeout (default 180s) for slow reasoning models that think silently before emitting tokens. High reasoning efforts scale this automatically (high 2x, xhigh 3x, max 4x). Also settable as `[provider] stream_idle_timeout_secs` in `config.toml`.
...
For agents and scripts, the preferred path is the one-shot provider profile command. It writes a named profile to `~/.mona/config.toml`, stores secrets in jcode's private app config directory when requested, and prints exact run/validation commands:
...
jcode --provider-profile my-api auth-test --prompt 'Reply exactly MONA_PROVIDER_SETUP_OK'
...
The generated profile can also be edited manually in `~/.mona/config.toml`:
...
api_key_env = "MONA_PROVIDER_MY_API_API_KEY"
...
2. For built-in profiles (e.g. `nvidia-nim`) or any endpoint, via the `MONA_OPENAI_EXTRA_BODY` environment variable (a JSON object string). It can live in the provider's env file (`~/.config/jcode/nvidia-nim.env`) next to the API key:

   ```bash
   MONA_OPENAI_EXTRA_BODY={"chat_template_kwargs":{"thinking":true,"reasoning_effort":"high"}}
   ```

Keys from `extra_body` are merged last and override any mona-generated body field with the same name (`MONA_OPENAI_EXTRA_BODY` wins over the config `extra_body` on key collisions). Invalid values are logged and ignored rather than failing the request.
...
```bash
MONA_OPENAI_COMPAT_API_BASE=http://192.168.1.50:8000/v1
MONA_OPENAI_COMPAT_DEFAULT_MODEL=Qwen/Qwen3-Coder-30B-A3B-Instruct
...
- `~/.mona/mcp.json` for global MCP servers
...
`~/.codex/config.toml` into `~/.mona/mcp.json` when the latter does not exist.
That imported file is then mona-owned; later Codex changes are not synced
...
Pending scriptable login state is stored under `~/.mona/pending-login/`, automatically expires, and stale entries are cleaned up when new scriptable logins start or resume.
...
<div align="center">
  ...
  <a href="https://github.com/1jehuang/jcode/releases/download/readme-assets/workflow.mp4">
    <img src="https://github.com/1jehuang/jcode/releases/download/readme-assets/mona-workflow-demonstration.webp" alt="jcode workflow demonstration" width="900">
  </a>
...
4. Before attempting any interactive login flow, assess which providers are already available non-interactively and prefer those first. Check existing local credentials, config files, CLI sessions, and environment variables such as:
   - Claude: `~/.mona/auth.json`, `~/.claude/.credentials.json`, `~/.local/share/opencode/auth.json`, `ANTHROPIC_API_KEY`
   - OpenAI: `~/.mona/openai-auth.json`, `~/.codex/auth.json`, `OPENAI_API_KEY`
   - Gemini: `~/.mona/gemini_oauth.json`, `~/.gemini/oauth_creds.json`
